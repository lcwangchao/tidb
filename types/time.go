...
func (t Time) check(sc Context, explicitTz *gotime.Location) error {
	...
	switch t.Type() {
	case mysql.TypeTimestamp:
		err = checkTimestampType(sc, t.coreTime, explicitTz)
	case mysql.TypeDatetime, mysql.TypeDate:
		err = checkDatetimeType(sc, t.coreTime)
	}
	return errors.Trace(err)
}
...
func ParseDuration(sc Context, str string, fsp int) (Duration, bool, error) {
	...
	if !canFallbackToDateTime(rest) {
		return d, isNull, truncatedWrongVal(sc, "time", str)
	}
...
func parseTime(sc Context, str string, tp byte, fsp int, isFloat bool, explicitTz *gotime.Location) (Time, error) {
	...
	t, err := parseDatetime(sc, str, fsp, isFloat, explicitTz)
	if err != nil {
		return NewTime(ZeroCoreTime, tp, DefaultFsp), errors.Trace(err)
	}
...
func ParseTimeFromNum(sc Context, num int64, tp byte, fsp int) (Time, error) {
	...
	if num == 0 {
		...
		switch tp {
		case mysql.TypeTimestamp:
			err = zeroDateErr(sc, TimestampStr, "0")
		case mysql.TypeDate:
			err = zeroDateErr(sc, DateStr, "0")
		case mysql.TypeDatetime:
			err = zeroDateErr(sc, DateTimeStr, "0")
		}
		return zt, errors.Trace(err)
	}
	fsp, err := CheckFsp(fsp)
	if err != nil {
		return NewTime(ZeroCoreTime, tp, DefaultFsp), errors.Trace(err)
...
func checkDateType(ctx Context, t CoreTime) error {
	...
	if month == 0 || day == 0 {
		return errors.Trace(zeroInDateErr(ctx, DateTimeStr, fmt.Sprintf("%04d-%02d-%02d", year, month, day)))
	}
	...
	if err := checkMonthDay(ctx, year, month, day); err != nil {
		return errors.Trace(err)
	}
...
func checkMonthDay(ctx Context, year, month, day int) error {
	...
	if day < 0 || day > maxDay {
		return errors.Trace(invalidDateErr(ctx, DateTimeStr, fmt.Sprintf("%d-%d-%d", year, month, day)))
	}
