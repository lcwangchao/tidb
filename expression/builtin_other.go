			...
			case columnType.GetType() == mysql.TypeBit && constant.Value.Kind() == types.KindInt64:
				if constant.Value.GetInt64() < 0 {
					if MaybeOverOptimized4PlanCache(ctx, args) {
						ctx.SetSkipPlanCache(errors.Errorf("Bit Column in (%v)", constant.Value.GetInt64()))
					}
					continue
				}
			}
...
func (b *builtinInIntSig) buildHashMapForConstArgs() error {
	...
	for i := 1; i < len(b.args); i++ {
		if b.args[i].ConstItem(b.ctx.ConstItemCtx()) {
			val, isNull, err := b.args[i].EvalInt(b.ctx.EvalCtx(), chunk.Row{})
			if err != nil {
				return err
			}
			if isNull {
...
func (b *builtinInStringSig) buildHashMapForConstArgs() error {
	...
	for i := 1; i < len(b.args); i++ {
		if b.args[i].ConstItem(b.ctx.ConstItemCtx()) {
			val, isNull, err := b.args[i].EvalString(b.ctx.EvalCtx(), chunk.Row{})
			if err != nil {
				return err
			}
			if isNull {
...
func (b *builtinInRealSig) buildHashMapForConstArgs() error {
	...
	for i := 1; i < len(b.args); i++ {
		if b.args[i].ConstItem(b.ctx.ConstItemCtx()) {
			val, isNull, err := b.args[i].EvalReal(b.ctx.EvalCtx(), chunk.Row{})
			if err != nil {
				return err
			}
			if isNull {
...
func (b *builtinInDecimalSig) buildHashMapForConstArgs() error {
	...
	for i := 1; i < len(b.args); i++ {
		if b.args[i].ConstItem(b.ctx.ConstItemCtx()) {
			val, isNull, err := b.args[i].EvalDecimal(b.ctx.EvalCtx(), chunk.Row{})
			if err != nil {
				return err
			}
			if isNull {
...
func (b *builtinInTimeSig) buildHashMapForConstArgs() error {
	...
	for i := 1; i < len(b.args); i++ {
		if b.args[i].ConstItem(b.ctx.ConstItemCtx()) {
			val, isNull, err := b.args[i].EvalTime(b.ctx.EvalCtx(), chunk.Row{})
			if err != nil {
				return err
			}
			if isNull {
...
func (b *builtinInDurationSig) buildHashMapForConstArgs() error {
	...
	for i := 1; i < len(b.args); i++ {
		if b.args[i].ConstItem(b.ctx.ConstItemCtx()) {
			val, isNull, err := b.args[i].EvalDuration(b.ctx.EvalCtx(), chunk.Row{})
			if err != nil {
				return err
			}
			if isNull {
...
func (b *builtinValuesIntSig) evalInt(ctx *EvalContext, _ chunk.Row) (int64, bool, error) {
	...
	if b.offset < row.Len() {
		...
		if len(val) < 8 {
			...
			v, err := binary.ToInt(ctx.StmtCtx)
			if err != nil {
