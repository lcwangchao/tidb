...
func (b *builtinInIntSig) buildHashMapForConstArgs(ctx sessionctx.Context) error {
	...
	for i := 1; i < len(b.args); i++ {
		if b.args[i].ConstItem(b.ctx.StmtCtx) {
			val, isNull, err := b.args[i].EvalInt(chunk.Row{})
			if err != nil {
				return err
			}
			if isNull {
...
func (b *builtinInIntSig) evalInt(row chunk.Row) (int64, bool, error) {
	...
	hasNull := b.hasNull
	for _, arg := range args {
		evaledArg, isNull, err := arg.EvalInt(row)
		if err != nil {
			return 0, true, err
		}
		if isNull {
...
func (b *builtinInStringSig) buildHashMapForConstArgs(ctx sessionctx.Context) error {
	...
	for i := 1; i < len(b.args); i++ {
		if b.args[i].ConstItem(b.ctx.StmtCtx) {
			val, isNull, err := b.args[i].EvalString(chunk.Row{})
			if err != nil {
				return err
			}
			if isNull {
...
func (b *builtinInStringSig) evalInt(row chunk.Row) (int64, bool, error) {
	...
	hasNull := b.hasNull
	for _, arg := range args {
		evaledArg, isNull, err := arg.EvalString(row)
		if err != nil {
			return 0, true, err
		}
		if isNull {
...
func (b *builtinInRealSig) buildHashMapForConstArgs(ctx sessionctx.Context) error {
	...
	for i := 1; i < len(b.args); i++ {
		if b.args[i].ConstItem(b.ctx.StmtCtx) {
			val, isNull, err := b.args[i].EvalReal(chunk.Row{})
			if err != nil {
				return err
			}
			if isNull {
...
func (b *builtinInRealSig) evalInt(row chunk.Row) (int64, bool, error) {
	...
	hasNull := b.hasNull
	for _, arg := range args {
		evaledArg, isNull, err := arg.EvalReal(row)
		if err != nil {
			return 0, true, err
		}
		if isNull {
...
func (b *builtinInDecimalSig) buildHashMapForConstArgs(ctx sessionctx.Context) error {
	...
	for i := 1; i < len(b.args); i++ {
		if b.args[i].ConstItem(b.ctx.StmtCtx) {
			val, isNull, err := b.args[i].EvalDecimal(chunk.Row{})
			if err != nil {
				return err
			}
			if isNull {
...
func (b *builtinInDecimalSig) evalInt(row chunk.Row) (int64, bool, error) {
	...
	hasNull := b.hasNull
	for _, arg := range args {
		evaledArg, isNull, err := arg.EvalDecimal(row)
		if err != nil {
			return 0, true, err
		}
		if isNull {
...
func (b *builtinInTimeSig) buildHashMapForConstArgs(ctx sessionctx.Context) error {
	...
	for i := 1; i < len(b.args); i++ {
		if b.args[i].ConstItem(b.ctx.StmtCtx) {
			val, isNull, err := b.args[i].EvalTime(chunk.Row{})
			if err != nil {
				return err
			}
			if isNull {
...
func (b *builtinInTimeSig) evalInt(row chunk.Row) (int64, bool, error) {
	...
	hasNull := b.hasNull
	for _, arg := range args {
		evaledArg, isNull, err := arg.EvalTime(row)
		if err != nil {
			return 0, true, err
		}
		if isNull {
...
func (b *builtinInDurationSig) buildHashMapForConstArgs(ctx sessionctx.Context) error {
	...
	for i := 1; i < len(b.args); i++ {
		if b.args[i].ConstItem(b.ctx.StmtCtx) {
			val, isNull, err := b.args[i].EvalDuration(chunk.Row{})
			if err != nil {
				return err
			}
			if isNull {
...
func (b *builtinInDurationSig) evalInt(row chunk.Row) (int64, bool, error) {
	...
	hasNull := b.hasNull
	for _, arg := range args {
		evaledArg, isNull, err := arg.EvalDuration(row)
		if err != nil {
			return 0, true, err
		}
		if isNull {
...
func (b *builtinInJSONSig) evalInt(row chunk.Row) (int64, bool, error) {
	...
	for _, arg := range b.args[1:] {
		evaledArg, isNull, err := arg.EvalJSON(row)
		if err != nil {
			return 0, true, err
		}
		if isNull {
...
func (b *builtinSetStringVarSig) evalString(row chunk.Row) (res string, isNull bool, err error) {
	var varName string
	varName, isNull, err = b.args[0].EvalString(row)
	if isNull || err != nil {
		return "", isNull, err
	}
	...
	b.ctx.SetStringUserVar(varName, stringutil.Copy(res), datum.Collation())
...
func (b *builtinSetRealVarSig) evalReal(row chunk.Row) (res float64, isNull bool, err error) {
	...
	b.ctx.SetUserVarVal(varName, datum)
	return res, false, nil
}
...
func (b *builtinGetStringVarSig) evalString(row chunk.Row) (string, bool, error) {
	...
	varName = strings.ToLower(varName)
	if v, ok := b.ctx.GetUserVarVal(varName); ok {
...
func (b *builtinValuesIntSig) evalInt(_ chunk.Row) (int64, bool, error) {
	...
	if b.offset < row.Len() {
		...
		if len(val) < 8 {
			...
			v, err := binary.ToInt(b.ctx.StmtCtx)
			if err != nil {
...
func (b *builtinGetParamStringSig) evalString(row chunk.Row) (string, bool, error) {
	...
	v := b.ctx.PlanCacheParams.GetParamValue(int(idx))
