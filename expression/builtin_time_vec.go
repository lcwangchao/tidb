...
func (b *builtinUTCTimeWithArgSig) vecEvalDuration(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		res, _, err := types.ParseDuration(stmtCtx.ValCtx, utc, int(fsp))
		if err != nil {
			return err
		}
		d64s[i] = res.Duration
...
func (b *builtinStrToDateDateSig) vecEvalTime(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		var t types.Time
		succ := t.StrToDate(sc.ValCtx, bufStrings.GetString(i), bufFormats.GetString(i))
		if !succ {
...
func (b *builtinStrToDateDatetimeSig) vecEvalTime(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		var t types.Time
		succ := t.StrToDate(sc.ValCtx, dateBuf.GetString(i), formatBuf.GetString(i))
		if !succ {
			if err = handleInvalidTimeError(b.ctx, types.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, t.String())); err != nil {
				return err
			}
...
func (b *builtinSecToTimeSig) vecEvalDuration(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		duration, _, err := types.ParseDuration(b.ctx.GetSessionVars().StmtCtx.ValCtx, fmt.Sprintf("%s%02d:%02d:%s", negative, hour, minute, strconv.FormatFloat(secondDemical, 'f', -1, 64)), b.tp.GetDecimal())
		if err != nil {
			return err
		}
...
func (b *builtinUTCTimeWithoutArgSig) vecEvalDuration(input *chunk.Chunk, result *chunk.Column) error {
	...
	res, _, err := types.ParseDuration(b.ctx.GetSessionVars().StmtCtx.ValCtx, nowTs.UTC().Format(types.TimeFormat), types.DefaultFsp)
	if err != nil {
		return err
	}
	result.ResizeGoDuration(n, false)
...
func (b *builtinCurrentTime1ArgSig) vecEvalDuration(input *chunk.Chunk, result *chunk.Column) error {
	...
	durations := result.GoDurations()
	for i := 0; i < n; i++ {
		res, _, err := types.ParseDuration(stmtCtx.ValCtx, dur, int(i64s[i]))
		if err != nil {
			return err
		}
		durations[i] = res.Duration
...
func (b *builtinAddSubDateDurationAnySig) vecEvalTime(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		t, err := iterDuration.ConvertToTime(sc.ValCtx, mysql.TypeDatetime)
		if err != nil {
			result.SetNull(i, true)
		}
