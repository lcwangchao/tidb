...
func getFlen4LpadAndRpad(ctx *ExprContext, arg Expression) int {
	if constant, ok := arg.(*Constant); ok {
		length, isNull, err := constant.EvalInt(ctx.EvalCtx(), chunk.Row{})
		if err != nil {
...
func (c *charFunctionClass) getFunction(ctx *ExprContext, args []Expression) (builtinFunc, error) {
	...
	charsetName, isNull, err := args[len(args)-1].EvalString(ctx.EvalCtx(), chunk.Row{})
	if err != nil {
		return nil, err
	}
	if isNull {
...
func (c *makeSetFunctionClass) getFlen(ctx *ExprContext, args []Expression) int {
	flen, count := 0, 0
	if constant, ok := args[0].(*Constant); ok {
		bits, isNull, err := constant.EvalInt(ctx.EvalCtx(), chunk.Row{})
		if err == nil && !isNull {
...
func (b *builtinFormatWithLocaleSig) evalString(ctx *EvalContext, row chunk.Row) (string, bool, error) {
	...
	if isNull {
		ctx.StmtCtx.AppendWarning(errUnknownLocale.GenWithStackByArgs("NULL"))
	} else if !strings.EqualFold(locale, "en_US") { // TODO: support other locales.
		ctx.StmtCtx.AppendWarning(errUnknownLocale.GenWithStackByArgs(locale))
	}
	locale = "en_US"
...
func (b *builtinWeightStringSig) evalString(ctx *EvalContext, row chunk.Row) (string, bool, error) {
	...
	case weightStringPaddingAsBinary:
		...
		if b.length < lenStr {
			...
			ctx.StmtCtx.AppendWarning(errTruncatedWrongValue.GenWithStackByArgs(tpInfo, str))
			str = str[:b.length]
