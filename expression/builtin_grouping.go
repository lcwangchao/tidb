...
func (c *groupingImplFunctionClass) getFunction(ctx *ExprContext, args []Expression) (builtinFunc, error) {
...
func (b *BuiltinGroupingImplSig) evalInt(row chunk.Row) (int64, bool, error) {
	...
	groupingID, isNull, err := b.args[0].EvalInt(row)
	if isNull || err != nil {
		return 0, isNull, err
	}
...
func (b *BuiltinGroupingImplSig) vecEvalInt(input *chunk.Chunk, result *chunk.Column) error {
	...
	if err = b.args[0].VecEvalInt(input, bufVal); err != nil {
		return err
	}
