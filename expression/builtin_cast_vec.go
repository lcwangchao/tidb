...
func (b *builtinCastRealAsStringSig) vecEvalString(ctx *EvalContext, input *chunk.Chunk, result *chunk.Column) error {
	...
	for i, v := range f64s {
		...
		res, isNull, err = padZeroForBinaryType(ctx, res, b.tp, b.ctx)
		if err != nil {
			return err
		}
		if isNull {
...
func (b *builtinCastDecimalAsStringSig) vecEvalString(ctx *EvalContext, input *chunk.Chunk, result *chunk.Column) error {
	...
	for i, v := range vas {
		...
		str, b, e1 := padZeroForBinaryType(ctx, res, b.tp, b.ctx)
		if e1 != nil {
			return e1
		}
		if b {
...
func (b *builtinCastDurationAsIntSig) vecEvalInt(ctx *EvalContext, input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		duration.Fsp = fsp
		dur, err := duration.RoundFrac(types.DefaultFsp, ctx.Location)
		if err != nil {
			return err
		}
...
func (b *builtinCastIntAsStringSig) vecEvalString(ctx *EvalContext, input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		str, err = types.ProduceStrWithSpecifiedTp(str, b.tp, ctx.StmtCtx, false)
		if err != nil {
			return err
		}
		var d bool
		str, d, err = padZeroForBinaryType(ctx, str, b.tp, b.ctx)
		if err != nil {
			return err
		}
		if d {
...
func (b *builtinCastRealAsDecimalSig) vecEvalDecimal(ctx *EvalContext, input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		dec, err := types.ProduceDecWithSpecifiedTp(&resdecimal[i], b.tp, ctx.StmtCtx)
		if err != nil {
			return err
		}
		resdecimal[i] = *dec
...
func (b *builtinCastStringAsDurationSig) vecEvalDuration(ctx *EvalContext, input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		dur, isNull, err := types.ParseDuration(ctx.StmtCtx, buf.GetString(i), b.tp.GetDecimal())
		if err != nil {
			if types.ErrTruncatedWrongVal.Equal(err) {
				err = ctx.StmtCtx.HandleTruncate(err)
			}
...
func (b *builtinCastJSONAsStringSig) vecEvalString(ctx *EvalContext, input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		s, err := types.ProduceStrWithSpecifiedTp(buf.GetJSON(i).String(), b.tp, ctx.StmtCtx, false)
		if err != nil {
			return err
		}
...
func (b *builtinCastRealAsDurationSig) vecEvalDuration(ctx *EvalContext, input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		dur, _, err := types.ParseDuration(ctx.StmtCtx, strconv.FormatFloat(f64s[i], 'f', -1, 64), b.tp.GetDecimal())
		if err != nil {
			if types.ErrTruncatedWrongVal.Equal(err) {
				err = ctx.StmtCtx.HandleTruncate(err)
				if err != nil {
...
func (b *builtinCastTimeAsDurationSig) vecEvalDuration(ctx *EvalContext, input *chunk.Chunk, result *chunk.Column) error {
	...
	for i, t := range arg0s {
		...
		d, err = d.RoundFrac(b.tp.GetDecimal(), ctx.Location)
		if err != nil {
			return err
		}
		ds[i] = d.Duration
...
func (b *builtinCastDurationAsDurationSig) vecEvalDuration(ctx *EvalContext, input *chunk.Chunk, result *chunk.Column) error {
	...
	for i, v := range res {
		...
		dur.Duration = v
		rd, err = dur.RoundFrac(b.tp.GetDecimal(), ctx.Location)
		if err != nil {
			return err
		}
		res[i] = rd.Duration
...
func (b *builtinCastStringAsTimeSig) vecEvalTime(ctx *EvalContext, input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		if tm.IsZero() && ctx.SQLMode.HasNoZeroDateMode() {
...
func (b *builtinCastDecimalAsDurationSig) vecEvalDuration(ctx *EvalContext, input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		dur, _, err := types.ParseDuration(ctx.StmtCtx, string(args[i].ToString()), b.tp.GetDecimal())
		if err != nil {
			if types.ErrTruncatedWrongVal.Equal(err) {
				err = ctx.StmtCtx.HandleTruncate(err)
				if err != nil {
