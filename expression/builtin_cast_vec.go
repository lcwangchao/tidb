...
func (b *builtinCastIntAsDurationSig) vecEvalDuration(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		dur, err := types.NumberToDuration(b.ctx.GetSessionVars().StmtCtx.ValCtx, i64s[i], b.tp.GetDecimal())
		if err != nil {
...
func (b *builtinCastRealAsStringSig) vecEvalString(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i, v := range f64s {
		...
		res, err = types.ProduceStrWithSpecifiedTp(strconv.FormatFloat(v, 'f', -1, bits), b.tp, sc.ValCtx, false)
		if err != nil {
			return err
		}
...
func (b *builtinCastDecimalAsStringSig) vecEvalString(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i, v := range vas {
		...
		res, e := types.ProduceStrWithSpecifiedTp(string(v.ToString()), b.tp, sc.ValCtx, false)
		if e != nil {
			return e
		}
...
func (b *builtinCastTimeAsDecimalSig) vecEvalDecimal(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		*dec = types.MyDecimal{}
		...
		dec, err = types.ProduceDecWithSpecifiedTp(dec, b.tp, sc.ValCtx)
		if err != nil {
			return err
		}
		decs[i] = *dec
...
func (b *builtinCastIntAsTimeSig) vecEvalTime(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		if b.args[0].GetType().GetType() == mysql.TypeYear {
			tm, err = types.ParseTimeFromYear(stmt.ValCtx, i64s[i])
		} else {
			tm, err = types.ParseTimeFromNum(stmt.ValCtx, i64s[i], b.tp.GetType(), fsp)
		}
...
func (b *builtinCastJSONAsRealSig) vecEvalReal(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		f64s[i], err = types.ConvertJSONToFloat(sc.ValCtx, buf.GetJSON(i))
		if err != nil {
			return err
		}
	}
...
func (b *builtinCastJSONAsTimeSig) vecEvalTime(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		case types.JSONTypeCodeDuration:
			...
			sc := b.ctx.GetSessionVars().StmtCtx
			tm, err := duration.ConvertToTimeWithTimestamp(sc.ValCtx, b.tp.GetType(), ts)
			if err != nil {
				if err = handleInvalidTimeError(b.ctx, err); err != nil {
					return err
				}
			...
			tm, err = tm.RoundFrac(stmtCtx.ValCtx, fsp)
			if err != nil {
				return err
			}
			times[i] = tm
		case types.JSONTypeCodeString:
			...
			tm, err := types.ParseTime(stmtCtx.ValCtx, s, b.tp.GetType(), fsp, nil)
			if err != nil {
				if err = handleInvalidTimeError(b.ctx, err); err != nil {
					return err
				}
...
func (b *builtinCastRealAsTimeSig) vecEvalTime(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		tm, err := types.ParseTimeFromFloatString(stmt.ValCtx, fv, b.tp.GetType(), fsp)
		if err != nil {
			if err = handleInvalidTimeError(b.ctx, err); err != nil {
				return err
			}
...
func (b *builtinCastDurationAsTimeSig) vecEvalTime(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		duration.Duration = ds[i]
		duration.Fsp = fsp
		tm, err := duration.ConvertToTimeWithTimestamp(stmtCtx.ValCtx, b.tp.GetType(), ts)
		if err != nil {
			if err = handleInvalidTimeError(b.ctx, err); err != nil {
				return err
			}
...
func (b *builtinCastIntAsStringSig) vecEvalString(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		str, err = types.ProduceStrWithSpecifiedTp(str, b.tp, b.ctx.GetSessionVars().StmtCtx.ValCtx, false)
		if err != nil {
			return err
		}
		var d bool
...
func (b *builtinCastRealAsIntSig) vecEvalInt(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		if !unsigned {
			i64s[i], err = types.ConvertFloatToInt(b.ctx.GetSessionVars().StmtCtx.ValCtx, f64s[i], types.IntergerSignedLowerBound(mysql.TypeLonglong), types.IntergerSignedUpperBound(mysql.TypeLonglong), mysql.TypeLonglong)
		} else if b.inUnion && f64s[i] < 0 {
			i64s[i] = 0
		} else {
			var uintVal uint64
			sc := b.ctx.GetSessionVars().StmtCtx
			uintVal, err = types.ConvertFloatToUint(sc.ValCtx, f64s[i], types.IntergerUnsignedUpperBound(mysql.TypeLonglong), mysql.TypeLonglong)
...
func (b *builtinCastRealAsDecimalSig) vecEvalDecimal(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		dec, err := types.ProduceDecWithSpecifiedTp(&resdecimal[i], b.tp, b.ctx.GetSessionVars().StmtCtx.ValCtx)
		if err != nil {
			return err
		}
		resdecimal[i] = *dec
...
func (b *builtinCastStringAsIntSig) vecEvalInt(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		if !isNegative {
			ures, err = types.StrToUint(sc.ValCtx, val, true)
			...
			res, err = types.StrToInt(sc.ValCtx, val, true)
...
func (b *builtinCastStringAsDurationSig) vecEvalDuration(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		dur, isNull, err := types.ParseDuration(b.ctx.GetSessionVars().StmtCtx.ValCtx, buf.GetString(i), b.tp.GetDecimal())
		if err != nil {
			if types.ErrTruncatedWrongVal.Equal(err) {
				err = b.ctx.GetSessionVars().StmtCtx.HandleTruncate(err)
			}
...
func (b *builtinCastDurationAsDecimalSig) vecEvalDecimal(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		duration.Duration = ds[i]
		duration.Fsp = fsp
		res, err := types.ProduceDecWithSpecifiedTp(duration.ToNumber(), b.tp, sc.ValCtx)
		if err != nil {
			return err
		}
		d64s[i] = *res
...
func (b *builtinCastJSONAsStringSig) vecEvalString(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		s, err := types.ProduceStrWithSpecifiedTp(buf.GetJSON(i).String(), b.tp, b.ctx.GetSessionVars().StmtCtx.ValCtx, false)
		if err != nil {
			return err
		}
...
func (b *builtinCastJSONAsIntSig) vecEvalInt(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		i64s[i], err = types.ConvertJSONToInt64(sc.ValCtx, buf.GetJSON(i), mysql.HasUnsignedFlag(b.tp.GetFlag()))
		if err != nil {
			return err
		}
	}
...
func (b *builtinCastRealAsDurationSig) vecEvalDuration(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		dur, _, err := types.ParseDuration(b.ctx.GetSessionVars().StmtCtx.ValCtx, strconv.FormatFloat(f64s[i], 'f', -1, 64), b.tp.GetDecimal())
		if err != nil {
			if types.ErrTruncatedWrongVal.Equal(err) {
				err = b.ctx.GetSessionVars().StmtCtx.HandleTruncate(err)
				if err != nil {
...
func (b *builtinCastDurationAsStringSig) vecEvalString(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		res, err = types.ProduceStrWithSpecifiedTp(buf.GetDuration(i, fsp).String(), b.tp, sc.ValCtx, false)
		if err != nil {
			return err
		}
		res, isNull, err = padZeroForBinaryType(res, b.tp, b.ctx)
...
func (b *builtinCastDecimalAsTimeSig) vecEvalTime(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		tm, err := types.ParseTimeFromFloatString(stmt.ValCtx, string(decimals[i].ToString()), b.tp.GetType(), fsp)
		if err != nil {
			if err = handleInvalidTimeError(b.ctx, err); err != nil {
				return err
			}
...
func (b *builtinCastTimeAsIntSig) vecEvalInt(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		t, err := times[i].RoundFrac(sc.ValCtx, types.DefaultFsp)
		if err != nil {
			return err
		}
...
func (b *builtinCastTimeAsTimeSig) vecEvalTime(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		res, err := times[i].Convert(stmt.ValCtx, b.tp.GetType())
		if err != nil {
			if err = handleInvalidTimeError(b.ctx, err); err != nil {
				return err
			}
		...
		tm, err := res.RoundFrac(stmt.ValCtx, fsp)
		if err != nil {
			return err
		}
		times[i] = tm
...
func (b *builtinCastTimeAsStringSig) vecEvalString(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i, v := range vas {
		...
		res, err = types.ProduceStrWithSpecifiedTp(v.String(), b.tp, sc.ValCtx, false)
		if err != nil {
			return err
		}
		res, isNull, err = padZeroForBinaryType(res, b.tp, b.ctx)
...
func (b *builtinCastJSONAsDecimalSig) vecEvalDecimal(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		tempres, err := types.ConvertJSONToDecimal(sc.ValCtx, buf.GetJSON(i))
		if err != nil {
			return err
		}
		tempres, err = types.ProduceDecWithSpecifiedTp(tempres, b.tp, sc.ValCtx)
		if err != nil {
			return err
		}
		res[i] = *tempres
...
func (b *builtinCastStringAsRealSig) vecEvalReal(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		res, err := types.StrToFloat(sc.ValCtx, buf.GetString(i), true)
		if err != nil {
			return err
		}
		...
		res, err = types.ProduceFloatWithSpecifiedTp(res, b.tp, sc.ValCtx)
		if err != nil {
			return err
		}
		ret[i] = res
...
func (b *builtinCastStringAsDecimalSig) vecEvalDecimal(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		if !(b.inUnion && mysql.HasUnsignedFlag(b.tp.GetFlag()) && isNegative) {
			...
			dec, err := types.ProduceDecWithSpecifiedTp(dec, b.tp, stmtCtx.ValCtx)
			if err != nil {
				return err
			}
			res[i] = *dec
...
func (b *builtinCastStringAsTimeSig) vecEvalTime(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		tm, err := types.ParseTime(stmtCtx.ValCtx, buf.GetString(i), b.tp.GetType(), fsp, nil)
		if err != nil {
...
func (b *builtinCastDecimalAsDurationSig) vecEvalDuration(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		dur, _, err := types.ParseDuration(b.ctx.GetSessionVars().StmtCtx.ValCtx, string(args[i].ToString()), b.tp.GetDecimal())
		if err != nil {
			if types.ErrTruncatedWrongVal.Equal(err) {
				err = b.ctx.GetSessionVars().StmtCtx.HandleTruncate(err)
				if err != nil {
...
func (b *builtinCastStringAsStringSig) vecEvalString(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		res, err = types.ProduceStrWithSpecifiedTp(buf.GetString(i), b.tp, sc.ValCtx, false)
		if err != nil {
			return err
		}
		res, isNull, err = padZeroForBinaryType(res, b.tp, b.ctx)
...
func (b *builtinCastJSONAsDurationSig) vecEvalDuration(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		case types.JSONTypeCodeString:
			...
			dur, _, err = types.ParseDuration(stmtCtx.ValCtx, s, b.tp.GetDecimal())
			if types.ErrTruncatedWrongVal.Equal(err) {
