...
func (cf *concatFunction) initSeparator(evalCtx *AggEvaluateContext, row chunk.Row) error {
	sepArg := cf.Args[len(cf.Args)-1]
	sepDatum, err := sepArg.Eval(evalCtx.EvalCtx, row)
	if err != nil {
		return err
	}
	if sepDatum.IsNull() {
...
func (cf *concatFunction) Update(evalCtx *AggEvaluateContext, sc *stmtctx.StatementContext, row chunk.Row) error {
	...
	if !cf.sepInited {
		err := cf.initSeparator(evalCtx, row)
		if err != nil {
			return err
		}
		cf.sepInited = true
	...
	for i, length := 0, len(cf.Args)-1; i < length; i++ {
		value, err := cf.Args[i].Eval(evalCtx.EvalCtx, row)
		if err != nil {
			return err
		}
		if value.IsNull() {
