...
func (b *executorBuilder) buildImportInto(v *plannercore.ImportInto) exec.Executor {
	...
	latestIS := b.sctx.GetDomainInfoSchema().(infoschema.InfoSchema)
...
func (b *executorBuilder) buildHashJoin(v *plannercore.PhysicalHashJoin) exec.Executor {
	if b.sctx.GetSessionVars().UseHashJoinV2 && join.IsHashJoinV2Supported() && v.CanUseHashJoinV2() {
...
func (b *executorBuilder) buildAnalyzeColumnsPushdown(
	...
	b.err = tables.SetPBColumnsDefaultValue(b.sctx.GetExprCtx(), e.analyzePB.ColReq.ColumnsInfo, cols)
	return &analyzeTask{taskType: colTask, colExec: e, job: job}
}
...
func (b *executorBuilder) buildIndexUsageReporter(plan tableStatsPreloader) (indexUsageReporter *exec.IndexUsageReporter) {
	return buildIndexUsageReporter(b.sctx, plan)
}
...
func (b *executorBuilder) buildIndexMergeReader(v *plannercore.PhysicalIndexMergeReader) exec.Executor {
	...
	for i := 0; i < len(v.PartialPlans); i++ {
		if is, ok := v.PartialPlans[i][0].(*plannercore.PhysicalIndexScan); ok {
			...
			partialTS := v.PartialPlans[i][0].(*plannercore.PhysicalTableScan)
...
func (builder *dataReaderBuilder) buildProjectionForIndexJoin(
	...
	e := &ProjectionExec{
		projectionExecutorContext: newProjectionExecutorContext(builder.sctx),
		BaseExecutorV2:            exec.NewBaseExecutorV2(builder.sctx.GetSessionVars(), v.Schema(), v.ID(), childExec),
		numWorkers:                int64(builder.sctx.GetSessionVars().ProjectionConcurrency()),
		evaluatorSuit:             expression.NewEvaluatorSuite(v.Exprs, v.AvoidColumnEvaluator),
		calculateNoDelay:          v.CalculateNoDelay,
	}
	...
	// than a Chunk size, we turn back to the un-parallel Projection
	// implementation to reduce the goroutine overhead.
	if int64(v.StatsCount()) < int64(builder.sctx.GetSessionVars().MaxChunkSize) {
		e.numWorkers = 0
	}
...
func (b *executorBuilder) buildWindow(v *plannercore.PhysicalWindow) exec.Executor {
	...
	var err error
	if b.sctx.GetSessionVars().EnablePipelinedWindowExec {
		exec := &PipelinedWindowExec{
			BaseExecutor:   base,
			groupChecker:   vecgroupchecker.NewVecGroupChecker(b.sctx.GetExprCtx().GetEvalCtx(), b.sctx.GetSessionVars().EnableVectorizedExpression, groupByItems),
		...
		if v.Frame == nil {
			...
			if v.Frame.Type == ast.Ranges {
				...
				err = exec.start.UpdateCompareCols(b.sctx, exec.orderByCols)
				if err != nil {
					return nil
				}
				err = exec.end.UpdateCompareCols(b.sctx, exec.orderByCols)
				if err != nil {
					return nil
				}
			}
	...
	if v.Frame == nil {
		...
		err = tmpProcessor.start.UpdateCompareCols(b.sctx, orderByCols)
		if err != nil {
			return nil
		}
		err = tmpProcessor.end.UpdateCompareCols(b.sctx, orderByCols)
		if err != nil {
			return nil
		}
...
func (b *executorBuilder) buildCTE(v *plannercore.PhysicalCTE) exec.Executor {
	...
	storageMap, ok := b.sctx.GetSessionVars().StmtCtx.CTEStorageMap.(map[int]*CTEStorages)
	if !ok {
