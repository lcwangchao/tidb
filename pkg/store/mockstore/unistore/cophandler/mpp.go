...
func (b *mppExecBuilder) buildMPPTableScan(pb *tipb.TableScan) (*tableScanExec, error) {
	...
	ts := &tableScanExec{
		baseMPPExec: baseMPPExec{sc: b.sc, sctx: b.dagCtx.sctx, mppCtx: b.mppCtx},
...
func (b *mppExecBuilder) buildIdxScan(pb *tipb.IndexScan) (*indexScanExec, error) {
	...
	idxScan := &indexScanExec{
		baseMPPExec:     baseMPPExec{sc: b.sc, sctx: b.dagCtx.sctx, fieldTypes: fieldTypes},
		startTS:         b.dagCtx.startTS,
		kvRanges:        ranges,
		dbReader:        b.dbReader,
...
func (b *mppExecBuilder) buildLimit(pb *tipb.Limit) (*limitExec, error) {
	...
	exec := &limitExec{
		baseMPPExec: baseMPPExec{sc: b.sc, sctx: b.dagCtx.sctx, mppCtx: b.mppCtx, fieldTypes: child.getFieldTypes(), children: []mppExec{child}},
		limit:       pb.GetLimit(),
	}
	return exec, nil
}
...
func (b *mppExecBuilder) buildExpand(pb *tipb.Expand) (mppExec, error) {
	...
	exec := &expandExec{
		baseMPPExec: baseMPPExec{sc: b.sc, sctx: b.dagCtx.sctx, mppCtx: b.mppCtx, children: []mppExec{child}},
	}
	...
	for _, gs := range pb.GroupingSets {
		...
		for _, groupingExprs := range gs.GroupingExprs {
			tidbGroupingExprs, err := convertToExprs(b.dagCtx.sctx, childFieldTypes, groupingExprs.GroupingExpr)
			if err != nil {
				return nil, err
			}
...
func (b *mppExecBuilder) buildTopN(pb *tipb.TopN) (mppExec, error) {
	...
	if conds, err = convertToExprs(b.dagCtx.sctx, fieldTps, pbConds); err != nil {
		return nil, errors.Trace(err)
	}
	exec := &topNExec{
		baseMPPExec: baseMPPExec{sc: b.sc, sctx: b.dagCtx.sctx, mppCtx: b.mppCtx, fieldTypes: fieldTps, children: []mppExec{child}},
		heap:        heap,
		conds:       conds,
...
func (b *mppExecBuilder) buildMPPExchangeSender(pb *tipb.ExchangeSender) (*exchSenderExec, error) {
	...
	if pb.Tp == tipb.ExchangeType_Hash {
		...
		for _, partitionKey := range pb.PartitionKeys {
			expr, err := expression.PBToExpr(partitionKey, child.getFieldTypes(), e.sctx)
			if err != nil {
				return nil, errors.Trace(err)
			}
...
func (b *mppExecBuilder) buildMPPJoin(pb *tipb.Join, children []*tipb.Executor) (*joinExec, error) {
	...
	if pb.InnerIdx == 1 {
		...
		probeExpr, err := expression.PBToExpr(pb.LeftJoinKeys[0], leftCh.getFieldTypes(), e.sctx)
		if err != nil {
			return nil, errors.Trace(err)
		}
		e.probeKey = probeExpr.(*expression.Column)
		buildExpr, err := expression.PBToExpr(pb.RightJoinKeys[0], rightCh.getFieldTypes(), e.sctx)
		if err != nil {
			return nil, errors.Trace(err)
		}
		...
		e.buildChild = leftCh
		buildExpr, err := expression.PBToExpr(pb.LeftJoinKeys[0], leftCh.getFieldTypes(), e.sctx)
		if err != nil {
			return nil, errors.Trace(err)
		}
		e.buildKey = buildExpr.(*expression.Column)
		probeExpr, err := expression.PBToExpr(pb.RightJoinKeys[0], rightCh.getFieldTypes(), e.sctx)
		if err != nil {
			return nil, errors.Trace(err)
		}
		e.probeKey = probeExpr.(*expression.Column)
...
func (b *mppExecBuilder) buildMPPProj(proj *tipb.Projection) (*projExec, error) {
	...
	for _, pbExpr := range proj.Exprs {
		expr, err := expression.PBToExpr(pbExpr, chExec.getFieldTypes(), e.sctx)
		if err != nil {
			return nil, errors.Trace(err)
		}
...
func (b *mppExecBuilder) buildMPPAgg(agg *tipb.Aggregation) (*aggExec, error) {
	...
	for _, aggFunc := range agg.AggFunc {
		...
		aggExpr, err := aggregation.NewDistAggFunc(aggFunc, chExec.getFieldTypes(), b.dagCtx.sctx)
		if err != nil {
			return nil, errors.Trace(err)
		}
