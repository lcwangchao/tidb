...
func (e *hashAggExec) Next(ctx context.Context) (value [][]byte, err error) {
	...
	for i, agg := range e.aggExprs {
		...
		for _, result := range partialResults {
			data, err := codec.EncodeValue(e.evalCtx.sctx.GetSessionVars().StmtCtx, nil, result)
			if err != nil {
				return nil, errors.Trace(err)
			}
...
func (e *hashAggExec) getGroupKey() ([]byte, [][]byte, error) {
	...
	for _, item := range e.groupByExprs {
		...
		b, err := codec.EncodeValue(e.evalCtx.sctx.GetSessionVars().StmtCtx, nil, v)
		if err != nil {
			return nil, nil, errors.Trace(err)
		}
		bufLen += len(b)
...
func (e *streamAggExec) getPartialResult() ([][]byte, error) {
	...
	for _, d := range e.currGroupByRow {
		buf, err := codec.EncodeValue(e.evalCtx.sctx.GetSessionVars().StmtCtx, nil, d)
		if err != nil {
			return nil, errors.Trace(err)
		}
...
func (e *streamAggExec) meetNewGroup(row [][]byte) (bool, error) {
	...
	for i, item := range e.groupByExprs {
		...
		if matched {
			c, err := d.Compare(e.evalCtx.sctx.GetSessionVars().StmtCtx, &e.nextGroupByRow[i], e.groupByCollators[i])
			if err != nil {
				return false, errors.Trace(err)
			}
			matched = c == 0
