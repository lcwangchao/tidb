...
func (b *builtinMonthSig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	...
	if err := b.args[0].VecEvalTime(sctx, input, buf); err != nil {
		return err
	}
...
func (b *builtinYearSig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinDateSig) vecEvalTime(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < len(times); i++ {
		...
		if times[i].IsZero() && sctx.GetSessionVars().SQLMode.HasNoZeroDateMode() {
			if err := handleInvalidTimeError(sctx, types.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, times[i].String())); err != nil {
				return err
			}
...
func (b *builtinFromUnixTime2ArgSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	...
	if err = b.args[0].VecEvalDecimal(sctx, input, buf1); err != nil {
		return err
	}
	...
	if err = b.args[1].VecEvalString(sctx, input, buf2); err != nil {
		return err
	}
	...
	for i := 0; i < n; i++ {
		...
		t, isNull, err := evalFromUnixTime(sctx, fsp, &ds[i])
		if err != nil {
			return err
		}
		if isNull {
...
func (b *builtinSysDateWithoutFspSig) vecEvalTime(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
...
func (b *builtinExtractDatetimeFromStringSig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinDayNameSig) vecEvalIndex(sctx sessionctx.Context, input *chunk.Chunk, apply func(i int, res int), applyNull func(i int)) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
	...
	for i := 0; i < n; i++ {
		...
		if ds[i].InvalidZero() {
			if err := handleInvalidTimeError(sctx, types.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, ds[i].String())); err != nil {
				return err
			}
			applyNull(i)
			continue
...
func (b *builtinDayNameSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	result.ReserveString(n)

	return b.vecEvalIndex(sctx, input, func(i, res int) {
...
func (b *builtinDayNameSig) vecEvalReal(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
...
func (b *builtinDayNameSig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	result.ResizeInt64(n, false)
	i64s := result.Int64s()
...
func (b *builtinWeekDaySig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinTimeFormatSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
	...
	defer b.bufAllocator.put(buf)
	if err := b.args[0].VecEvalDuration(sctx, input, buf); err != nil {
		...
		return vecEvalStringByRows(sctx, b, input, result)
	}
	...
	for i := 0; i < n; i++ {
		...
		res, err := b.formatTime(sctx, buf.GetDuration(i, 0), buf1.GetString(i))
		if err != nil {
			return err
		}
		result.AppendString(res)
...
func (b *builtinUTCTimeWithArgSig) vecEvalDuration(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinUnixTimestampCurrentSig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	nowTs, err := getStmtTimestamp(sctx)
	if err != nil {
		return err
	}
...
func (b *builtinYearWeekWithoutModeSig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
	...
	for i := 0; i < n; i++ {
		...
		date := ds[i]
		if date.InvalidZero() {
			if err := handleInvalidTimeError(sctx, types.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, date.String())); err != nil {
				return err
			}
			result.SetNull(i, true)
			continue
...
func (b *builtinPeriodDiffSig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	if err := b.args[0].VecEvalInt(sctx, input, result); err != nil {
		return err
	}
...
func (b *builtinNowWithArgSig) vecEvalTime(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	...
	if err = b.args[0].VecEvalInt(sctx, input, bufFsp); err != nil {
		return err
	}
	...
	for i := 0; i < n; i++ {
		...
		t, isNull, err := evalNowWithFsp(sctx, fsp)
		if err != nil {
			return err
		}
		if isNull {
...
func (b *builtinGetFormatSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
...
func (b *builtinLastDaySig) vecEvalTime(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	if err := b.args[0].VecEvalTime(sctx, input, result); err != nil {
		return err
	}
...
func (b *builtinStrToDateDateSig) vecEvalTime(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	...
	if err := b.args[0].VecEvalString(sctx, input, bufStrings); err != nil {
		return err
	}
	...
	if err := b.args[1].VecEvalString(sctx, input, bufFormats); err != nil {
		return err
	}
	...
	for i := 0; i < n; i++ {
		...
		if !succ {
			if err := handleInvalidTimeError(sctx, types.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, t.String())); err != nil {
				return err
			}
			result.SetNull(i, true)
			continue
...
func (b *builtinSysDateWithFspSig) vecEvalTime(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinTidbParseTsoSig) vecEvalTime(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
	...
	for i := 0; i < n; i++ {
		...
		if err := r.ConvertTimeZone(time.Local, sctx.GetSessionVars().Location()); err != nil {
			return err
		}
		times[i] = r
	}
...
func (b *builtinTiDBBoundedStalenessSig) vecEvalTime(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf0, err := b.bufAllocator.get()
	if err != nil {
		return err
	...
	minSafeTime := getMinSafeTime(sctx, timeZone)
	result.ResizeTime(n, false)
	...
	for i := 0; i < n; i++ {
		...
		if invalidArg0, invalidArg1 := args0[i].InvalidZero(), args1[i].InvalidZero(); invalidArg0 || invalidArg1 {
			if invalidArg0 {
				err = handleInvalidTimeError(sctx, types.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, args0[i].String()))
			}
			if invalidArg1 {
				err = handleInvalidTimeError(sctx, types.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, args1[i].String()))
			}
			if err != nil {
				return err
			}
...
func (b *builtinFromDaysSig) vecEvalTime(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinMicroSecondSig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
	...
	defer b.bufAllocator.put(buf)
	if err = b.args[0].VecEvalDuration(sctx, input, buf); err != nil {
		return vecEvalIntByRows(sctx, b, input, result)
	}
...
func (b *builtinQuarterSig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinWeekWithModeSig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf1, err := b.bufAllocator.get()
	if err != nil {
		return err
	}
	if err := b.args[0].VecEvalTime(sctx, input, buf1); err != nil {
		return err
	}
	defer b.bufAllocator.put(buf1)
	...
	if err := b.args[1].VecEvalInt(sctx, input, buf2); err != nil {
		return err
	}
	defer b.bufAllocator.put(buf2)
...
func (b *builtinExtractDatetimeSig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	...
	if err := b.args[0].VecEvalString(sctx, input, unit); err != nil {
		return err
	}
	...
	if err = b.args[1].VecEvalTime(sctx, input, dt); err != nil {
		return err
	}
	result.ResizeInt64(n, false)
...
func (b *builtinExtractDurationSig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	unit, err := b.bufAllocator.get()
	if err != nil {
		return err
	...
	if err = b.args[1].VecEvalDuration(sctx, input, dur); err != nil {
		return err
	}
	result.ResizeInt64(n, false)
...
func (b *builtinStrToDateDurationSig) vecEvalDuration(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	bufStrings, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinToSecondsSig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
	...
	for i := 0; i < n; i++ {
		...
		if ret == 0 {
			if err := handleInvalidTimeError(sctx, types.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, arg.String())); err != nil {
				return err
			}
			result.SetNull(i, true)
			continue
...
func (b *builtinMinuteSig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinSecondSig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinNowWithoutArgSig) vecEvalTime(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	nowTs, isNull, err := evalNowWithFsp(sctx, 0)
	if err != nil {
		return err
	}
	if isNull {
...
func (b *builtinTimestampLiteralSig) vecEvalTime(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	result.ResizeTime(n, false)
	times := result.Times()
	for i := range times {
...
func (b *builtinMakeDateSig) vecEvalTime(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf1, err := b.bufAllocator.get()
	if err != nil {
		return err
	...
	defer b.bufAllocator.put(buf1)
	if err := b.args[0].VecEvalInt(sctx, input, buf1); err != nil {
		return err
	}
	...
	for i := 0; i < n; i++ {
		...
		if retTimestamp == 0 {
			if err = handleInvalidTimeError(sctx, types.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, startTime.String())); err != nil {
				return err
			}
			result.SetNull(i, true)
			continue
...
func (b *builtinWeekOfYearSig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinUTCTimestampWithArgSig) vecEvalTime(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
	...
	for i := 0; i < n; i++ {
		...
		res, isNull, err := evalUTCTimestampWithFsp(sctx, int(fsp))
		if err != nil {
			return err
		}
		if isNull {
...
func (b *builtinTimeToSecSig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinStrToDateDatetimeSig) vecEvalTime(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	...
	if err = b.args[0].VecEvalString(sctx, input, dateBuf); err != nil {
		return err
	}
	...
	if err = b.args[1].VecEvalString(sctx, input, formatBuf); err != nil {
		return err
	}
	...
	times := result.Times()
	sc := sctx.GetSessionVars().StmtCtx
	hasNoZeroDateMode := sctx.GetSessionVars().SQLMode.HasNoZeroDateMode()
	fsp := b.tp.GetDecimal()
...
func (b *builtinUTCDateSig) vecEvalTime(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	nowTs, err := getStmtTimestamp(sctx)
	if err != nil {
		return err
	}
...
func (b *builtinWeekWithoutModeSig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
	...
	if ok && modeStr != "" {
		...
		if err != nil {
			return handleInvalidTimeError(sctx, types.ErrInvalidWeekModeFormat.GenWithStackByArgs(modeStr))
		}
	}
	for i := 0; i < n; i++ {
		if result.IsNull(i) {
...
func (b *builtinUnixTimestampDecSig) vecEvalDecimal(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	...
	if err := b.args[0].VecEvalTime(sctx, input, timeBuf); err != nil {
		for i := 0; i < n; i++ {
			temp, isNull, err := b.evalDecimal(nil, input.GetRow(i))
			if err != nil {
				return err
			}
			ts[i] = *temp
		...
		for i := 0; i < n; i++ {
			...
			t, err := timeBuf.GetTime(i).GoTime(getTimeZone(sctx))
			if err != nil {
...
func (b *builtinPeriodAddSig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	if err := b.args[0].VecEvalInt(sctx, input, result); err != nil {
		return err
	}
...
func (b *builtinTimestampAddSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
	...
	defer b.bufAllocator.put(buf2)
	if err := b.args[2].VecEvalTime(sctx, input, buf2); err != nil {
		return err
	}
	...
	for i := 0; i < n; i++ {
		...
		if overflow {
			if err = handleInvalidTimeError(sctx, types.ErrDatetimeFunctionOverflow.GenWithStackByArgs("datetime")); err != nil {
				return err
			}
			result.AppendNull()
			continue
		...
		if err = r.Check(sctx.GetSessionVars().StmtCtx); err != nil {
			if err = handleInvalidTimeError(sctx, err); err != nil {
				return err
			}
			result.AppendNull()
			continue
...
func (b *builtinToDaysSig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinDateFormatSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	...
	defer b.bufAllocator.put(dateBuf)
	if err := b.args[0].VecEvalTime(sctx, input, dateBuf); err != nil {
		return err
	}
	times := dateBuf.Times()
	...
	for i := range times {
		...
		if t.InvalidZero() {
			...
			if errHandled := handleInvalidTimeError(sctx, types.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, t.String())); errHandled != nil {
				return errHandled
			}
			continue
		}
...
func (b *builtinHourSig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinSecToTimeSig) vecEvalDuration(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
	...
	for i := 0; i < n; i++ {
		...
		duration, _, err := types.ParseDuration(sctx.GetSessionVars().StmtCtx, fmt.Sprintf("%s%02d:%02d:%s", negative, hour, minute, strconv.FormatFloat(secondDemical, 'f', -1, 64)), b.tp.GetDecimal())
		if err != nil {
			return err
		}
...
// See https://dev.mysql.com/doc/refman/5.7/en/date-and-time-functions.html#function_utc-time
func (b *builtinUTCTimeWithoutArgSig) vecEvalDuration(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	nowTs, err := getStmtTimestamp(sctx)
	if err != nil {
		return err
	}
	res, _, err := types.ParseDuration(sctx.GetSessionVars().StmtCtx, nowTs.UTC().Format(types.TimeFormat), types.DefaultFsp)
	if err != nil {
		return err
	}
	result.ResizeGoDuration(n, false)
...
func (b *builtinDateDiffSig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf0, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinCurrentDateSig) vecEvalTime(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	nowTs, err := getStmtTimestamp(sctx)
	if err != nil {
		return err
	}
...
func (b *builtinMakeTimeSig) getVecIntParam(sctx sessionctx.Context, arg Expression, input *chunk.Chunk, col *chunk.Column) (err error) {
	if arg.GetType().EvalType() == types.ETReal {
		err = arg.VecEvalReal(sctx, input, col)
		if err != nil {
			return err
		}
		f64s := col.Float64s()
	...
	err = arg.VecEvalInt(sctx, input, col)
	return err
}

func (b *builtinMakeTimeSig) vecEvalDuration(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	result.ResizeInt64(n, false)
	hoursBuf := result
	var err error
	if err = b.getVecIntParam(sctx, b.args[0], input, hoursBuf); err != nil {
		return err
	}
	...
	if err = b.getVecIntParam(sctx, b.args[1], input, minutesBuf); err != nil {
		return err
	}
	...
	if err = b.args[2].VecEvalReal(sctx, input, secondsBuf); err != nil {
		return err
	}
	...
	for i := 0; i < n; i++ {
		...
		dur, err := b.makeTime(sctx, hours[i], minutes[i], seconds[i], hourUnsignedFlag)
		if err != nil {
			return err
		}
		durs[i] = dur.Duration
...
func (b *builtinDayOfYearSig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinFromUnixTime1ArgSig) vecEvalTime(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
// See https://dev.mysql.com/doc/refman/5.7/en/date-and-time-functions.html#function_yearweek
func (b *builtinYearWeekWithModeSig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf1, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinTimestampDiffSig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	...
	if err := b.args[0].VecEvalString(sctx, input, unitBuf); err != nil {
		return err
	}
	...
	if err := b.args[1].VecEvalTime(sctx, input, lhsBuf); err != nil {
		return err
	}
	...
	if err := b.args[2].VecEvalTime(sctx, input, rhsBuf); err != nil {
		return err
	}
	...
	for i := 0; i < n; i++ {
		...
		if invalidLHS, invalidRHS := lhs[i].InvalidZero(), rhs[i].InvalidZero(); invalidLHS || invalidRHS {
			if invalidLHS {
				err = handleInvalidTimeError(sctx, types.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, lhs[i].String()))
			}
			if invalidRHS {
				err = handleInvalidTimeError(sctx, types.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, rhs[i].String()))
			}
			if err != nil {
				return err
			}
...
func (b *builtinUnixTimestampIntSig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
	...
	if err := b.args[0].VecEvalTime(sctx, input, buf); err != nil {
		var isNull bool
		for i := 0; i < n; i++ {
			i64s[i], isNull, err = b.evalInt(nil, input.GetRow(i))
			if err != nil {
				return err
			}
			if isNull {
		...
		for i := 0; i < n; i++ {
			...
			t, err := buf.GetTime(i).AdjustedGoTime(getTimeZone(sctx))
			if err != nil {
				i64s[i] = 0
				continue
			}
...
func (b *builtinCurrentTime0ArgSig) vecEvalDuration(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	nowTs, err := getStmtTimestamp(sctx)
	if err != nil {
		return err
...
func (b *builtinTimeSig) vecEvalDuration(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinDateLiteralSig) vecEvalTime(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
...
func (b *builtinTimeLiteralSig) vecEvalDuration(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	result.ResizeGoDuration(n, false)
	d64s := result.GoDurations()
	for i := 0; i < n; i++ {
...
func (b *builtinMonthNameSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
	...
	for i := 0; i < n; i++ {
		...
		mon := ds[i].Month()
		if (ds[i].IsZero() && sctx.GetSessionVars().SQLMode.HasNoZeroDateMode()) || mon < 0 || mon > len(types.MonthNames) {
			if err := handleInvalidTimeError(sctx, types.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, ds[i].String())); err != nil {
				return err
			}
			result.AppendNull()
...
func (b *builtinDayOfWeekSig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinCurrentTime1ArgSig) vecEvalDuration(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
// See https://dev.mysql.com/doc/refman/5.7/en/date-and-time-functions.html#function_utc-timestamp
func (b *builtinUTCTimestampWithoutArgSig) vecEvalTime(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	res, isNull, err := evalUTCTimestampWithFsp(sctx, types.DefaultFsp)
	if err != nil {
		return err
	}
	if isNull {
...
func (b *builtinConvertTzSig) vecEvalTime(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	if err := b.args[0].VecEvalTime(sctx, input, result); err != nil {
		return err
	}
	...
	if err := b.args[1].VecEvalString(sctx, input, fromTzBuf); err != nil {
		return err
	}
	...
	if err := b.args[2].VecEvalString(sctx, input, toTzBuf); err != nil {
		return err
	}
...
func (b *builtinTimestamp1ArgSig) vecEvalTime(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinTimestamp2ArgsSig) vecEvalTime(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf0, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinDayOfMonthSig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinAddSubDateAsStringSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	unit, isNull, err := b.args[2].EvalString(sctx, chunk.Row{})
	if err != nil {
		return err
	}
	if isNull {
	...
	if err := b.vecGetInterval(&b.baseDateArithmetical, sctx, &b.baseBuiltinFunc, input, unit, intervalBuf); err != nil {
		return err
	}
	...
	defer b.bufAllocator.put(dateBuf)
	if err := b.vecGetDate(&b.baseDateArithmetical, sctx, &b.baseBuiltinFunc, input, unit, dateBuf); err != nil {
		return err
	}
	...
	for i := 0; i < n; i++ {
		...
		resDate, isNull, err := b.timeOp(&b.baseDateArithmetical, sctx, dateBuf.Times()[i], intervalBuf.GetString(i), unit, b.tp.GetDecimal())
		if err != nil {
			return err
		}
		if isNull {
...
func (b *builtinAddSubDateDatetimeAnySig) vecEvalTime(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	unit, isNull, err := b.args[2].EvalString(sctx, chunk.Row{})
	if err != nil {
		return err
	...
	if err := b.vecGetDateFromDatetime(&b.baseBuiltinFunc, sctx, input, unit, result); err != nil {
		return err
	}
	...
	for i := 0; i < n; i++ {
		...
		resDate, isNull, err := b.timeOp(&b.baseDateArithmetical, sctx, resDates[i], intervalBuf.GetString(i), unit, b.tp.GetDecimal())
		if err != nil {
			return err
		}
		if isNull {
...
func (b *builtinAddSubDateDurationAnySig) vecEvalTime(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	unit, isNull, err := b.args[2].EvalString(sctx, chunk.Row{})
	if err != nil {
		return err
	...
	if err := b.args[0].VecEvalDuration(sctx, input, durBuf); err != nil {
		return err
	}
	...
	for i := 0; i < n; i++ {
		...
		resDate, isNull, err := b.timeOp(&b.baseDateArithmetical, sctx, t, intervalBuf.GetString(i), unit, b.tp.GetDecimal())
		if err != nil {
			return err
		}
		if isNull {
...
func (b *builtinAddSubDateDurationAnySig) vecEvalDuration(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	unit, isNull, err := b.args[2].EvalString(sctx, chunk.Row{})
	if err != nil {
		return err
	...
	result.ResizeGoDuration(n, false)
	if err := b.args[0].VecEvalDuration(sctx, input, result); err != nil {
		return err
	}
	...
	for i := 0; i < n; i++ {
		...
		resDuration, isNull, err := b.durationOp(&b.baseDateArithmetical, sctx, iterDuration, intervalBuf.GetString(i), unit, b.tp.GetDecimal())
		if err != nil {
			return err
		}
		if isNull {
