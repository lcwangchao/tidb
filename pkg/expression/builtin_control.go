...
func (b *builtinCaseWhenIntSig) evalInt(sctx sessionctx.Context, row chunk.Row) (ret int64, isNull bool, err error) {
	var condition int64
	args, l := b.getArgs(), len(b.getArgs())
	for i := 0; i < l-1; i += 2 {
		condition, isNull, err = args[i].EvalInt(sctx, row)
		if err != nil {
			return 0, isNull, err
		}
		...
		ret, isNull, err = args[i+1].EvalInt(sctx, row)
	...
	if l%2 == 1 {
		ret, isNull, err = args[l-1].EvalInt(sctx, row)
		return ret, isNull, err
	}
	return ret, true, nil
}
...
// See https://dev.mysql.com/doc/refman/5.7/en/control-flow-functions.html#operator_case
func (b *builtinCaseWhenRealSig) evalReal(sctx sessionctx.Context, row chunk.Row) (ret float64, isNull bool, err error) {
	var condition int64
	args, l := b.getArgs(), len(b.getArgs())
	for i := 0; i < l-1; i += 2 {
		condition, isNull, err = args[i].EvalInt(sctx, row)
		...
		ret, isNull, err = args[i+1].EvalReal(sctx, row)
		return ret, isNull, err
	}
	// when clause(condition, result) -> args[i], args[i+1]; (i >= 0 && i+1 < l-1)
	// else clause -> args[l-1]
	// If case clause has else clause, l%2 == 1.
	if l%2 == 1 {
		ret, isNull, err = args[l-1].EvalReal(sctx, row)
		return ret, isNull, err
	}
	return ret, true, nil
}
...
// See https://dev.mysql.com/doc/refman/5.7/en/control-flow-functions.html#operator_case
func (b *builtinCaseWhenDecimalSig) evalDecimal(sctx sessionctx.Context, row chunk.Row) (ret *types.MyDecimal, isNull bool, err error) {
	var condition int64
	args, l := b.getArgs(), len(b.getArgs())
	for i := 0; i < l-1; i += 2 {
		condition, isNull, err = args[i].EvalInt(sctx, row)
		...
		ret, isNull, err = args[i+1].EvalDecimal(sctx, row)
		return ret, isNull, err
	}
	// when clause(condition, result) -> args[i], args[i+1]; (i >= 0 && i+1 < l-1)
	// else clause -> args[l-1]
	// If case clause has else clause, l%2 == 1.
	if l%2 == 1 {
		ret, isNull, err = args[l-1].EvalDecimal(sctx, row)
		return ret, isNull, err
	}
	return ret, true, nil
}
...
// See https://dev.mysql.com/doc/refman/5.7/en/control-flow-functions.html#operator_case
func (b *builtinCaseWhenStringSig) evalString(sctx sessionctx.Context, row chunk.Row) (ret string, isNull bool, err error) {
	var condition int64
	args, l := b.getArgs(), len(b.getArgs())
	for i := 0; i < l-1; i += 2 {
		condition, isNull, err = args[i].EvalInt(sctx, row)
		...
		ret, isNull, err = args[i+1].EvalString(sctx, row)
		return ret, isNull, err
	}
	// when clause(condition, result) -> args[i], args[i+1]; (i >= 0 && i+1 < l-1)
	// else clause -> args[l-1]
	// If case clause has else clause, l%2 == 1.
	if l%2 == 1 {
		ret, isNull, err = args[l-1].EvalString(sctx, row)
		return ret, isNull, err
	}
	return ret, true, nil
}
...
// See https://dev.mysql.com/doc/refman/5.7/en/control-flow-functions.html#operator_case
func (b *builtinCaseWhenTimeSig) evalTime(sctx sessionctx.Context, row chunk.Row) (ret types.Time, isNull bool, err error) {
	var condition int64
	args, l := b.getArgs(), len(b.getArgs())
	for i := 0; i < l-1; i += 2 {
		condition, isNull, err = args[i].EvalInt(sctx, row)
		...
		ret, isNull, err = args[i+1].EvalTime(sctx, row)
		return ret, isNull, err
	}
	// when clause(condition, result) -> args[i], args[i+1]; (i >= 0 && i+1 < l-1)
	// else clause -> args[l-1]
	// If case clause has else clause, l%2 == 1.
	if l%2 == 1 {
		ret, isNull, err = args[l-1].EvalTime(sctx, row)
		return ret, isNull, err
	}
	return ret, true, nil
}
...
// See https://dev.mysql.com/doc/refman/5.7/en/control-flow-functions.html#operator_case
func (b *builtinCaseWhenDurationSig) evalDuration(sctx sessionctx.Context, row chunk.Row) (ret types.Duration, isNull bool, err error) {
	var condition int64
	args, l := b.getArgs(), len(b.getArgs())
	for i := 0; i < l-1; i += 2 {
		condition, isNull, err = args[i].EvalInt(sctx, row)
		...
		ret, isNull, err = args[i+1].EvalDuration(sctx, row)
		return ret, isNull, err
	}
	// when clause(condition, result) -> args[i], args[i+1]; (i >= 0 && i+1 < l-1)
	// else clause -> args[l-1]
	// If case clause has else clause, l%2 == 1.
	if l%2 == 1 {
		ret, isNull, err = args[l-1].EvalDuration(sctx, row)
		return ret, isNull, err
	}
	return ret, true, nil
}
...
// See https://dev.mysql.com/doc/refman/5.7/en/control-flow-functions.html#operator_case
func (b *builtinCaseWhenJSONSig) evalJSON(sctx sessionctx.Context, row chunk.Row) (ret types.BinaryJSON, isNull bool, err error) {
	var condition int64
	args, l := b.getArgs(), len(b.getArgs())
	for i := 0; i < l-1; i += 2 {
		condition, isNull, err = args[i].EvalInt(sctx, row)
		...
		return args[i+1].EvalJSON(sctx, row)
	}
	// when clause(condition, result) -> args[i], args[i+1]; (i >= 0 && i+1 < l-1)
	// else clause -> args[l-1]
	// If case clause has else clause, l%2 == 1.
	if l%2 == 1 {
		return args[l-1].EvalJSON(sctx, row)
	}
	return ret, true, nil
}
...
func (b *builtinIfIntSig) evalInt(sctx sessionctx.Context, row chunk.Row) (val int64, isNull bool, err error) {
	...
	return b.args[2].EvalInt(sctx, row)
}
...
func (b *builtinIfRealSig) evalReal(sctx sessionctx.Context, row chunk.Row) (val float64, isNull bool, err error) {
	arg0, isNull0, err := b.args[0].EvalInt(sctx, row)
	if err != nil {
		return 0, true, err
	}
	...
	return b.args[2].EvalReal(sctx, row)
}
...
func (b *builtinIfDecimalSig) evalDecimal(sctx sessionctx.Context, row chunk.Row) (val *types.MyDecimal, isNull bool, err error) {
	arg0, isNull0, err := b.args[0].EvalInt(sctx, row)
	if err != nil {
		return nil, true, err
	}
	...
	return b.args[2].EvalDecimal(sctx, row)
}
...
func (b *builtinIfStringSig) evalString(sctx sessionctx.Context, row chunk.Row) (val string, isNull bool, err error) {
	arg0, isNull0, err := b.args[0].EvalInt(sctx, row)
	if err != nil {
		return "", true, err
	}
	...
	return b.args[2].EvalString(sctx, row)
}
...
func (b *builtinIfTimeSig) evalTime(sctx sessionctx.Context, row chunk.Row) (ret types.Time, isNull bool, err error) {
	arg0, isNull0, err := b.args[0].EvalInt(sctx, row)
	if err != nil {
		return ret, true, err
	}
	...
	return b.args[2].EvalTime(sctx, row)
}
...
func (b *builtinIfDurationSig) evalDuration(sctx sessionctx.Context, row chunk.Row) (ret types.Duration, isNull bool, err error) {
	arg0, isNull0, err := b.args[0].EvalInt(sctx, row)
	if err != nil {
		return ret, true, err
	}
	...
	return b.args[2].EvalDuration(sctx, row)
}
...
func (b *builtinIfJSONSig) evalJSON(sctx sessionctx.Context, row chunk.Row) (ret types.BinaryJSON, isNull bool, err error) {
	arg0, isNull0, err := b.args[0].EvalInt(sctx, row)
	if err != nil {
		return ret, true, err
	}
	...
	return b.args[2].EvalJSON(sctx, row)
}
...
func (b *builtinIfNullIntSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	arg0, isNull, err := b.args[0].EvalInt(sctx, row)
	if !isNull || err != nil {
		return arg0, err != nil, err
	}
	arg1, isNull, err := b.args[1].EvalInt(sctx, row)
...
func (b *builtinIfNullRealSig) evalReal(sctx sessionctx.Context, row chunk.Row) (float64, bool, error) {
	arg0, isNull, err := b.args[0].EvalReal(sctx, row)
	if !isNull || err != nil {
		return arg0, err != nil, err
	}
	arg1, isNull, err := b.args[1].EvalReal(sctx, row)
	return arg1, isNull || err != nil, err
}
...
func (b *builtinIfNullDecimalSig) evalDecimal(sctx sessionctx.Context, row chunk.Row) (*types.MyDecimal, bool, error) {
	arg0, isNull, err := b.args[0].EvalDecimal(sctx, row)
	if !isNull || err != nil {
		return arg0, err != nil, err
	}
	arg1, isNull, err := b.args[1].EvalDecimal(sctx, row)
	return arg1, isNull || err != nil, err
}
...
func (b *builtinIfNullStringSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
...
func (b *builtinIfNullTimeSig) evalTime(sctx sessionctx.Context, row chunk.Row) (types.Time, bool, error) {
	arg0, isNull, err := b.args[0].EvalTime(sctx, row)
	if !isNull || err != nil {
		return arg0, err != nil, err
	}
	arg1, isNull, err := b.args[1].EvalTime(sctx, row)
	return arg1, isNull || err != nil, err
}
...
func (b *builtinIfNullDurationSig) evalDuration(sctx sessionctx.Context, row chunk.Row) (types.Duration, bool, error) {
...
func (b *builtinIfNullJSONSig) evalJSON(sctx sessionctx.Context, row chunk.Row) (types.BinaryJSON, bool, error) {
	arg0, isNull, err := b.args[0].EvalJSON(sctx, row)
	if !isNull {
		return arg0, err != nil, err
	}
	arg1, isNull, err := b.args[1].EvalJSON(sctx, row)
	return arg1, isNull || err != nil, err
}
