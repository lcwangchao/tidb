...
func (cf *concatFunction) initSeparator(_ *stmtctx.StatementContext, sctx sessionctx.Context, row chunk.Row) error {
	sepArg := cf.Args[len(cf.Args)-1]
	sepDatum, err := sepArg.Eval(sctx, row)
	if err != nil {
		return err
	}
	if sepDatum.IsNull() {
...
func (cf *concatFunction) Update(evalCtx *AggEvaluateContext, sc *stmtctx.StatementContext, row chunk.Row) error {
	...
	if !cf.sepInited {
		err := cf.initSeparator(sc, evalCtx.Sctx, row)
		if err != nil {
			return err
		}
		cf.sepInited = true
	...
	for i, length := 0, len(cf.Args)-1; i < length; i++ {
		value, err := cf.Args[i].Eval(evalCtx.Sctx, row)
		if err != nil {
			return err
		}
		if value.IsNull() {
...
func (cf *concatFunction) ResetContext(sctx sessionctx.Context, evalCtx *AggEvaluateContext) {
	if cf.HasDistinct {
