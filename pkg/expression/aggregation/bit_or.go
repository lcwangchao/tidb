...
func (bf *bitOrFunction) CreateContext(sctx sessionctx.Context) *AggEvaluateContext {
	evalCtx := bf.aggFunction.CreateContext(sctx)
	evalCtx.Value.SetUint64(0)
	return evalCtx
}

func (*bitOrFunction) ResetContext(sctx sessionctx.Context, evalCtx *AggEvaluateContext) {
	evalCtx.Value.SetUint64(0)
}
