...
func (b *builtinLengthSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	val, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return 0, isNull, err
	}
...
func (b *builtinASCIISig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	val, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return 0, isNull, err
	}
...
func (b *builtinConcatSig) evalString(sctx sessionctx.Context, row chunk.Row) (d string, isNull bool, err error) {
	//nolint: prealloc
	var s []byte
	for _, a := range b.getArgs() {
		d, isNull, err = a.EvalString(sctx, row)
		if isNull || err != nil {
			return d, isNull, err
		}
		if uint64(len(s)+len(d)) > b.maxAllowedPacket {
			return "", true, handleAllowedPacketOverflowed(sctx, "concat", b.maxAllowedPacket)
		}
		s = append(s, []byte(d)...)
	}
...
func (b *builtinConcatWSSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	args := b.getArgs()
	...
	N := len(args)
	if N > 0 {
		val, isNull, err := args[0].EvalString(sctx, row)
		if err != nil || isNull {
	...
	for i := 1; i < N; i++ {
		val, isNull, err := args[i].EvalString(sctx, row)
		if err != nil {
			return val, isNull, err
		}
		if isNull {
		...
		if uint64(targetLength) > b.maxAllowedPacket {
			return "", true, handleAllowedPacketOverflowed(sctx, "concat_ws", b.maxAllowedPacket)
		}
...
func (b *builtinLeftSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	str, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return "", true, err
	}
	left, isNull, err := b.args[1].EvalInt(sctx, row)
	if isNull || err != nil {
		return "", true, err
	}
	leftLength := int(left)
...
// evalString evals LEFT(str,len).
// See https://dev.mysql.com/doc/refman/5.7/en/string-functions.html#function_left
func (b *builtinLeftUTF8Sig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	str, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return "", true, err
	}
...
func (b *builtinRightSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	str, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return "", true, err
	}
	right, isNull, err := b.args[1].EvalInt(sctx, row)
	if isNull || err != nil {
		return "", true, err
	}
...
// evalString evals RIGHT(str,len).
// See https://dev.mysql.com/doc/refman/5.7/en/string-functions.html#function_right
func (b *builtinRightUTF8Sig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	str, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return "", true, err
	}
...
func (b *builtinRepeatSig) evalString(sctx sessionctx.Context, row chunk.Row) (val string, isNull bool, err error) {
	str, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return "", isNull, err
	}
	...
	num, isNull, err := b.args[1].EvalInt(sctx, row)
	if isNull || err != nil {
		return "", isNull, err
	}
	if num < 1 {
	...
	if uint64(byteLength)*uint64(num) > b.maxAllowedPacket {
		return "", true, handleAllowedPacketOverflowed(sctx, "repeat", b.maxAllowedPacket)
	}
...
func (b *builtinLowerUTF8Sig) evalString(sctx sessionctx.Context, row chunk.Row) (d string, isNull bool, err error) {
	d, isNull, err = b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return d, isNull, err
	}
...
// See https://dev.mysql.com/doc/refman/5.7/en/string-functions.html#function_lower
func (b *builtinLowerSig) evalString(sctx sessionctx.Context, row chunk.Row) (d string, isNull bool, err error) {
	d, isNull, err = b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return d, isNull, err
	}
...
func (b *builtinReverseSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	str, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return "", true, err
	}
...
// evalString evals a REVERSE(str).
// See https://dev.mysql.com/doc/refman/5.7/en/string-functions.html#function_reverse
func (b *builtinReverseUTF8Sig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	str, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return "", true, err
	}
...
func (b *builtinSpaceSig) evalString(sctx sessionctx.Context, row chunk.Row) (d string, isNull bool, err error) {
	var x int64

	x, isNull, err = b.args[0].EvalInt(sctx, row)
	if isNull || err != nil {
		return d, isNull, err
	}
	if x < 0 {
	...
	if uint64(x) > b.maxAllowedPacket {
		return d, true, handleAllowedPacketOverflowed(sctx, "space", b.maxAllowedPacket)
	}
...
func (b *builtinUpperUTF8Sig) evalString(sctx sessionctx.Context, row chunk.Row) (d string, isNull bool, err error) {
	d, isNull, err = b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return d, isNull, err
	}
...
// See https://dev.mysql.com/doc/refman/5.7/en/string-functions.html#function_upper
func (b *builtinUpperSig) evalString(sctx sessionctx.Context, row chunk.Row) (d string, isNull bool, err error) {
	d, isNull, err = b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return d, isNull, err
	}
...
func (b *builtinStrcmpSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	var (
		left, right string
		isNull      bool
		err         error
	...
	left, isNull, err = b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return 0, isNull, err
	}
	right, isNull, err = b.args[1].EvalString(sctx, row)
	if isNull || err != nil {
		return 0, isNull, err
	}
...
func (b *builtinReplaceSig) evalString(sctx sessionctx.Context, row chunk.Row) (d string, isNull bool, err error) {
	...
	oldStr, isNull, err = b.args[1].EvalString(sctx, row)
	if isNull || err != nil {
		return d, isNull, err
	}
	newStr, isNull, err = b.args[2].EvalString(sctx, row)
	if isNull || err != nil {
		return d, isNull, err
	}
	if oldStr == "" {
...
// See https://dev.mysql.com/doc/refman/5.7/en/cast-functions.html#function_convert
func (b *builtinConvertSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
...
func (b *builtinSubstring2ArgsSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	str, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return "", true, err
	}
	pos, isNull, err := b.args[1].EvalInt(sctx, row)
	if isNull || err != nil {
		return "", true, err
	}
...
// evalString evals SUBSTR(str,pos), SUBSTR(str FROM pos), SUBSTR() is a synonym for SUBSTRING().
// See https://dev.mysql.com/doc/refman/5.7/en/string-functions.html#function_substr
func (b *builtinSubstring2ArgsUTF8Sig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	str, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return "", true, err
	}
...
// See https://dev.mysql.com/doc/refman/5.7/en/string-functions.html#function_substr
func (b *builtinSubstring3ArgsSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	str, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return "", true, err
	}
	...
	length, isNull, err := b.args[2].EvalInt(sctx, row)
	if isNull || err != nil {
		return "", true, err
	}
...
// evalString evals SUBSTR(str,pos,len), SUBSTR(str FROM pos FOR len), SUBSTR() is a synonym for SUBSTRING().
// See https://dev.mysql.com/doc/refman/5.7/en/string-functions.html#function_substr
func (b *builtinSubstring3ArgsUTF8Sig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	str, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return "", true, err
	}
...
func (b *builtinSubstringIndexSig) evalString(sctx sessionctx.Context, row chunk.Row) (d string, isNull bool, err error) {
	var (
		str, delim string
		count      int64
	)
	...
	delim, isNull, err = b.args[1].EvalString(sctx, row)
	if isNull || err != nil {
		return d, isNull, err
	}
	count, isNull, err = b.args[2].EvalInt(sctx, row)
	if isNull || err != nil {
		return d, isNull, err
	}
	if len(delim) == 0 {
...
func (b *builtinLocate2ArgsSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	subStr, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return 0, isNull, err
	}
	str, isNull, err := b.args[1].EvalString(sctx, row)
	if isNull || err != nil {
		return 0, isNull, err
	}
...
// See https://dev.mysql.com/doc/refman/5.7/en/string-functions.html#function_locate
func (b *builtinLocate2ArgsUTF8Sig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	subStr, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return 0, isNull, err
	}
...
// See https://dev.mysql.com/doc/refman/5.7/en/string-functions.html#function_locate
func (b *builtinLocate3ArgsSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	subStr, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return 0, isNull, err
	}
	...
	pos, isNull, err := b.args[2].EvalInt(sctx, row)
...
// See https://dev.mysql.com/doc/refman/5.7/en/string-functions.html#function_locate
func (b *builtinLocate3ArgsUTF8Sig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	subStr, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return 0, isNull, err
	}
...
func (b *builtinHexStrArgSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	d, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return d, isNull, err
	}
...
// See https://dev.mysql.com/doc/refman/5.7/en/string-functions.html#function_hex
func (b *builtinHexIntArgSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	x, isNull, err := b.args[0].EvalInt(sctx, row)
	if isNull || err != nil {
		return "", isNull, err
	}
...
func (b *builtinUnHexSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	var bs []byte
...
func (b *builtinTrim1ArgSig) evalString(sctx sessionctx.Context, row chunk.Row) (d string, isNull bool, err error) {
	d, isNull, err = b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return d, isNull, err
	}
...
// See https://dev.mysql.com/doc/refman/5.7/en/string-functions.html#function_trim
func (b *builtinTrim2ArgsSig) evalString(sctx sessionctx.Context, row chunk.Row) (d string, isNull bool, err error) {
	...
	remstr, isNull, err = b.args[1].EvalString(sctx, row)
	if isNull || err != nil {
		return d, isNull, err
	}
...
// See https://dev.mysql.com/doc/refman/5.7/en/string-functions.html#function_trim
func (b *builtinTrim3ArgsSig) evalString(sctx sessionctx.Context, row chunk.Row) (d string, isNull bool, err error) {
	var (
		str, remstr  string
		x            int64
	...
	remstr, isRemStrNull, err = b.args[1].EvalString(sctx, row)
	if err != nil || isRemStrNull {
		return d, isRemStrNull, err
	}
	x, isNull, err = b.args[2].EvalInt(sctx, row)
	if isNull || err != nil {
		return d, isNull, err
	}
...
func (b *builtinLTrimSig) evalString(sctx sessionctx.Context, row chunk.Row) (d string, isNull bool, err error) {
	d, isNull, err = b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return d, isNull, err
	}
...
func (b *builtinRTrimSig) evalString(sctx sessionctx.Context, row chunk.Row) (d string, isNull bool, err error) {
	d, isNull, err = b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return d, isNull, err
	}
...
func (b *builtinLpadSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	str, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return "", true, err
	}
	...
	length, isNull, err := b.args[1].EvalInt(sctx, row)
	if isNull || err != nil {
		return "", true, err
	}
	...
	if uint64(targetLength) > b.maxAllowedPacket {
		return "", true, handleAllowedPacketOverflowed(sctx, "lpad", b.maxAllowedPacket)
	}

	padStr, isNull, err := b.args[2].EvalString(sctx, row)
	if isNull || err != nil {
		return "", true, err
	}
...
// evalString evals LPAD(str,len,padstr).
// See https://dev.mysql.com/doc/refman/5.7/en/string-functions.html#function_lpad
func (b *builtinLpadUTF8Sig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	str, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return "", true, err
	}
...
func (b *builtinRpadSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	str, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return "", true, err
	}
	...
	targetLength := int(length)
	if uint64(targetLength) > b.maxAllowedPacket {
		return "", true, handleAllowedPacketOverflowed(sctx, "rpad", b.maxAllowedPacket)
	}
...
// evalString evals RPAD(str,len,padstr).
// See https://dev.mysql.com/doc/refman/5.7/en/string-functions.html#function_rpad
func (b *builtinRpadUTF8Sig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	str, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return "", true, err
	}
...
func (b *builtinBitLengthSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	val, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return 0, isNull, err
	}
...
func (b *builtinCharSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	bigints := make([]int64, 0, len(b.args)-1)

	for i := 0; i < len(b.args)-1; i++ {
		val, IsNull, err := b.args[i].EvalInt(sctx, row)
		if err != nil {
			return "", true, err
		}
		if IsNull {
	...
	if err != nil {
		...
		if sctx.GetSessionVars().StrictSQLMode {
			return "", true, nil
		}
	}
...
func (b *builtinCharLengthBinarySig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	val, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return 0, isNull, err
	}
...
// See https://dev.mysql.com/doc/refman/5.7/en/string-functions.html#function_char-length
func (b *builtinCharLengthUTF8Sig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	val, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return 0, isNull, err
	}
...
func (b *builtinFindInSetSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	str, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return 0, isNull, err
	}

	strlist, isNull, err := b.args[1].EvalString(sctx, row)
	if isNull || err != nil {
		return 0, isNull, err
	}
...
func (b *builtinFieldIntSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	str, isNull, err := b.args[0].EvalInt(sctx, row)
	if isNull || err != nil {
		return 0, err != nil, err
	}
	for i, length := 1, len(b.args); i < length; i++ {
		stri, isNull, err := b.args[i].EvalInt(sctx, row)
		if err != nil {
			return 0, true, err
		}
		if !isNull && str == stri {
...
// evalInt evals FIELD(str,str1,str2,str3,...).
// See https://dev.mysql.com/doc/refman/5.7/en/string-functions.html#function_field
func (b *builtinFieldRealSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	str, isNull, err := b.args[0].EvalReal(sctx, row)
	if isNull || err != nil {
		return 0, err != nil, err
	}
	for i, length := 1, len(b.args); i < length; i++ {
		stri, isNull, err := b.args[i].EvalReal(sctx, row)
		if err != nil {
			return 0, true, err
		}
		if !isNull && str == stri {
...
// evalInt evals FIELD(str,str1,str2,str3,...).
// See https://dev.mysql.com/doc/refman/5.7/en/string-functions.html#function_field
func (b *builtinFieldStringSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	str, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return 0, err != nil, err
	}
	for i, length := 1, len(b.args); i < length; i++ {
		stri, isNull, err := b.args[i].EvalString(sctx, row)
		if err != nil {
			return 0, true, err
		}
...
func (b *builtinMakeSetSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	bits, isNull, err := b.args[0].EvalInt(sctx, row)
	if isNull || err != nil {
		return "", true, err
	}
	...
	for i, length := 1, len(b.args); i < length; i++ {
		...
		str, isNull, err := b.args[i].EvalString(sctx, row)
		if err != nil {
			return "", true, err
		}
		if !isNull {
...
func (b *builtinOctIntSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	val, isNull, err := b.args[0].EvalInt(sctx, row)
	if isNull || err != nil {
		return "", isNull, err
	}
...
// evalString evals OCT(N).
// See https://dev.mysql.com/doc/refman/5.7/en/string-functions.html#function_oct
func (b *builtinOctStringSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	val, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return "", isNull, err
	}
...
func (b *builtinOrdSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	str, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return 0, isNull, err
	}
...
func (b *builtinQuoteSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	str, isNull, err := b.args[0].EvalString(sctx, row)
	if err != nil {
		return "", true, err
	} else if isNull {
...
func (b *builtinBinSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	val, isNull, err := b.args[0].EvalInt(sctx, row)
	if isNull || err != nil {
		return "", true, err
	}
...
func (b *builtinEltSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	idx, isNull, err := b.args[0].EvalInt(sctx, row)
	if isNull || err != nil {
		return "", true, err
	}
	...
	arg, isNull, err := b.args[idx].EvalString(sctx, row)
	if isNull || err != nil {
		return "", true, err
	}
...
// See https://dev.mysql.com/doc/refman/5.7/en/string-functions.html#function_export-set
func (b *builtinExportSet3ArgSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	bits, isNull, err := b.args[0].EvalInt(sctx, row)
	if isNull || err != nil {
		return "", true, err
	}

	on, isNull, err := b.args[1].EvalString(sctx, row)
	if isNull || err != nil {
		return "", true, err
	}

	off, isNull, err := b.args[2].EvalString(sctx, row)
	if isNull || err != nil {
		return "", true, err
	}
...
// See https://dev.mysql.com/doc/refman/5.7/en/string-functions.html#function_export-set
func (b *builtinExportSet4ArgSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	bits, isNull, err := b.args[0].EvalInt(sctx, row)
	if isNull || err != nil {
		return "", true, err
	}
	...
	separator, isNull, err := b.args[3].EvalString(sctx, row)
	if isNull || err != nil {
		return "", true, err
	}
...
// See https://dev.mysql.com/doc/refman/5.7/en/string-functions.html#function_export-set
func (b *builtinExportSet5ArgSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	bits, isNull, err := b.args[0].EvalInt(sctx, row)
	if isNull || err != nil {
		return "", true, err
	}
	...
	numberOfBits, isNull, err := b.args[4].EvalInt(sctx, row)
	if isNull || err != nil {
		return "", true, err
	}
...
func (b *builtinFormatWithLocaleSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	x, d, isNull, err := evalNumDecArgsForFormat(sctx, b, row)
	if isNull || err != nil {
		return "", isNull, err
	}
	locale, isNull, err := b.args[2].EvalString(sctx, row)
	if err != nil {
		return "", false, err
	}
	if isNull {
		...
		sctx.GetSessionVars().StmtCtx.AppendWarning(errUnknownLocale.GenWithStackByArgs(locale))
	}
	locale = "en_US"
...
// See https://dev.mysql.com/doc/refman/5.7/en/string-functions.html#function_format
func (b *builtinFormatSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	x, d, isNull, err := evalNumDecArgsForFormat(sctx, b, row)
	if isNull || err != nil {
		return "", isNull, err
	}
...
func (b *builtinFromBase64Sig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	str, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return "", true, err
	}
	...
	if needDecodeLen > int(b.maxAllowedPacket) {
		return "", true, handleAllowedPacketOverflowed(sctx, "from_base64", b.maxAllowedPacket)
	}
...
func (b *builtinToBase64Sig) evalString(sctx sessionctx.Context, row chunk.Row) (val string, isNull bool, err error) {
	str, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return "", isNull, err
	}
	...
	if needEncodeLen > int(b.maxAllowedPacket) {
		return "", true, handleAllowedPacketOverflowed(sctx, "to_base64", b.maxAllowedPacket)
	}
...
func (b *builtinInsertSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	str, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return "", true, err
	}
	...
	newstr, isNull, err := b.args[3].EvalString(sctx, row)
	if isNull || err != nil {
		return "", true, err
	}
	...
	if uint64(strLength-length+int64(len(newstr))) > b.maxAllowedPacket {
		return "", true, handleAllowedPacketOverflowed(sctx, "insert", b.maxAllowedPacket)
	}
...
// evalString evals INSERT(str,pos,len,newstr).
// See https://dev.mysql.com/doc/refman/5.6/en/string-functions.html#function_insert
func (b *builtinInsertUTF8Sig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	str, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return "", true, err
	}
...
func (b *builtinInstrUTF8Sig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	str, IsNull, err := b.args[0].EvalString(sctx, row)
	if IsNull || err != nil {
		return 0, true, err
	}
	substr, IsNull, err := b.args[1].EvalString(sctx, row)
	if IsNull || err != nil {
		return 0, true, err
	}
	if collate.IsCICollation(b.collation) {
...
// See https://dev.mysql.com/doc/refman/5.7/en/string-functions.html#function_instr
func (b *builtinInstrSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	str, IsNull, err := b.args[0].EvalString(sctx, row)
	if IsNull || err != nil {
		return 0, true, err
	}
...
func (b *builtinLoadFileSig) evalString(sctx sessionctx.Context, row chunk.Row) (d string, isNull bool, err error) {
	d, isNull, err = b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return d, isNull, err
	}
...
func (b *builtinWeightStringNullSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	return "", true, nil
}
...
// See https://dev.mysql.com/doc/refman/5.7/en/string-functions.html#function_weight-string
func (b *builtinWeightStringSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	str, isNull, err := b.args[0].EvalString(sctx, row)
	if err != nil {
		return "", false, err
	}
	...
	case weightStringPaddingAsChar:
		...
		if b.length < lenRunes {
			...
			if uint64(b.length-lenRunes) > b.maxAllowedPacket {
				return "", true, handleAllowedPacketOverflowed(sctx, "weight_string", b.maxAllowedPacket)
			}
	...
	case weightStringPaddingAsBinary:
		...
		if b.length < lenStr {
			...
			sctx.GetSessionVars().StmtCtx.AppendWarning(errTruncatedWrongValue.GenWithStackByArgs(tpInfo, str))
			str = str[:b.length]
		} else if b.length > lenStr {
			if uint64(b.length-lenStr) > b.maxAllowedPacket {
				return "", true, handleAllowedPacketOverflowed(sctx, "cast_as_binary", b.maxAllowedPacket)
			}
...
func (b *builtinTranslateBinarySig) evalString(sctx sessionctx.Context, row chunk.Row) (d string, isNull bool, err error) {
	var (
		srcStr, fromStr, toStr     string
		isFromStrNull, isToStrNull bool
		tgt                        []byte
	)
	srcStr, isNull, err = b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return d, isNull, err
	}
	fromStr, isFromStrNull, err = b.args[1].EvalString(sctx, row)
	if isFromStrNull || err != nil {
		return d, isFromStrNull, err
	}
	toStr, isToStrNull, err = b.args[2].EvalString(sctx, row)
...
// See https://docs.oracle.com/cd/B19306_01/server.102/b14200/functions196.htm
func (b *builtinTranslateUTF8Sig) evalString(sctx sessionctx.Context, row chunk.Row) (d string, isNull bool, err error) {
	var (
		srcStr, fromStr, toStr     string
		isFromStrNull, isToStrNull bool
		tgt                        strings.Builder
