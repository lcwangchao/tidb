...
func (re *regexpBaseFuncSig) canMemorize(sctx sessionctx.Context, matchTypeIdx int) bool {
	return re.args[patternIdx].ConstItem(sctx.GetSessionVars().StmtCtx) && (len(re.args) <= matchTypeIdx || re.args[matchTypeIdx].ConstItem(sctx.GetSessionVars().StmtCtx))
}
...
func (re *regexpBaseFuncSig) tryToMemorize(sctx sessionctx.Context, params []*funcParam, matchTypeIdx int, n int) error {
	// Check memorization
	if n == 0 || !re.canMemorize(sctx, matchTypeIdx) {
		return nil
	}
...
func (re *builtinRegexpLikeFuncSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	expr, isNull, err := re.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return 0, true, err
	}

	pat, isNull, err := re.args[1].EvalString(sctx, row)
	if isNull || err != nil {
		return 0, true, err
	} else if len(pat) == 0 {
	...
	matchType := ""
	if len(re.args) == 3 {
		matchType, isNull, err = re.args[2].EvalString(sctx, row)
		if isNull || err != nil {
			return 0, true, err
		}
	}
	...
	if re.canMemorize(sctx, regexpLikeMatchTypeIdx) {
...
func (re *builtinRegexpLikeFuncSig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	params := make([]*funcParam, 0, 3)
	defer releaseBuffers(&re.baseBuiltinFunc, params)

	for i := 0; i < 2; i++ {
		param, isConstNull, err := buildStringParam(sctx, &re.baseBuiltinFunc, i, input, false)
		if err != nil {
			return ErrRegexp.GenWithStackByArgs(err)
		}
		if isConstNull {
	...
	param, isConstNull, err := buildStringParam(sctx, &re.baseBuiltinFunc, 2, input, !hasMatchType)
	params = append(params, param)
	if err != nil {
		return ErrRegexp.GenWithStackByArgs(err)
	}
	...
	err = re.tryToMemorize(sctx, params, regexpLikeMatchTypeIdx, n)
	if err != nil {
		return err
	}
...
func (re *builtinRegexpSubstrFuncSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	expr, isNull, err := re.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return "", true, err
	}
	...
	if argNum >= 3 {
		pos, isNull, err := re.args[2].EvalInt(sctx, row)
		if isNull || err != nil {
			return "", true, err
		}
	...
	if argNum >= 4 {
		occurrence, isNull, err = re.args[3].EvalInt(sctx, row)
		if isNull || err != nil {
			return "", true, err
		}
	...
	if argNum == 5 {
		matchType, isNull, err = re.args[4].EvalString(sctx, row)
		if isNull || err != nil {
			return "", true, err
		}
	}
	...
	if re.canMemorize(sctx, regexpSubstrMatchTypeIdx) {
		re.once.Do(memorize) // Avoid data race
	}
...
func (re *builtinRegexpSubstrFuncSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	...
	param, isConstNull, err := buildIntParam(sctx, &re.baseBuiltinFunc, 2, input, !hasPosition, 1)
	params = append(params, param)
	...
	hasOccur := (paramLen >= 4)
	param, isConstNull, err = buildIntParam(sctx, &re.baseBuiltinFunc, 3, input, !hasOccur, 1)
	params = append(params, param)
	...
	param, isConstNull, err = buildStringParam(sctx, &re.baseBuiltinFunc, 4, input, !hasMatchType)
	params = append(params, param)
	...
	// Check memorization
	err = re.tryToMemorize(sctx, params, regexpSubstrMatchTypeIdx, n)
	if err != nil {
		return err
	}
...
func (re *builtinRegexpInStrFuncSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	expr, isNull, err := re.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return 0, true, err
	}
	...
	if argNum >= 5 {
		returnOption, isNull, err = re.args[4].EvalInt(sctx, row)
		if isNull || err != nil {
			return 0, true, err
		}
	...
	if argNum == 6 {
		matchType, isNull, err = re.args[5].EvalString(sctx, row)
		if isNull || err != nil {
			return 0, true, err
		}
	}
	...
	if re.canMemorize(sctx, regexpInstrMatchTypeIdx) {
		re.once.Do(memorize) // Avoid data race
	}
...
func (re *builtinRegexpInStrFuncSig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	params := make([]*funcParam, 0, 5)
	defer releaseBuffers(&re.baseBuiltinFunc, params)
	...
	param, isConstNull, err = buildIntParam(sctx, &re.baseBuiltinFunc, 4, input, !hasRetOpt, 0)
	params = append(params, param)
	...
	param, isConstNull, err = buildStringParam(sctx, &re.baseBuiltinFunc, 5, input, !hasMatchType)
	params = append(params, param)
	...
	err = re.tryToMemorize(sctx, params, regexpInstrMatchTypeIdx, n)
	if err != nil {
		return err
	}
...
func (c *regexpReplaceFunctionClass) getFunction(ctx sessionctx.Context, args []Expression) (builtinFunc, error) {
	...
	sig := builtinRegexpReplaceFuncSig{
		...
		instrMemorizedErr: nil,
		isInstrMemorized:  false,
	}
...
type builtinRegexpReplaceFuncSig struct {
	...
	instrMemorizedErr error
	instrMemorizeOnce sync.Once
	isInstrMemorized  bool
}
...
func (re *builtinRegexpReplaceFuncSig) canInstructionsMemorized(sctx sessionctx.Context) bool {
	return re.args[replacementIdx].ConstItem(sctx.GetSessionVars().StmtCtx)
}

func (re *builtinRegexpReplaceFuncSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	expr, isNull, err := re.args[0].EvalString(sctx, row)
	trimmedExpr := expr
	if isNull || err != nil {
		return "", true, err
	...
	repl, isNull, err := re.args[2].EvalString(sctx, row)
	if isNull || err != nil {
		return "", true, err
	}
	...
	trimmedLen := int64(0)
	if argNum >= 4 {
		pos, isNull, err = re.args[3].EvalInt(sctx, row)
		if isNull || err != nil {
			return "", true, err
		}
	...
	if argNum >= 5 {
		occurrence, isNull, err = re.args[4].EvalInt(sctx, row)
		if isNull || err != nil {
			return "", true, err
		}
	...
	if re.canMemorize(sctx, regexpReplaceMatchTypeIdx) {
		re.once.Do(memorize) // Avoid data race
	}

	memorizeInstructions := func() {
		re.instructions, err = getInstructions([]byte(repl))
		if err != nil {
			re.instrMemorizedErr = err
			return
		}
		re.isInstrMemorized = true
	}

	if re.canInstructionsMemorized(sctx) {
		re.instrMemorizeOnce.Do(memorizeInstructions) // Avoid data race
	}

	var instructions []Instruction
	if re.isInstrMemorized {
...
func (re *builtinRegexpReplaceFuncSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	...
	hasRepl := (paramLen >= 3)
	param, isConstNull, err := buildStringParam(sctx, &re.baseBuiltinFunc, 2, input, !hasRepl)
	params = append(params, param)
	...
	param, isConstNull, err = buildIntParam(sctx, &re.baseBuiltinFunc, 3, input, !hasPosition, 1)
	params = append(params, param)
	...
	// Handle occurrence parameter
	hasOccur := (paramLen >= 5)
	param, isConstNull, err = buildIntParam(sctx, &re.baseBuiltinFunc, 4, input, !hasOccur, 0)
	params = append(params, param)
	...
	err = re.tryToMemorize(sctx, params, regexpReplaceMatchTypeIdx, n)
	if err != nil {
		return err
	}
	...
	instructions := make([]Instruction, 0)
	isReplConst := re.baseBuiltinFunc.args[2].ConstItem(sctx.GetSessionVars().StmtCtx)
	if isReplConst {
		// repl is const
	...
	for i := 0; i < n; i++ {
		...
		if re.isBinaryCollation() {
			...
			if re.canMemorize(sctx, regexpReplaceMatchTypeIdx) {
				replacedBStr, _, err = re.getReplacedBinStr(re.memorizedRegexp, bexpr, trimmedBexpr, instructions, pos, occurrence)
			} else {
			...
			if re.canMemorize(sctx, regexpReplaceMatchTypeIdx) {
				replacedStr, _, err = re.getReplacedStr(re.memorizedRegexp, expr, trimmedExpr, instructions, trimmedLen+1, occurrence)
			} else {
