...
func (b *builtinJSONStorageFreeSig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
...
func (b *builtinJSONStorageSizeSig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinJSONDepthSig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinJSONKeysSig) vecEvalJSON(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinJSONInsertSig) vecEvalJSON(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	err := vecJSONModify(sctx, b.args, b.bufAllocator, input, result, types.JSONModifyInsert)
	return err
}
...
func (b *builtinJSONReplaceSig) vecEvalJSON(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	err := vecJSONModify(sctx, b.args, b.bufAllocator, input, result, types.JSONModifyReplace)
	return err
}
...
func (b *builtinJSONArraySig) vecEvalJSON(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	nr := input.NumRows()
	...
	for _, arg := range b.args {
		...
		if err = arg.VecEvalJSON(sctx, input, j); err != nil {
			return err
		}
		for i := 0; i < nr; i++ {
			if j.IsNull(i) {
...
func (b *builtinJSONMemberOfSig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	nr := input.NumRows()
	...
	if err := b.args[0].VecEvalJSON(sctx, input, targetCol); err != nil {
		return err
	}
	...
	if err := b.args[1].VecEvalJSON(sctx, input, objCol); err != nil {
		return err
	}
...
func (b *builtinJSONContainsSig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	nr := input.NumRows()
	...
	if err := b.args[0].VecEvalJSON(sctx, input, objCol); err != nil {
		return err
	}
	...
	if err := b.args[1].VecEvalJSON(sctx, input, targetCol); err != nil {
		return err
	}
	...
	if len(b.args) == 3 {
		...
		if err := b.args[2].VecEvalString(sctx, input, pathCol); err != nil {
			return err
		}
...
func (b *builtinJSONOverlapsSig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	nr := input.NumRows()
...
func (b *builtinJSONQuoteSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinJSONSearchSig) vecEvalJSON(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	nr := input.NumRows()
	...
	if err := b.args[0].VecEvalJSON(sctx, input, jsonBuf); err != nil {
		return err
	}
	...
	if err := b.args[1].VecEvalString(sctx, input, typeBuf); err != nil {
		return err
	}
	...
	if err := b.args[2].VecEvalString(sctx, input, searchBuf); err != nil {
		return err
	}
	...
	if len(b.args) >= 4 {
		...
		if err := b.args[3].VecEvalString(sctx, input, escapeBuf); err != nil {
			return err
		}
	}
	...
	if len(b.args) >= 5 {
		...
		for i := 4; i < len(b.args); i++ {
			...
			if err := b.args[i].VecEvalString(sctx, input, pathBufs[index]); err != nil {
				return err
			}
		}
	}
...
func (b *builtinJSONSetSig) vecEvalJSON(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	err := vecJSONModify(sctx, b.args, b.bufAllocator, input, result, types.JSONModifySet)
	return err
}
...
func (b *builtinJSONObjectSig) vecEvalJSON(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	nr := input.NumRows()
	if len(b.args)&1 == 1 {
	...
	for i := 0; i < len(b.args); i++ {
		if i&1 == 0 {
			...
			if err = b.args[i].VecEvalString(sctx, input, argBuffers[i]); err != nil {
				return err
			}
		} else {
			if argBuffers[i], err = b.bufAllocator.get(); err != nil {
			...
			if err = b.args[i].VecEvalJSON(sctx, input, argBuffers[i]); err != nil {
				return err
			}
		}
	}
...
func (b *builtinJSONArrayInsertSig) vecEvalJSON(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	nr := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
	...
	for i := 1; i < len(b.args); i++ {
		if i&1 == 0 {
			...
			if err := b.args[i].VecEvalJSON(sctx, input, valueBufs[i/2-1]); err != nil {
				return err
			}
		} else {
			...
			if err := b.args[i].VecEvalString(sctx, input, pathBufs[(i-1)/2]); err != nil {
				return err
			}
		}
	}
...
func (b *builtinJSONKeys2ArgsSig) vecEvalJSON(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	nr := input.NumRows()
	jsonBuf, err := b.bufAllocator.get()
	if err != nil {
		return err
	...
	if err := b.args[1].VecEvalString(sctx, input, pathBuf); err != nil {
		return err
	}
...
func (b *builtinJSONLengthSig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	nr := input.NumRows()
...
func (b *builtinJSONTypeSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinJSONExtractSig) vecEvalJSON(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	var err error
	...
	for k := 0; k < len(pathArgs); k++ {
		...
		if err := pathArgs[k].VecEvalString(sctx, input, pathBuffers[k]); err != nil {
			return err
		}
	}
...
func (b *builtinJSONRemoveSig) vecEvalJSON(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	nr := input.NumRows()
	jsonBuf, err := b.bufAllocator.get()
	if err != nil {
		return err
	...
	for i := 1; i < len(b.args); i++ {
		...
		if err := b.args[i].VecEvalString(sctx, input, strBufs[i-1]); err != nil {
			return err
		}
	}
...
func (b *builtinJSONMergeSig) vecEvalJSON(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	nr := input.NumRows()
	argBuffers := make([]*chunk.Column, len(b.args))
	var err error
	for i, arg := range b.args {
		...
		if err := arg.VecEvalJSON(sctx, input, argBuffers[i]); err != nil {
			return err
		}
	}
...
func (b *builtinJSONContainsPathSig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	jsonBuf, err := b.bufAllocator.get()
	if err != nil {
		return err
	...
	for i := 0; i < len(pathBufs); i++ {
		...
		pathBufs[i] = pathBuf
		if err := b.args[2+i].VecEvalString(sctx, input, pathBuf); err != nil {
			return err
		}
	}
...
func (b *builtinJSONArrayAppendSig) vecEvalJSON(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	m := (len(b.args) - 1) / 2
	...
	if err := b.args[0].VecEvalJSON(sctx, input, jsonBufs); err != nil {
		return err
	}
	...
	for i := 1; i < len(b.args)-1; i += 2 {
		...
		if err := b.args[i].VecEvalString(sctx, input, pathBuf); err != nil {
			return err
		}
		...
		if err := b.args[i+1].VecEvalJSON(sctx, input, valBuf); err != nil {
			return err
		}
...
func (b *builtinJSONUnquoteSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinJSONSPrettySig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinJSONMergePatchSig) vecEvalJSON(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	nr := input.NumRows()
	argBuffers := make([]*chunk.Column, len(b.args))
	var err error
	for i, arg := range b.args {
