...
func TestBaseBuiltin(t *testing.T) {
	...
	require.Error(t, err)
	_, _, err = bf.evalString(nil, chunk.Row{})
	require.Error(t, err)
	_, _, err = bf.evalDecimal(nil, chunk.Row{})
	require.Error(t, err)
	_, _, err = bf.evalTime(nil, chunk.Row{})
	require.Error(t, err)
	_, _, err = bf.evalDuration(nil, chunk.Row{})
	require.Error(t, err)
	_, _, err = bf.evalJSON(nil, chunk.Row{})
	require.Error(t, err)
}
...
func TestHashGroupKey(t *testing.T) {
	...
	for i := 0; i < len(tNames); i++ {
		...
		for j := 0; j < input.NumRows(); j++ {
			d, err := colExpr.Eval(nil, input.GetRow(j))
			require.NoError(t, err)
