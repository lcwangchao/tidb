...
func (b *builtinDatabaseSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
...
func (b *builtinFoundRowsSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	data := sctx.GetSessionVars()
	if data == nil {
...
func (b *builtinCurrentUserSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	data := sctx.GetSessionVars()
...
func (b *builtinCurrentRoleSig) evalString(sctx sessionctx.Context, row chunk.Row) (res string, isNull bool, err error) {
	data := sctx.GetSessionVars()
...
func (b *builtinCurrentResourceGroupSig) evalString(sctx sessionctx.Context, row chunk.Row) (val string, isNull bool, err error) {
	data := sctx.GetSessionVars()
	if data == nil {
		return "", true, errors.Errorf("Missing session variable when eval builtin")
	}
...
func (b *builtinUserSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	data := sctx.GetSessionVars()
	if data == nil || data.User == nil {
		return "", true, errors.Errorf("Missing session variable when eval builtin")
	}
...
func (b *builtinConnectionIDSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	data := sctx.GetSessionVars()
	if data == nil {
...
func (b *builtinLastInsertIDSig) evalInt(sctx sessionctx.Context, row chunk.Row) (res int64, isNull bool, err error) {
	res = int64(sctx.GetSessionVars().StmtCtx.PrevLastInsertID)
	return res, false, nil
}
...
// See https://dev.mysql.com/doc/refman/5.7/en/information-functions.html#function_last-insert-id.
func (b *builtinLastInsertIDWithIDSig) evalInt(sctx sessionctx.Context, row chunk.Row) (res int64, isNull bool, err error) {
	res, isNull, err = b.args[0].EvalInt(sctx, row)
	if isNull || err != nil {
		return res, isNull, err
	}

	sctx.GetSessionVars().SetLastInsertID(uint64(res))
	return res, false, nil
}
...
func (b *builtinVersionSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
...
func (b *builtinTiDBVersionSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
...
func (b *builtinTiDBIsDDLOwnerSig) evalInt(sctx sessionctx.Context, row chunk.Row) (res int64, isNull bool, err error) {
	if sctx.IsDDLOwner() {
		res = 1
	}
...
func (b *builtinBenchmarkSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	// Get loop count.
	var loopCount int64
	var isNull bool
	var err error
	if b.constLoopCount > 0 {
		...
		loopCount, isNull, err = b.args[0].EvalInt(sctx, row)
		if isNull || err != nil {
			return 0, isNull, err
		}
	}
...
func (b *builtinCharsetSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
...
func (c *builtinCoercibilitySig) evalInt(sctx sessionctx.Context, row chunk.Row) (val int64, isNull bool, err error) {
...
func (b *builtinCollationSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
...
func (b *builtinRowCountSig) evalInt(sctx sessionctx.Context, row chunk.Row) (res int64, isNull bool, err error) {
	res = sctx.GetSessionVars().StmtCtx.PrevAffectedRows
	return res, false, nil
}
...
func (b *builtinTiDBDecodeKeySig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	s, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return "", isNull, err
	}
	decode := func(ctx sessionctx.Context, s string) string { return s }
	if fn := sctx.Value(TiDBDecodeKeyFunctionKey); fn != nil {
		decode = fn.(func(ctx sessionctx.Context, s string) string)
	}
	return decode(sctx, s), false, nil
}
...
func (b *builtinTiDBDecodeSQLDigestsSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	args := b.getArgs()
	digestsStr, isNull, err := args[0].EvalString(sctx, row)
	if err != nil {
		return "", true, err
	}
	if isNull {
	...
	if len(args) > 1 {
		stmtTruncateLength, isNull, err = args[1].EvalInt(sctx, row)
		if err != nil {
			return "", true, err
		}
		if isNull {
	...
	if err != nil {
		...
		sctx.GetSessionVars().StmtCtx.AppendWarning(errIncorrectArgs.GenWithStack("The argument can't be unmarshalled as JSON array: '%s'", digestsStr))
		return "", true, nil
	}
	...
	timeout := time.Duration(sctx.GetSessionVars().MaxExecutionTime) * time.Millisecond
	if timeout == 0 || timeout > 20*time.Second {
		timeout = 20 * time.Second
	}
	ctx, cancel := context.WithTimeout(context.Background(), timeout)
	defer cancel()
	err = retriever.RetrieveGlobal(ctx, sctx)
	if err != nil {
		...
		sctx.GetSessionVars().StmtCtx.AppendWarning(errUnknown.GenWithStack("Retrieving statements information failed with error: %v", err))
		return "", true, nil
	}
	...
	if err != nil {
		sctx.GetSessionVars().StmtCtx.AppendWarning(errUnknown.GenWithStack("Marshalling result as JSON failed with error: %v", err))
		return "", true, nil
	}
...
func (b *builtinTiDBEncodeSQLDigestSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	orgSQLStr, isNull, err := b.getArgs()[0].EvalString(sctx, row)
	if err != nil {
		return "", true, err
	}
	if isNull {
...
func (b *builtinTiDBDecodePlanSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	planString, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return "", isNull, err
	}
...
func (b *builtinTiDBDecodeBinaryPlanSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	planString, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return "", isNull, err
	}
...
func (b *builtinNextValSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	sequenceName, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return 0, isNull, err
	}
	...
	sequence, err := util.GetSequenceByName(sctx.GetInfoSchema(), model.NewCIStr(db), model.NewCIStr(seq))
	if err != nil {
		return 0, false, err
	}
	...
	if checker != nil && !checker.RequestVerification(sctx.GetSessionVars().ActiveRoles, db, seq, "", mysql.InsertPriv) {
		return 0, false, errSequenceAccessDenied.GenWithStackByArgs("INSERT", user.AuthUsername, user.AuthHostname, seq)
	}
	nextVal, err := sequence.GetSequenceNextVal(sctx, db, seq)
	if err != nil {
		return 0, false, err
	}
	// update the sequenceState.
	sctx.GetSessionVars().SequenceState.UpdateState(sequence.GetSequenceID(), nextVal)
...
func (b *builtinLastValSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	sequenceName, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return 0, isNull, err
	}
	...
	checker := privilege.GetPrivilegeManager(sctx)
	user := sctx.GetSessionVars().User
	if checker != nil && !checker.RequestVerification(sctx.GetSessionVars().ActiveRoles, db, seq, "", mysql.SelectPriv) {
		return 0, false, errSequenceAccessDenied.GenWithStackByArgs("SELECT", user.AuthUsername, user.AuthHostname, seq)
	}
	return sctx.GetSessionVars().SequenceState.GetLastValue(sequence.GetSequenceID())
}
...
func (b *builtinSetValSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	sequenceName, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return 0, isNull, err
	}
	...
	setValue, isNull, err := b.args[1].EvalInt(sctx, row)
	if isNull || err != nil {
		return 0, isNull, err
	}
	return sequence.SetSequenceVal(sctx, setValue, db, seq)
}
...
func (b *builtinFormatBytesSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
...
func (b *builtinFormatNanoTimeSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	val, isNull, err := b.args[0].EvalReal(sctx, row)
	if isNull || err != nil {
		return "", isNull, err
	}
