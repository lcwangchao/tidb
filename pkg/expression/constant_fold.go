...
func isNullHandler(expr *ScalarFunction) (Expression, bool) {
	...
	if constArg, isConst := arg0.(*Constant); isConst {
		...
		value, err := expr.Eval(NilEvalCtx, chunk.Row{})
		if err != nil {
...
func foldConstant(expr Expression) (Expression, bool) {
	...
	case *ScalarFunction:
		...
		if !allConstArg {
			...
			value, err := dummyScalarFunc.Eval(NilEvalCtx, chunk.Row{})
			if err != nil {
				return expr, isDeferredConst
			}
			if value.IsNull() {
		...
		value, err := x.Eval(NilEvalCtx, chunk.Row{})
	...
	case *Constant:
		if x.ParamMarker != nil {
			...
			value, err := x.DeferredExpr.Eval(NilEvalCtx, chunk.Row{})
			if err != nil {
				logutil.BgLogger().Debug("fold expression to constant", zap.String("expression", x.ExplainInfo()), zap.Error(err))
				return expr, true
			}
