...
func FoldConstant(sctx sessionctx.Context, expr Expression) Expression {
	e, _ := foldConstant(sctx, expr)
...
func isNullHandler(sctx sessionctx.Context, expr *ScalarFunction) (Expression, bool) {
	arg0 := expr.GetArgs()[0]
	if constArg, isConst := arg0.(*Constant); isConst {
		isDeferredConst := constArg.DeferredExpr != nil || constArg.ParamMarker != nil
		value, err := expr.Eval(sctx, chunk.Row{})
		if err != nil {
...
func ifFoldHandler(sctx sessionctx.Context, expr *ScalarFunction) (Expression, bool) {
	args := expr.GetArgs()
	foldedArg0, _ := foldConstant(sctx, args[0])
	if constArg, isConst := foldedArg0.(*Constant); isConst {
		arg0, isNull0, err := constArg.EvalInt(sctx, chunk.Row{})
		if err != nil {
			// Failed to fold this expr to a constant, print the DEBUG log and
			// return the original expression to let the error to be evaluated
			// again, in that time, the error is returned to the client.
		...
		if !isNull0 && arg0 != 0 {
			return foldConstant(sctx, args[1])
		}
		return foldConstant(sctx, args[2])
	}
	// if the condition is not const, which branch is unknown to run, so directly return.
	return expr, false
}

func ifNullFoldHandler(sctx sessionctx.Context, expr *ScalarFunction) (Expression, bool) {
	args := expr.GetArgs()
	foldedArg0, isDeferred := foldConstant(sctx, args[0])
	if constArg, isConst := foldedArg0.(*Constant); isConst {
...
func caseWhenHandler(sctx sessionctx.Context, expr *ScalarFunction) (Expression, bool) {
	args, l := expr.GetArgs(), len(expr.GetArgs())
	var isDeferred, isDeferredConst bool
	for i := 0; i < l-1; i += 2 {
		expr.GetArgs()[i], isDeferred = foldConstant(sctx, args[i])
		...
		val, isNull, err := args[i].EvalInt(sctx, chunk.Row{})
		if err != nil {
			return expr, false
		}
		if val != 0 && !isNull {
			foldedExpr, isDeferred := foldConstant(sctx, args[i+1])
			isDeferredConst = isDeferredConst || isDeferred
	...
	if l%2 == 1 {
		foldedExpr, isDeferred := foldConstant(sctx, args[l-1])
		isDeferredConst = isDeferredConst || isDeferred
		if _, isConst := foldedExpr.(*Constant); isConst {
			foldedExpr.GetType().SetDecimal(expr.GetType().GetDecimal())
			return foldedExpr, isDeferredConst
...
func foldConstant(sctx sessionctx.Context, expr Expression) (Expression, bool) {
	switch x := expr.(type) {
	case *ScalarFunction:
		if _, ok := unFoldableFunctions[x.FuncName.L]; ok {
			return expr, false
		}
		if function := specialFoldHandler[x.FuncName.L]; function != nil && !MaybeOverOptimized4PlanCache(sctx, []Expression{expr}) {
			return function(sctx, x)
		}
		...
		if !allConstArg {
			...
			dummyScalarFunc, err := NewFunctionBase(sctx, x.FuncName.L, x.GetType(), constArgs...)
			if err != nil {
				return expr, isDeferredConst
			}
			value, err := dummyScalarFunc.Eval(sctx, chunk.Row{})
			if err != nil {
				return expr, isDeferredConst
			}
			if value.IsNull() {
		...
		value, err := x.Eval(sctx, chunk.Row{})
	...
	case *Constant:
		if x.ParamMarker != nil {
			...
			value, err := x.DeferredExpr.Eval(sctx, chunk.Row{})
			if err != nil {
				logutil.BgLogger().Debug("fold expression to constant", zap.String("expression", x.ExplainInfo()), zap.Error(err))
				return expr, true
			}
