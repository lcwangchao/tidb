...
func (b *builtinDateSig) evalTime(sctx sessionctx.Context, row chunk.Row) (types.Time, bool, error) {
	expr, isNull, err := b.args[0].EvalTime(sctx, row)
	if isNull || err != nil {
		return types.ZeroTime, true, handleInvalidTimeError(sctx, err)
	}

	if expr.IsZero() && sctx.GetSessionVars().SQLMode.HasNoZeroDateMode() {
		return types.ZeroTime, true, handleInvalidTimeError(sctx, types.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, expr.String()))
	}
...
func (b *builtinDateLiteralSig) evalTime(sctx sessionctx.Context, row chunk.Row) (types.Time, bool, error) {
	mode := sctx.GetSessionVars().SQLMode
...
func (b *builtinDateDiffSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	lhs, isNull, err := b.args[0].EvalTime(sctx, row)
	if isNull || err != nil {
		return 0, true, handleInvalidTimeError(sctx, err)
	}
	rhs, isNull, err := b.args[1].EvalTime(sctx, row)
	if isNull || err != nil {
		return 0, true, handleInvalidTimeError(sctx, err)
	}
	if invalidLHS, invalidRHS := lhs.InvalidZero(), rhs.InvalidZero(); invalidLHS || invalidRHS {
		if invalidLHS {
			err = handleInvalidTimeError(sctx, types.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, lhs.String()))
		}
		if invalidRHS {
			err = handleInvalidTimeError(sctx, types.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, rhs.String()))
		}
		return 0, true, err
	}
...
func (b *builtinDurationDurationTimeDiffSig) evalDuration(sctx sessionctx.Context, row chunk.Row) (d types.Duration, isNull bool, err error) {
	lhs, isNull, err := b.args[0].EvalDuration(sctx, row)
	if isNull || err != nil {
		return d, isNull, err
	}

	rhs, isNull, err := b.args[1].EvalDuration(sctx, row)
	if isNull || err != nil {
		return d, isNull, err
	}

	d, isNull, err = calculateDurationTimeDiff(sctx, lhs, rhs)
	return d, isNull, err
}
...
// See https://dev.mysql.com/doc/refman/5.7/en/date-and-time-functions.html#function_timediff
func (b *builtinTimeTimeTimeDiffSig) evalDuration(sctx sessionctx.Context, row chunk.Row) (d types.Duration, isNull bool, err error) {
	lhs, isNull, err := b.args[0].EvalTime(sctx, row)
	if isNull || err != nil {
		return d, isNull, err
	}
...
// See https://dev.mysql.com/doc/refman/5.7/en/date-and-time-functions.html#function_timediff
func (b *builtinDurationStringTimeDiffSig) evalDuration(sctx sessionctx.Context, row chunk.Row) (d types.Duration, isNull bool, err error) {
	lhs, isNull, err := b.args[0].EvalDuration(sctx, row)
	if isNull || err != nil {
		return d, isNull, err
	}

	rhsStr, isNull, err := b.args[1].EvalString(sctx, row)
	if isNull || err != nil {
		return d, isNull, err
	}
...
// See https://dev.mysql.com/doc/refman/5.7/en/date-and-time-functions.html#function_timediff
func (b *builtinStringDurationTimeDiffSig) evalDuration(sctx sessionctx.Context, row chunk.Row) (d types.Duration, isNull bool, err error) {
	lhsStr, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return d, isNull, err
	}
...
// See https://dev.mysql.com/doc/refman/5.7/en/date-and-time-functions.html#function_timediff
func (b *builtinTimeStringTimeDiffSig) evalDuration(sctx sessionctx.Context, row chunk.Row) (d types.Duration, isNull bool, err error) {
	lhs, isNull, err := b.args[0].EvalTime(sctx, row)
	if isNull || err != nil {
		return d, isNull, err
	}
...
// See https://dev.mysql.com/doc/refman/5.7/en/date-and-time-functions.html#function_timediff
func (b *builtinStringTimeTimeDiffSig) evalDuration(sctx sessionctx.Context, row chunk.Row) (d types.Duration, isNull bool, err error) {
	lhsStr, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return d, isNull, err
	}
...
// See https://dev.mysql.com/doc/refman/5.7/en/date-and-time-functions.html#function_timediff
func (b *builtinStringStringTimeDiffSig) evalDuration(sctx sessionctx.Context, row chunk.Row) (d types.Duration, isNull bool, err error) {
	lhs, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return d, isNull, err
	}

	rhs, isNull, err := b.args[1].EvalString(sctx, row)
	if isNull || err != nil {
		return d, isNull, err
	}
	...
	if lhsIsDuration {
		d, isNull, err = calculateDurationTimeDiff(sctx, lhsDur, rhsDur)
	} else {
...
// See https://dev.mysql.com/doc/refman/5.7/en/date-and-time-functions.html#function_timediff
func (b *builtinNullTimeDiffSig) evalDuration(sctx sessionctx.Context, row chunk.Row) (d types.Duration, isNull bool, err error) {
	return d, true, nil
}
...
func (b *builtinDateFormatSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	t, isNull, err := b.args[0].EvalTime(sctx, row)
	if isNull || err != nil {
		return "", isNull, handleInvalidTimeError(sctx, err)
	}
	formatMask, isNull, err := b.args[1].EvalString(sctx, row)
	if isNull || err != nil {
		return "", isNull, err
	}
	...
	if t.InvalidZero() {
		...
		return "", true, handleInvalidTimeError(sctx, types.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, t.String()))
	}
...
func (b *builtinFromDaysSig) evalTime(sctx sessionctx.Context, row chunk.Row) (types.Time, bool, error) {
	n, isNull, err := b.args[0].EvalInt(sctx, row)
	if isNull || err != nil {
...
func (b *builtinHourSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	dur, isNull, err := b.args[0].EvalDuration(sctx, row)
...
func (b *builtinMinuteSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	dur, isNull, err := b.args[0].EvalDuration(sctx, row)
	// ignore error and return NULL
	if isNull || err != nil {
		return 0, true, nil
...
func (b *builtinSecondSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	dur, isNull, err := b.args[0].EvalDuration(sctx, row)
	// ignore error and return NULL
	if isNull || err != nil {
		return 0, true, nil
...
func (b *builtinMicroSecondSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	dur, isNull, err := b.args[0].EvalDuration(sctx, row)
	// ignore error and return NULL
	if isNull || err != nil {
		return 0, true, nil
...
func (b *builtinMonthSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	date, isNull, err := b.args[0].EvalTime(sctx, row)
...
func (b *builtinMonthNameSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	arg, isNull, err := b.args[0].EvalTime(sctx, row)
	if isNull || err != nil {
		return "", true, handleInvalidTimeError(sctx, err)
	}
	mon := arg.Month()
	if (arg.IsZero() && sctx.GetSessionVars().SQLMode.HasNoZeroDateMode()) || mon < 0 || mon > len(types.MonthNames) {
		return "", true, handleInvalidTimeError(sctx, types.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, arg.String()))
...
func (b *builtinDayNameSig) evalIndex(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	arg, isNull, err := b.args[0].EvalTime(sctx, row)
	if isNull || err != nil {
		return 0, isNull, err
	}
	if arg.InvalidZero() {
		return 0, true, handleInvalidTimeError(sctx, types.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, arg.String()))
	}
...
func (b *builtinDayNameSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	idx, isNull, err := b.evalIndex(sctx, row)
	if isNull || err != nil {
		return "", isNull, err
	}
...
func (b *builtinDayNameSig) evalReal(sctx sessionctx.Context, row chunk.Row) (float64, bool, error) {
	idx, isNull, err := b.evalIndex(sctx, row)
	if isNull || err != nil {
		return 0, isNull, err
	}
...
func (b *builtinDayNameSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	idx, isNull, err := b.evalIndex(sctx, row)
	if isNull || err != nil {
		return 0, isNull, err
	}
...
func (b *builtinDayOfMonthSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	arg, isNull, err := b.args[0].EvalTime(sctx, row)
	if isNull || err != nil {
		return 0, true, handleInvalidTimeError(sctx, err)
	}
...
func (b *builtinDayOfWeekSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	arg, isNull, err := b.args[0].EvalTime(sctx, row)
	if isNull || err != nil {
		return 0, true, handleInvalidTimeError(sctx, err)
	}
...
func (b *builtinDayOfYearSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	arg, isNull, err := b.args[0].EvalTime(sctx, row)
	if isNull || err != nil {
		return 0, isNull, handleInvalidTimeError(sctx, err)
	}
	if arg.InvalidZero() {
		return 0, true, handleInvalidTimeError(sctx, types.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, arg.String()))
	}
...
func (b *builtinWeekWithModeSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	date, isNull, err := b.args[0].EvalTime(sctx, row)
	...
	if date.IsZero() || date.InvalidZero() {
		return 0, true, handleInvalidTimeError(sctx, types.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, date.String()))
	}

	mode, isNull, err := b.args[1].EvalInt(sctx, row)
	if isNull || err != nil {
		return 0, isNull, err
	}
...
// see: https://dev.mysql.com/doc/refman/5.7/en/date-and-time-functions.html#function_week
func (b *builtinWeekWithoutModeSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	date, isNull, err := b.args[0].EvalTime(sctx, row)
	...
	mode := 0
	modeStr, ok := sctx.GetSessionVars().GetSystemVar(variable.DefaultWeekFormat)
	if ok && modeStr != "" {
		mode, err = strconv.Atoi(modeStr)
		if err != nil {
			return 0, true, handleInvalidTimeError(sctx, types.ErrInvalidWeekModeFormat.GenWithStackByArgs(modeStr))
		}
	}
...
func (b *builtinWeekDaySig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	date, isNull, err := b.args[0].EvalTime(sctx, row)
	if isNull || err != nil {
		return 0, true, handleInvalidTimeError(sctx, err)
	}
...
func (b *builtinWeekOfYearSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	date, isNull, err := b.args[0].EvalTime(sctx, row)
...
func (b *builtinYearSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	date, isNull, err := b.args[0].EvalTime(sctx, row)
...
func (b *builtinYearWeekWithModeSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	date, isNull, err := b.args[0].EvalTime(sctx, row)
	if isNull || err != nil {
		return 0, isNull, handleInvalidTimeError(sctx, err)
	}
...
// See https://dev.mysql.com/doc/refman/5.7/en/date-and-time-functions.html#function_yearweek
func (b *builtinYearWeekWithoutModeSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	date, isNull, err := b.args[0].EvalTime(sctx, row)
	if isNull || err != nil {
		return 0, true, handleInvalidTimeError(sctx, err)
	}
...
func (b *builtinFromUnixTime1ArgSig) evalTime(sctx sessionctx.Context, row chunk.Row) (res types.Time, isNull bool, err error) {
	unixTimeStamp, isNull, err := b.args[0].EvalDecimal(sctx, row)
	if err != nil || isNull {
		return res, isNull, err
	}
	return evalFromUnixTime(sctx, b.tp.GetDecimal(), unixTimeStamp)
}
...
// See https://dev.mysql.com/doc/refman/5.7/en/date-and-time-functions.html#function_from-unixtime
func (b *builtinFromUnixTime2ArgSig) evalString(sctx sessionctx.Context, row chunk.Row) (res string, isNull bool, err error) {
	format, isNull, err := b.args[1].EvalString(sctx, row)
	if isNull || err != nil {
		return "", true, err
	}
	unixTimeStamp, isNull, err := b.args[0].EvalDecimal(sctx, row)
	...
	t, isNull, err := evalFromUnixTime(sctx, b.tp.GetDecimal(), unixTimeStamp)
	if isNull || err != nil {
		return "", isNull, err
	}
...
func (b *builtinGetFormatSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	t, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return "", isNull, err
	}
	l, isNull, err := b.args[1].EvalString(sctx, row)
	if isNull || err != nil {
		return "", isNull, err
	}
...
func (b *builtinStrToDateDateSig) evalTime(sctx sessionctx.Context, row chunk.Row) (types.Time, bool, error) {
	date, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
	...
	if !succ {
		return types.ZeroTime, true, handleInvalidTimeError(sctx, types.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, t.String()))
	}
	if sctx.GetSessionVars().SQLMode.HasNoZeroDateMode() && (t.Year() == 0 || t.Month() == 0 || t.Day() == 0) {
		return types.ZeroTime, true, handleInvalidTimeError(sctx, types.ErrWrongValueForType.GenWithStackByArgs(types.DateTimeStr, date, ast.StrToDate))
	}
...
func (b *builtinStrToDateDatetimeSig) evalTime(sctx sessionctx.Context, row chunk.Row) (types.Time, bool, error) {
	date, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return types.ZeroTime, isNull, err
	}
...
func (b *builtinStrToDateDurationSig) evalDuration(sctx sessionctx.Context, row chunk.Row) (types.Duration, bool, error) {
	date, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
	...
	succ := t.StrToDate(sc, date, format)
	if !succ {
		return types.Duration{}, true, handleInvalidTimeError(sctx, types.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, t.String()))
	}
	t.SetFsp(b.tp.GetDecimal())
...
func (b *builtinSysDateWithFspSig) evalTime(sctx sessionctx.Context, row chunk.Row) (val types.Time, isNull bool, err error) {
	fsp, isNull, err := b.args[0].EvalInt(sctx, row)
	if isNull || err != nil {
		return types.ZeroTime, isNull, err
	}
...
// See https://dev.mysql.com/doc/refman/5.7/en/date-and-time-functions.html#function_sysdate
func (b *builtinSysDateWithoutFspSig) evalTime(sctx sessionctx.Context, row chunk.Row) (val types.Time, isNull bool, err error) {
...
func (b *builtinCurrentDateSig) evalTime(sctx sessionctx.Context, row chunk.Row) (val types.Time, isNull bool, err error) {
	tz := sctx.GetSessionVars().Location()
	nowTs, err := getStmtTimestamp(sctx)
	if err != nil {
		return types.ZeroTime, true, err
	}
...
func (b *builtinCurrentTime0ArgSig) evalDuration(sctx sessionctx.Context, row chunk.Row) (types.Duration, bool, error) {
	tz := sctx.GetSessionVars().Location()
	nowTs, err := getStmtTimestamp(sctx)
	if err != nil {
	...
	res, _, err := types.ParseDuration(sctx.GetSessionVars().StmtCtx, dur, types.MinFsp)
	if err != nil {
		return types.Duration{}, true, err
	}
	return res, false, nil
...
func (b *builtinCurrentTime1ArgSig) evalDuration(sctx sessionctx.Context, row chunk.Row) (types.Duration, bool, error) {
	fsp, _, err := b.args[0].EvalInt(sctx, row)
	if err != nil {
		return types.Duration{}, true, err
	}
	tz := sctx.GetSessionVars().Location()
	...
	res, _, err := types.ParseDuration(sctx.GetSessionVars().StmtCtx, dur, int(fsp))
	if err != nil {
		return types.Duration{}, true, err
	}
	return res, false, nil
...
func (b *builtinTimeSig) evalDuration(sctx sessionctx.Context, row chunk.Row) (res types.Duration, isNull bool, err error) {
	expr, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return res, isNull, err
	}
...
// See https://dev.mysql.com/doc/refman/5.7/en/date-and-time-literals.html
func (b *builtinTimeLiteralSig) evalDuration(sctx sessionctx.Context, row chunk.Row) (types.Duration, bool, error) {
...
func (b *builtinUTCDateSig) evalTime(sctx sessionctx.Context, row chunk.Row) (types.Time, bool, error) {
	nowTs, err := getStmtTimestamp(sctx)
	if err != nil {
		return types.ZeroTime, true, err
	}
...
func (b *builtinUTCTimestampWithArgSig) evalTime(sctx sessionctx.Context, row chunk.Row) (types.Time, bool, error) {
	num, isNull, err := b.args[0].EvalInt(sctx, row)
	if err != nil {
		return types.ZeroTime, true, err
	}
	...
	result, isNull, err := evalUTCTimestampWithFsp(sctx, int(num))
...
// See https://dev.mysql.com/doc/refman/5.7/en/date-and-time-functions.html#function_utc-timestamp
func (b *builtinUTCTimestampWithoutArgSig) evalTime(sctx sessionctx.Context, row chunk.Row) (types.Time, bool, error) {
	result, isNull, err := evalUTCTimestampWithFsp(sctx, 0)
	return result, isNull, err
}
...
func (b *builtinNowWithArgSig) evalTime(sctx sessionctx.Context, row chunk.Row) (types.Time, bool, error) {
	fsp, isNull, err := b.args[0].EvalInt(sctx, row)
	...
	result, isNull, err := evalNowWithFsp(sctx, int(fsp))
	return result, isNull, err
}
...
// evalTime evals NOW()
// see: https://dev.mysql.com/doc/refman/5.7/en/date-and-time-functions.html#function_now
func (b *builtinNowWithoutArgSig) evalTime(sctx sessionctx.Context, row chunk.Row) (types.Time, bool, error) {
	result, isNull, err := evalNowWithFsp(sctx, 0)
	return result, isNull, err
}
...
func (b *builtinExtractDatetimeFromStringSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	unit, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return 0, isNull, err
	}
	dtStr, isNull, err := b.args[1].EvalString(sctx, row)
	if isNull || err != nil {
		return 0, isNull, err
	}
	sc := sctx.GetSessionVars().StmtCtx
...
// See https://dev.mysql.com/doc/refman/5.7/en/date-and-time-functions.html#function_extract
func (b *builtinExtractDatetimeSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	unit, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return 0, isNull, err
	}
	dt, isNull, err := b.args[1].EvalTime(sctx, row)
	if isNull || err != nil {
		return 0, isNull, err
	}
	res, err := types.ExtractDatetimeNum(&dt, unit)
...
// See https://dev.mysql.com/doc/refman/5.7/en/date-and-time-functions.html#function_extract
func (b *builtinExtractDurationSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	unit, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return 0, isNull, err
	}
	dur, isNull, err := b.args[1].EvalDuration(sctx, row)
	if isNull || err != nil {
		return 0, isNull, err
	}
	res, err := types.ExtractDurationNum(&dur, unit)
...
func (du *baseDateArithmetical) vecGetDateFromInt(b *baseBuiltinFunc, sctx sessionctx.Context, input *chunk.Chunk, unit string, result *chunk.Column) error {
	n := input.NumRows()
	...
	for i := 0; i < n; i++ {
		...
		if err != nil {
			err = handleInvalidTimeError(sctx, err)
			if err != nil {
				return err
			}
...
func (du *baseDateArithmetical) vecGetDateFromReal(b *baseBuiltinFunc, sctx sessionctx.Context, input *chunk.Chunk, unit string, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
	...
	defer b.bufAllocator.put(buf)
	if err := b.args[0].VecEvalReal(sctx, input, buf); err != nil {
		return err
	}
...
func (du *baseDateArithmetical) vecGetDateFromDecimal(b *baseBuiltinFunc, sctx sessionctx.Context, input *chunk.Chunk, unit string, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
	...
	defer b.bufAllocator.put(buf)
	if err := b.args[0].VecEvalDecimal(sctx, input, buf); err != nil {
		return err
	}
...
func (du *baseDateArithmetical) vecGetDateFromString(b *baseBuiltinFunc, sctx sessionctx.Context, input *chunk.Chunk, unit string, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
	...
	for i := 0; i < n; i++ {
		...
		if err != nil {
			...
			result.SetNull(i, true)
		} else if sctx.GetSessionVars().SQLMode.HasNoZeroDateMode() && (date.Year() == 0 || date.Month() == 0 || date.Day() == 0) {
			err = handleInvalidTimeError(sctx, types.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, dateStr))
			if err != nil {
				return err
			}
			result.SetNull(i, true)
...
func (du *baseDateArithmetical) vecGetDateFromDatetime(b *baseBuiltinFunc, sctx sessionctx.Context, input *chunk.Chunk, unit string, result *chunk.Column) error {
	n := input.NumRows()
	result.ResizeTime(n, false)
	if err := b.args[0].VecEvalTime(sctx, input, result); err != nil {
		return err
	}
...
func (du *baseDateArithmetical) vecGetIntervalFromString(b *baseBuiltinFunc, sctx sessionctx.Context, input *chunk.Chunk, unit string, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
	...
	defer b.bufAllocator.put(buf)
	if err := b.args[1].VecEvalString(sctx, input, buf); err != nil {
		return err
	}
...
func (du *baseDateArithmetical) vecGetIntervalFromDecimal(b *baseBuiltinFunc, sctx sessionctx.Context, input *chunk.Chunk, unit string, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
	...
	defer b.bufAllocator.put(buf)
	if err := b.args[1].VecEvalDecimal(sctx, input, buf); err != nil {
		return err
	}
	...
	default:
		// YEAR, QUARTER, MONTH, WEEK, DAY, HOUR, MINUTE, MICROSECOND
		castExpr := WrapWithCastAsString(sctx, WrapWithCastAsInt(sctx, b.args[1]))
		amendInterval = func(_ string, row *chunk.Row) (string, bool, error) {
			interval, isNull, err := castExpr.EvalString(sctx, *row)
...
func (du *baseDateArithmetical) vecGetIntervalFromInt(b *baseBuiltinFunc, sctx sessionctx.Context, input *chunk.Chunk, unit string, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
	...
	defer b.bufAllocator.put(buf)
	if err := b.args[1].VecEvalInt(sctx, input, buf); err != nil {
		return err
	}
...
func (du *baseDateArithmetical) vecGetIntervalFromReal(b *baseBuiltinFunc, sctx sessionctx.Context, input *chunk.Chunk, unit string, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
	...
	defer b.bufAllocator.put(buf)
	if err := b.args[1].VecEvalReal(sctx, input, buf); err != nil {
		return err
	}
...
func (b *builtinAddSubDateAsStringSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	unit, isNull, err := b.args[2].EvalString(sctx, row)
	if isNull || err != nil {
		return types.ZeroTime.String(), true, err
	}

	date, isNull, err := b.getDate(&b.baseDateArithmetical, sctx, b.args, row, unit)
	if isNull || err != nil {
		return types.ZeroTime.String(), true, err
	}
	if date.InvalidZero() {
		return types.ZeroTime.String(), true, handleInvalidTimeError(sctx, types.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, date.String()))
	}

	interval, isNull, err := b.getInterval(&b.baseDateArithmetical, sctx, b.args, row, unit)
	if isNull || err != nil {
		return types.ZeroTime.String(), true, err
	}

	result, isNull, err := b.timeOp(&b.baseDateArithmetical, sctx, date, interval, unit, b.tp.GetDecimal())
...
func (b *builtinAddSubDateDatetimeAnySig) evalTime(sctx sessionctx.Context, row chunk.Row) (types.Time, bool, error) {
	unit, isNull, err := b.args[2].EvalString(sctx, row)
	if isNull || err != nil {
		return types.ZeroTime, true, err
	}

	date, isNull, err := b.getDateFromDatetime(sctx, b.args, row, unit)
	if isNull || err != nil {
		return types.ZeroTime, true, err
	}
...
func (b *builtinAddSubDateDurationAnySig) evalTime(sctx sessionctx.Context, row chunk.Row) (types.Time, bool, error) {
	unit, isNull, err := b.args[2].EvalString(sctx, row)
	if isNull || err != nil {
		return types.ZeroTime, true, err
	}

	d, isNull, err := b.args[0].EvalDuration(sctx, row)
	if isNull || err != nil {
		return types.ZeroTime, true, err
	}
	...
	result, isNull, err := b.timeOp(&b.baseDateArithmetical, sctx, t, interval, unit, b.tp.GetDecimal())
	return result, isNull || err != nil, err
}

func (b *builtinAddSubDateDurationAnySig) evalDuration(sctx sessionctx.Context, row chunk.Row) (types.Duration, bool, error) {
	unit, isNull, err := b.args[2].EvalString(sctx, row)
	if isNull || err != nil {
		return types.ZeroDuration, true, err
	}
	...
	result, isNull, err := b.durationOp(&b.baseDateArithmetical, sctx, dur, interval, unit, b.tp.GetDecimal())
	return result, isNull || err != nil, err
}
...
func (b *builtinTimestampDiffSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	unit, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return 0, isNull, err
	}
	lhs, isNull, err := b.args[1].EvalTime(sctx, row)
	if isNull || err != nil {
		return 0, isNull, handleInvalidTimeError(sctx, err)
	}
	rhs, isNull, err := b.args[2].EvalTime(sctx, row)
	if isNull || err != nil {
		return 0, isNull, handleInvalidTimeError(sctx, err)
	}
	if invalidLHS, invalidRHS := lhs.InvalidZero(), rhs.InvalidZero(); invalidLHS || invalidRHS {
...
func (b *builtinUnixTimestampCurrentSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	nowTs, err := getStmtTimestamp(sctx)
	if err != nil {
		return 0, true, err
	}
...
// See https://dev.mysql.com/doc/refman/5.7/en/date-and-time-functions.html#function_unix-timestamp
func (b *builtinUnixTimestampIntSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	return b.evalIntWithCtx(sctx, row)
}
...
// See https://dev.mysql.com/doc/refman/5.7/en/date-and-time-functions.html#function_unix-timestamp
func (b *builtinUnixTimestampDecSig) evalDecimal(sctx sessionctx.Context, row chunk.Row) (*types.MyDecimal, bool, error) {
	val, isNull, err := b.args[0].EvalTime(sctx, row)
	if isNull || err != nil {
		// Return 0 for invalid date time.
		return new(types.MyDecimal), isNull, nil
	}
	t, err := val.GoTime(getTimeZone(sctx))
	if err != nil {
...
func (b *builtinTimestamp1ArgSig) evalTime(sctx sessionctx.Context, row chunk.Row) (types.Time, bool, error) {
	s, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return types.ZeroTime, isNull, err
	}
	var tm types.Time
...
// See https://dev.mysql.com/doc/refman/5.5/en/date-and-time-functions.html#function_timestamp
func (b *builtinTimestamp2ArgsSig) evalTime(sctx sessionctx.Context, row chunk.Row) (types.Time, bool, error) {
	arg0, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return types.ZeroTime, isNull, err
	}
	var tm types.Time
	...
	arg1, isNull, err := b.args[1].EvalString(sctx, row)
	if isNull || err != nil {
		return types.ZeroTime, isNull, err
	}
	if !isDuration(arg1) {
...
// See https://dev.mysql.com/doc/refman/5.7/en/date-and-time-literals.html
func (b *builtinTimestampLiteralSig) evalTime(sctx sessionctx.Context, row chunk.Row) (types.Time, bool, error) {
...
func (b *builtinAddTimeDateTimeNullSig) evalTime(sctx sessionctx.Context, row chunk.Row) (types.Time, bool, error) {
...
// See https://dev.mysql.com/doc/refman/5.7/en/date-and-time-functions.html#function_addtime
func (b *builtinAddDatetimeAndDurationSig) evalTime(sctx sessionctx.Context, row chunk.Row) (types.Time, bool, error) {
	arg0, isNull, err := b.args[0].EvalTime(sctx, row)
	if isNull || err != nil {
		return types.ZeroDatetime, isNull, err
	}
	arg1, isNull, err := b.args[1].EvalDuration(sctx, row)
	if isNull || err != nil {
		return types.ZeroDatetime, isNull, err
	}
	result, err := arg0.Add(sctx.GetSessionVars().StmtCtx, arg1)
	return result, err != nil, err
}
...
// See https://dev.mysql.com/doc/refman/5.7/en/date-and-time-functions.html#function_addtime
func (b *builtinAddDatetimeAndStringSig) evalTime(sctx sessionctx.Context, row chunk.Row) (types.Time, bool, error) {
	arg0, isNull, err := b.args[0].EvalTime(sctx, row)
	if isNull || err != nil {
		return types.ZeroDatetime, isNull, err
	}
	s, isNull, err := b.args[1].EvalString(sctx, row)
	if isNull || err != nil {
		return types.ZeroDatetime, isNull, err
	}
	if !isDuration(s) {
...
// See https://dev.mysql.com/doc/refman/5.7/en/date-and-time-functions.html#function_addtime
func (b *builtinAddTimeDurationNullSig) evalDuration(sctx sessionctx.Context, row chunk.Row) (types.Duration, bool, error) {
...
// See https://dev.mysql.com/doc/refman/5.7/en/date-and-time-functions.html#function_addtime
func (b *builtinAddDurationAndDurationSig) evalDuration(sctx sessionctx.Context, row chunk.Row) (types.Duration, bool, error) {
	arg0, isNull, err := b.args[0].EvalDuration(sctx, row)
	if isNull || err != nil {
...
// See https://dev.mysql.com/doc/refman/5.7/en/date-and-time-functions.html#function_addtime
func (b *builtinAddDurationAndStringSig) evalDuration(sctx sessionctx.Context, row chunk.Row) (types.Duration, bool, error) {
	arg0, isNull, err := b.args[0].EvalDuration(sctx, row)
	if isNull || err != nil {
		return types.ZeroDuration, isNull, err
	}
...
// See https://dev.mysql.com/doc/refman/5.7/en/date-and-time-functions.html#function_addtime
func (b *builtinAddTimeStringNullSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	return "", true, nil
}
...
// See https://dev.mysql.com/doc/refman/5.7/en/date-and-time-functions.html#function_addtime
func (b *builtinAddStringAndDurationSig) evalString(sctx sessionctx.Context, row chunk.Row) (result string, isNull bool, err error) {
	var (
		arg0 string
		arg1 types.Duration
	)
...
// See https://dev.mysql.com/doc/refman/5.7/en/date-and-time-functions.html#function_addtime
func (b *builtinAddStringAndStringSig) evalString(sctx sessionctx.Context, row chunk.Row) (result string, isNull bool, err error) {
	var (
		arg0, arg1Str string
		arg1          types.Duration
	)
	...
	arg1Str, isNull, err = b.args[1].EvalString(sctx, row)
	if isNull || err != nil {
		return "", isNull, err
	}
	sc := sctx.GetSessionVars().StmtCtx
...
// evalString evals a builtinAddDurationAndDurationSig.
// See https://dev.mysql.com/doc/refman/5.7/en/date-and-time-functions.html#function_addtime
func (b *builtinAddDateAndDurationSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	arg0, isNull, err := b.args[0].EvalDuration(sctx, row)
	if isNull || err != nil {
		return "", isNull, err
	}
...
// See https://dev.mysql.com/doc/refman/5.7/en/date-and-time-functions.html#function_addtime
func (b *builtinAddDateAndStringSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	arg0, isNull, err := b.args[0].EvalDuration(sctx, row)
	if isNull || err != nil {
		return "", isNull, err
	}
...
func (b *builtinConvertTzSig) evalTime(sctx sessionctx.Context, row chunk.Row) (types.Time, bool, error) {
	dt, isNull, err := b.args[0].EvalTime(sctx, row)
	if isNull || err != nil {
		return types.ZeroTime, true, nil
	}
	if dt.InvalidZero() {
		return types.ZeroTime, true, handleInvalidTimeError(sctx, types.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, dt.String()))
	}
	fromTzStr, isNull, err := b.args[1].EvalString(sctx, row)
	if isNull || err != nil {
		return types.ZeroTime, true, nil
	}

	toTzStr, isNull, err := b.args[2].EvalString(sctx, row)
	if isNull || err != nil {
		return types.ZeroTime, true, nil
	}
...
func (b *builtinMakeDateSig) evalTime(sctx sessionctx.Context, row chunk.Row) (d types.Time, isNull bool, err error) {
	args := b.getArgs()
	var year, dayOfYear int64
	year, isNull, err = args[0].EvalInt(sctx, row)
	if isNull || err != nil {
		return d, true, err
	}
	dayOfYear, isNull, err = args[1].EvalInt(sctx, row)
	if isNull || err != nil {
		return d, true, err
	}
	...
	if retTimestamp == 0 {
		return d, true, handleInvalidTimeError(sctx, types.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, startTime.String()))
	}
...
func (b *builtinMakeTimeSig) makeTime(sctx sessionctx.Context, hour int64, minute int64, second float64, hourUnsignedFlag bool) (types.Duration, error) {
	var overflow bool
	...
	fsp := b.tp.GetDecimal()
	d, _, err := types.ParseDuration(sctx.GetSessionVars().StmtCtx, fmt.Sprintf("%02d:%02d:%v", hour, minute, second), fsp)
	return d, err
}
...
func (b *builtinMakeTimeSig) evalDuration(sctx sessionctx.Context, row chunk.Row) (types.Duration, bool, error) {
	dur := types.ZeroDuration
	dur.Fsp = types.MaxFsp
	hour, isNull, err := b.args[0].EvalInt(sctx, row)
	if isNull || err != nil {
		return dur, isNull, err
	}
	minute, isNull, err := b.args[1].EvalInt(sctx, row)
	if isNull || err != nil {
		return dur, isNull, err
	}
	...
	second, isNull, err := b.args[2].EvalReal(sctx, row)
	if isNull || err != nil {
		return dur, isNull, err
	}
	...
	dur, err = b.makeTime(sctx, hour, minute, second, mysql.HasUnsignedFlag(b.args[0].GetType().GetFlag()))
	if err != nil {
		return dur, true, err
	}
...
func (b *builtinPeriodAddSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	p, isNull, err := b.args[0].EvalInt(sctx, row)
	if isNull || err != nil {
		return 0, true, err
	}

	n, isNull, err := b.args[1].EvalInt(sctx, row)
	if isNull || err != nil {
		return 0, true, err
	}
...
func (b *builtinPeriodDiffSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	p1, isNull, err := b.args[0].EvalInt(sctx, row)
	if isNull || err != nil {
		return 0, isNull, err
	}

	p2, isNull, err := b.args[1].EvalInt(sctx, row)
	if isNull || err != nil {
		return 0, isNull, err
	}
...
func (b *builtinQuarterSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	date, isNull, err := b.args[0].EvalTime(sctx, row)
	if isNull || err != nil {
		return 0, true, handleInvalidTimeError(sctx, err)
	}
...
func (b *builtinSecToTimeSig) evalDuration(sctx sessionctx.Context, row chunk.Row) (types.Duration, bool, error) {
	secondsFloat, isNull, err := b.args[0].EvalReal(sctx, row)
	if isNull || err != nil {
		return types.Duration{}, isNull, err
	}
	var (
	...
	if hour > 838 {
		...
		second = 59
		demical = 0
		err = sctx.GetSessionVars().StmtCtx.HandleTruncate(errTruncatedWrongValue.GenWithStackByArgs("time", strconv.FormatFloat(secondsFloat, 'f', -1, 64)))
		if err != nil {
	...
	dur, _, err = types.ParseDuration(sctx.GetSessionVars().StmtCtx, fmt.Sprintf("%s%02d:%02d:%s", negative, hour, minute, strconv.FormatFloat(secondDemical, 'f', -1, 64)), b.tp.GetDecimal())
	if err != nil {
		return types.Duration{}, err != nil, err
	}
	return dur, false, nil
...
func (b *builtinSubDatetimeAndDurationSig) evalTime(sctx sessionctx.Context, row chunk.Row) (types.Time, bool, error) {
	arg0, isNull, err := b.args[0].EvalTime(sctx, row)
	if isNull || err != nil {
		return types.ZeroDatetime, isNull, err
	}
...
// See https://dev.mysql.com/doc/refman/5.7/en/date-and-time-functions.html#function_subtime
func (b *builtinSubDatetimeAndStringSig) evalTime(sctx sessionctx.Context, row chunk.Row) (types.Time, bool, error) {
	arg0, isNull, err := b.args[0].EvalTime(sctx, row)
	if isNull || err != nil {
		return types.ZeroDatetime, isNull, err
	}
...
// See https://dev.mysql.com/doc/refman/5.7/en/date-and-time-functions.html#function_subtime
func (b *builtinSubTimeDateTimeNullSig) evalTime(sctx sessionctx.Context, row chunk.Row) (types.Time, bool, error) {
	return types.ZeroDatetime, true, nil
}
...
// See https://dev.mysql.com/doc/refman/5.7/en/date-and-time-functions.html#function_subtime
func (b *builtinSubStringAndDurationSig) evalString(sctx sessionctx.Context, row chunk.Row) (result string, isNull bool, err error) {
	var (
		arg0 string
		arg1 types.Duration
	)
...
// See https://dev.mysql.com/doc/refman/5.7/en/date-and-time-functions.html#function_subtime
func (b *builtinSubStringAndStringSig) evalString(sctx sessionctx.Context, row chunk.Row) (result string, isNull bool, err error) {
	var (
		s, arg0 string
		arg1    types.Duration
	)
...
// See https://dev.mysql.com/doc/refman/5.7/en/date-and-time-functions.html#function_subtime
func (b *builtinSubTimeStringNullSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	return "", true, nil
}
...
// See https://dev.mysql.com/doc/refman/5.7/en/date-and-time-functions.html#function_subtime
func (b *builtinSubDurationAndDurationSig) evalDuration(sctx sessionctx.Context, row chunk.Row) (types.Duration, bool, error) {
	arg0, isNull, err := b.args[0].EvalDuration(sctx, row)
	if isNull || err != nil {
		return types.ZeroDuration, isNull, err
	}
...
// See https://dev.mysql.com/doc/refman/5.7/en/date-and-time-functions.html#function_subtime
func (b *builtinSubDurationAndStringSig) evalDuration(sctx sessionctx.Context, row chunk.Row) (types.Duration, bool, error) {
	arg0, isNull, err := b.args[0].EvalDuration(sctx, row)
	if isNull || err != nil {
		return types.ZeroDuration, isNull, err
	}
...
// See https://dev.mysql.com/doc/refman/5.7/en/date-and-time-functions.html#function_subtime
func (b *builtinSubTimeDurationNullSig) evalDuration(sctx sessionctx.Context, row chunk.Row) (types.Duration, bool, error) {
	return types.ZeroDuration, true, nil
}
...
// See https://dev.mysql.com/doc/refman/5.7/en/date-and-time-functions.html#function_subtime
func (b *builtinSubDateAndDurationSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	arg0, isNull, err := b.args[0].EvalDuration(sctx, row)
	if isNull || err != nil {
		return "", isNull, err
	}
...
// See https://dev.mysql.com/doc/refman/5.7/en/date-and-time-functions.html#function_subtime
func (b *builtinSubDateAndStringSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	arg0, isNull, err := b.args[0].EvalDuration(sctx, row)
	if isNull || err != nil {
		return "", isNull, err
	}
...
func (b *builtinTimeFormatSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	dur, isNull, err := b.args[0].EvalDuration(sctx, row)
	...
	res, err := b.formatTime(sctx, dur, formatMask)
	return res, isNull, err
}
...
func (b *builtinTimeToSecSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	duration, isNull, err := b.args[0].EvalDuration(sctx, row)
	if isNull || err != nil {
		return 0, isNull, err
	}
	var sign int
...
func (b *builtinTimestampAddSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	unit, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return "", isNull, err
	}
	v, isNull, err := b.args[1].EvalReal(sctx, row)
	if isNull || err != nil {
		return "", isNull, err
	}
	arg, isNull, err := b.args[2].EvalTime(sctx, row)
	if isNull || err != nil {
		return "", isNull, err
	}
	...
	if overflow {
		return "", true, handleInvalidTimeError(sctx, types.ErrDatetimeFunctionOverflow.GenWithStackByArgs("datetime"))
	}
	...
	if err = r.Check(sctx.GetSessionVars().StmtCtx); err != nil {
		return "", true, handleInvalidTimeError(sctx, err)
	}
...
func (b *builtinToDaysSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	arg, isNull, err := b.args[0].EvalTime(sctx, row)
...
func (b *builtinToSecondsSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	arg, isNull, err := b.args[0].EvalTime(sctx, row)
	if isNull || err != nil {
		return 0, true, handleInvalidTimeError(sctx, err)
	}
...
func (b *builtinUTCTimeWithoutArgSig) evalDuration(sctx sessionctx.Context, row chunk.Row) (types.Duration, bool, error) {
	nowTs, err := getStmtTimestamp(sctx)
	if err != nil {
		return types.Duration{}, true, err
	}
	v, _, err := types.ParseDuration(sctx.GetSessionVars().StmtCtx, nowTs.UTC().Format(types.TimeFormat), 0)
	return v, false, err
}
...
// See https://dev.mysql.com/doc/refman/5.7/en/date-and-time-functions.html#function_utc-time
func (b *builtinUTCTimeWithArgSig) evalDuration(sctx sessionctx.Context, row chunk.Row) (types.Duration, bool, error) {
	fsp, isNull, err := b.args[0].EvalInt(sctx, row)
	if isNull || err != nil {
		return types.Duration{}, isNull, err
	}
	...
	v, _, err := types.ParseDuration(sctx.GetSessionVars().StmtCtx, nowTs.UTC().Format(types.TimeFSPFormat), int(fsp))
	return v, false, err
}
...
func (b *builtinLastDaySig) evalTime(sctx sessionctx.Context, row chunk.Row) (types.Time, bool, error) {
	arg, isNull, err := b.args[0].EvalTime(sctx, row)
	if isNull || err != nil {
		return types.ZeroTime, true, handleInvalidTimeError(sctx, err)
	}
	...
	if tm.Month() == 0 || (tm.Day() == 0 && sctx.GetSessionVars().SQLMode.HasNoZeroDateMode()) {
		return types.ZeroTime, true, handleInvalidTimeError(sctx, types.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, arg.String()))
	}
...
func (b *builtinTidbParseTsoSig) evalTime(sctx sessionctx.Context, row chunk.Row) (types.Time, bool, error) {
	arg, isNull, err := b.args[0].EvalInt(sctx, row)
	...
	err = result.ConvertTimeZone(time.Local, sctx.GetSessionVars().Location())
	if err != nil {
		return types.ZeroTime, true, err
	}
	return result, false, nil
...
func (b *builtinTidbParseTsoLogicalSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	arg, isNull, err := b.args[0].EvalInt(sctx, row)
	if isNull || err != nil || arg <= 0 {
		return 0, true, err
	}
...
func (b *builtinTiDBBoundedStalenessSig) evalTime(sctx sessionctx.Context, row chunk.Row) (types.Time, bool, error) {
	leftTime, isNull, err := b.args[0].EvalTime(sctx, row)
	if isNull || err != nil {
		return types.ZeroTime, true, handleInvalidTimeError(sctx, err)
	}
	rightTime, isNull, err := b.args[1].EvalTime(sctx, row)
	if isNull || err != nil {
		return types.ZeroTime, true, handleInvalidTimeError(sctx, err)
	}
	if invalidLeftTime, invalidRightTime := leftTime.InvalidZero(), rightTime.InvalidZero(); invalidLeftTime || invalidRightTime {
		if invalidLeftTime {
			err = handleInvalidTimeError(sctx, types.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, leftTime.String()))
		}
		if invalidRightTime {
			err = handleInvalidTimeError(sctx, types.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, rightTime.String()))
		}
		return types.ZeroTime, true, err
	}
	timeZone := getTimeZone(sctx)
	...
	return types.NewTime(types.FromGoTime(calAppropriateTime(minTime, maxTime, getMinSafeTime(sctx, timeZone))), mysql.TypeDatetime, 3), false, nil
}
...
func (b *builtinTiDBCurrentTsoSig) evalInt(sctx sessionctx.Context, row chunk.Row) (val int64, isNull bool, err error) {
	tso, _ := sctx.GetSessionVars().GetSessionOrGlobalSystemVar(context.Background(), "tidb_current_ts")
