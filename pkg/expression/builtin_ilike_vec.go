...
func (b *builtinIlikeSig) getEscape(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) (int64, bool, error) {
	rowNum := input.NumRows()
	escape := int64('\\')

	if !b.args[2].ConstItem(sctx.GetSessionVars().StmtCtx) {
		return escape, true, errors.Errorf("escape should be const")
	}

	escape, isConstNull, err := b.args[2].EvalInt(sctx, chunk.Row{})
	if isConstNull {
...
func (b *builtinIlikeSig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	rowNum := input.NumRows()
	params := make([]*funcParam, 0, 3)
	defer releaseBuffers(&b.baseBuiltinFunc, params)

	for i := 0; i < 2; i++ {
		param, isConstNull, err := buildStringParam(sctx, &b.baseBuiltinFunc, i, input, false)
		if err != nil {
	...
	escape, ret, err := b.getEscape(sctx, input, result)
	if err != nil || ret {
		return err
	}
