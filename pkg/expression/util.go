...
func ColumnSubstitute(sctx sessionctx.Context, expr Expression, schema *Schema, newExprs []Expression) Expression {
	_, _, resExpr := ColumnSubstituteImpl(sctx, expr, schema, newExprs, false)
	return resExpr
}
...
func ColumnSubstituteAll(sctx sessionctx.Context, expr Expression, schema *Schema, newExprs []Expression) (bool, Expression) {
	_, hasFail, resExpr := ColumnSubstituteImpl(sctx, expr, schema, newExprs, true)
...
func ColumnSubstituteImpl(sctx sessionctx.Context, expr Expression, schema *Schema, newExprs []Expression, fail1Return bool) (bool, bool, Expression) {
	switch v := expr.(type) {
	case *Column:
	...
	case *ScalarFunction:
		...
		if v.FuncName.L == ast.Cast || v.FuncName.L == ast.Grouping {
			...
			substituted, hasFail, newArg = ColumnSubstituteImpl(sctx, v.GetArgs()[0], schema, newExprs, fail1Return)
			...
			if substituted {
				...
				if v.FuncName.L == ast.Cast {
					e = BuildCastFunction(sctx, newArg, v.RetType)
				} else {
		...
		oldCollEt, err := CheckAndDeriveCollationFromExprs(sctx, v.FuncName.L, v.RetType.EvalType(), v.GetArgs()...)
		if err != nil {
		...
		for idx, arg := range v.GetArgs() {
			changed, failed, newFuncExpr := ColumnSubstituteImpl(sctx, arg, schema, newExprs, fail1Return)
			...
			if collate.NewCollationEnabled() && changed {
				...
				newCollEt, err := CheckAndDeriveCollationFromExprs(sctx, v.FuncName.L, v.RetType.EvalType(), tmpArgForCollCheck...)
				if err != nil {
					logutil.BgLogger().Error("Unexpected error happened during ColumnSubstitution", zap.Stack("stack"))
		...
		if substituted {
			return true, hasFail, NewFunctionInternal(sctx, v.FuncName.L, v.RetType, refExprArr.Result()...)
		}
	}
...
func SubstituteCorCol2Constant(sctx sessionctx.Context, expr Expression) (Expression, error) {
	switch x := expr.(type) {
	case *ScalarFunction:
		allConstant := true
		newArgs := make([]Expression, 0, len(x.GetArgs()))
		for _, arg := range x.GetArgs() {
			newArg, err := SubstituteCorCol2Constant(sctx, arg)
			if err != nil {
				return nil, err
			}
		...
		if x.FuncName.L == ast.Cast {
			newSf = BuildCastFunction(sctx, newArgs[0], x.RetType)
		} else if x.FuncName.L == ast.Grouping {
			newSf = x.Clone()
			newSf.(*ScalarFunction).GetArgs()[0] = newArgs[0]
		} else {
			newSf, err = NewFunction(sctx, x.FuncName.L, x.GetType(), newArgs...)
		}
		return newSf, err
	case *CorrelatedColumn:
...
func GetUint64FromConstant(sctx sessionctx.Context, expr Expression) (uint64, bool, bool) {
	...
	if con.ParamMarker != nil {
		...
		var err error
		dt, err = con.DeferredExpr.Eval(sctx, chunk.Row{})
		if err != nil {
