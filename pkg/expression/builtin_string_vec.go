...
func (b *builtinLowerSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	// if error is not nil return error, or builtinLowerSig is for binary strings (do nothing)
	return b.args[0].VecEvalString(sctx, input, result)
}
...
func (b *builtinLowerUTF8Sig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
...
func (b *builtinRepeatSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
	...
	for i := 0; i < n; i++ {
		...
		if uint64(byteLength)*uint64(num) > b.maxAllowedPacket {
			if err := handleAllowedPacketOverflowed(sctx, "repeat", b.maxAllowedPacket); err != nil {
				return err
			}
			result.AppendNull()
			continue
...
func (b *builtinStringIsNullSig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinUpperUTF8Sig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinUpperSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	return b.args[0].VecEvalString(sctx, input, result)
}
...
func (b *builtinLeftUTF8Sig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinRightUTF8Sig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinSpaceSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
	...
	for i := 0; i < n; i++ {
		...
		if uint64(num) > b.maxAllowedPacket {
			if err := handleAllowedPacketOverflowed(sctx, "space", b.maxAllowedPacket); err != nil {
				return err
			}
...
func (b *builtinReverseUTF8Sig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	if err := b.args[0].VecEvalString(sctx, input, result); err != nil {
		return err
	}
...
func (b *builtinConcatSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
	...
	for j := 0; j < len(b.args); j++ {
		if err := b.args[j].VecEvalString(sctx, input, buf); err != nil {
			return err
		}
		for i := 0; i < n; i++ {
			if isNulls[i] {
			...
			if uint64(len(strs[i])+len(byteBuf)) > b.maxAllowedPacket {
				if err := handleAllowedPacketOverflowed(sctx, "concat", b.maxAllowedPacket); err != nil {
					return err
				}
...
func (b *builtinLocate3ArgsUTF8Sig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
	...
	if err := b.args[2].VecEvalInt(sctx, input, result); err != nil {
		return err
	}
...
func (b *builtinHexStrArgSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
...
func (b *builtinLTrimSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinQuoteSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinInsertSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	...
	if err := b.args[0].VecEvalString(sctx, input, str); err != nil {
		return err
	}
	...
	if err := b.args[1].VecEvalInt(sctx, input, pos); err != nil {
		return err
	}
	...
	if err := b.args[2].VecEvalInt(sctx, input, length); err != nil {
		return err
	}
	...
	if err := b.args[3].VecEvalString(sctx, input, newstr); err != nil {
		return err
	}
	posIs := pos.Int64s()
	...
	for i := 0; i < n; i++ {
		...
		if uint64(strLength-lengthI+int64(len(newstrI))) > b.maxAllowedPacket {
			if err := handleAllowedPacketOverflowed(sctx, "insert", b.maxAllowedPacket); err != nil {
				return err
			}
...
func (b *builtinConcatWSSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	argsLen := len(b.args)
	...
	for i := 0; i < argsLen; i++ {
		...
		if err := b.args[i].VecEvalString(sctx, input, bufs[i]); err != nil {
			return err
		}
	}
	...
	for j := 1; j < argsLen; j++ {
		for i := 0; i < n; i++ {
			...
			if uint64(targetLengths[i]) > b.maxAllowedPacket {
				if err := handleAllowedPacketOverflowed(sctx, "concat_ws", b.maxAllowedPacket); err != nil {
					return err
				}
...
func (b *builtinConvertSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	...
	if err := b.args[0].VecEvalString(sctx, input, expr); err != nil {
		return err
	}
...
func (b *builtinSubstringIndexSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
	...
	defer b.bufAllocator.put(buf2)
	if err := b.args[2].VecEvalInt(sctx, input, buf2); err != nil {
		return err
	}
...
func (b *builtinUnHexSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinExportSet3ArgSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	...
	if err := b.args[0].VecEvalInt(sctx, input, bits); err != nil {
		return err
	}
	...
	if err := b.args[1].VecEvalString(sctx, input, on); err != nil {
		return err
	}
	...
	if err := b.args[2].VecEvalString(sctx, input, off); err != nil {
		return err
	}
	result.ReserveString(n)
	i64s := bits.Int64s()
...
func (b *builtinASCIISig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinLpadSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	...
	if err := b.args[1].VecEvalInt(sctx, input, lenBuf); err != nil {
		return err
	}
	...
	if err := b.args[2].VecEvalString(sctx, input, padBuf); err != nil {
		return err
	}
	...
	for i := 0; i < n; i++ {
		...
		if uint64(targetLength) > b.maxAllowedPacket {
			if err := handleAllowedPacketOverflowed(sctx, "lpad", b.maxAllowedPacket); err != nil {
				return err
			}
...
// vecEvalString evals LPAD(str,len,padstr).
// See https://dev.mysql.com/doc/refman/5.7/en/string-functions.html#function_lpad
func (b *builtinLpadUTF8Sig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
	...
	defer b.bufAllocator.put(buf2)
	if err := b.args[2].VecEvalString(sctx, input, buf2); err != nil {
		return err
	}
...
func (b *builtinFindInSetSig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	str, err := b.bufAllocator.get()
	if err != nil {
		return err
	...
	if err := b.args[1].VecEvalString(sctx, input, strlist); err != nil {
		return err
	}
	result.ResizeInt64(n, false)
...
func (b *builtinLeftSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinReverseSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	if err := b.args[0].VecEvalString(sctx, input, result); err != nil {
		return err
	}
	for i := 0; i < input.NumRows(); i++ {
...
func (b *builtinRTrimSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinStrcmpSig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	...
	if err := b.args[0].VecEvalString(sctx, input, leftBuf); err != nil {
		return err
	}
	...
	if err := b.args[1].VecEvalString(sctx, input, rightBuf); err != nil {
		return err
	}
	result.ResizeInt64(n, false)
...
func (b *builtinLocate2ArgsSig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf0, err := b.bufAllocator.get()
	if err != nil {
		return err
...
// See https://dev.mysql.com/doc/refman/5.7/en/string-functions.html#function_locate
func (b *builtinLocate3ArgsSig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf0, err := b.bufAllocator.get()
	if err != nil {
		return err
...
// See https://dev.mysql.com/doc/refman/5.7/en/string-functions.html#function_export-set
func (b *builtinExportSet4ArgSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	bits, err := b.bufAllocator.get()
	if err != nil {
		return err
	...
	if err := b.args[3].VecEvalString(sctx, input, separator); err != nil {
		return err
	}
	result.ReserveString(n)
	i64s := bits.Int64s()
...
func (b *builtinRpadSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	strBuf, err := b.bufAllocator.get()
	if err != nil {
		return err
	...
	for i := 0; i < n; i++ {
		...
		targetLength := int(i64s[i])
		if uint64(targetLength) > b.maxAllowedPacket {
			if err := handleAllowedPacketOverflowed(sctx, "rpad", b.maxAllowedPacket); err != nil {
				return err
			}
...
func (b *builtinFormatWithLocaleSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	...
	if err := b.args[1].VecEvalInt(sctx, input, dBuf); err != nil {
		return err
	}
	dInt64s := dBuf.Int64s()
	...
	if err := b.args[2].VecEvalString(sctx, input, localeBuf); err != nil {
		return err
	}
	...
	if b.args[0].GetType().EvalType() == types.ETDecimal {
		...
		if err := b.args[0].VecEvalDecimal(sctx, input, xBuf); err != nil {
			return err
		}

		result.ReserveString(n)
		xBuf.MergeNulls(dBuf)
		return formatDecimal(sctx, xBuf, dInt64s, result, localeBuf)
	}
	...
	defer b.bufAllocator.put(xBuf)
	if err := b.args[0].VecEvalReal(sctx, input, xBuf); err != nil {
		return err
	}

	result.ReserveString(n)
	xBuf.MergeNulls(dBuf)
	return formatReal(sctx, xBuf, dInt64s, result, localeBuf)
}
...
func (b *builtinSubstring2ArgsSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinSubstring2ArgsUTF8Sig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinTrim2ArgsSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinInstrUTF8Sig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	str, err := b.bufAllocator.get()
	if err != nil {
		return err
	...
	if err := b.args[1].VecEvalString(sctx, input, substr); err != nil {
		return err
	}
	result.ResizeInt64(n, false)
...
func (b *builtinOctStringSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinEltSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf0, err := b.bufAllocator.get()
	if err != nil {
		return err
	...
	defer b.bufAllocator.put(buf0)
	if err := b.args[0].VecEvalInt(sctx, input, buf0); err != nil {
		return err
	}
	...
	for i := 0; i < n; i++ {
		...
		if bufs[j] == nil {
			...
			if err := b.args[j].VecEvalString(sctx, input, bufs[j]); err != nil {
				return err
			}
		}
		if bufs[j].IsNull(i) {
...
func (b *builtinInsertUTF8Sig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
	...
	if err := b.args[3].VecEvalString(sctx, input, buf3); err != nil {
		return err
	}
...
// See https://dev.mysql.com/doc/refman/5.7/en/string-functions.html#function_export-set
func (b *builtinExportSet5ArgSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	bits, err := b.bufAllocator.get()
	if err != nil {
		return err
	...
	if err := b.args[4].VecEvalInt(sctx, input, numberOfBits); err != nil {
		return err
	}
	result.ReserveString(n)
...
// See https://dev.mysql.com/doc/refman/5.7/en/string-functions.html#function_substr
func (b *builtinSubstring3ArgsUTF8Sig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinTrim3ArgsSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf0, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinOrdSig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinInstrSig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	str, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinLengthSig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
// See https://dev.mysql.com/doc/refman/5.7/en/string-functions.html#function_locate
func (b *builtinLocate2ArgsUTF8Sig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinBitLengthSig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinCharSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	l := len(b.args)
	...
	for i := 0; i < len(b.args)-1; i++ {
		...
		if err := b.args[i].VecEvalInt(sctx, input, buf[i]); err != nil {
			return err
		}
	}
	...
	hasStrictMode := sctx.GetSessionVars().StrictSQLMode
	for i := 0; i < n; i++ {
		bigints = bigints[0:0]
		for j := 0; j < l-1; j++ {
			if buf[j].IsNull(i) {
...
func (b *builtinReplaceSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinMakeSetSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	nr := input.NumRows()
	...
	if err := b.args[0].VecEvalInt(sctx, input, bitsBuf); err != nil {
		return err
	}
	...
	for i := 1; i < len(b.args); i++ {
		...
		if err := b.args[i].VecEvalString(sctx, input, strBuf[i-1]); err != nil {
			return err
		}
	}
...
func (b *builtinOctIntSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinToBase64Sig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
	...
	for i := 0; i < n; i++ {
		...
		if needEncodeLen == -1 {
			...
			if err := handleAllowedPacketOverflowed(sctx, "to_base64", b.maxAllowedPacket); err != nil {
				return err
			}
...
func (b *builtinTrim1ArgSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
// vecEvalString evals RPAD(str,len,padstr).
// See https://dev.mysql.com/doc/refman/5.7/en/string-functions.html#function_rpad
func (b *builtinRpadUTF8Sig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinCharLengthBinarySig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinBinSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinFormatSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	...
	if b.args[0].GetType().EvalType() == types.ETDecimal {
		...
		result.ReserveString(n)
		xBuf.MergeNulls(dBuf)
		return formatDecimal(sctx, xBuf, dInt64s, result, nil)
	}
	...
	result.ReserveString(n)
	xBuf.MergeNulls(dBuf)
	return formatReal(sctx, xBuf, dInt64s, result, nil)
}
...
func (b *builtinRightSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
// vecEvalString evals SUBSTR(str,pos,len), SUBSTR(str FROM pos FOR len), SUBSTR() is a synonym for SUBSTRING().
// See https://dev.mysql.com/doc/refman/5.7/en/string-functions.html#function_substr
func (b *builtinSubstring3ArgsSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinHexIntArgSig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinFromBase64Sig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
	...
	for i := 0; i < n; i++ {
		...
		if needDecodeLen == -1 {
			...
			if err := handleAllowedPacketOverflowed(sctx, "from_base64", b.maxAllowedPacket); err != nil {
				return err
			}
...
func (b *builtinCharLengthUTF8Sig) vecEvalInt(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinTranslateBinarySig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf0, err := b.bufAllocator.get()
	if err != nil {
		return err
...
func (b *builtinTranslateUTF8Sig) vecEvalString(sctx sessionctx.Context, input *chunk.Chunk, result *chunk.Column) error {
	n := input.NumRows()
	buf0, err := b.bufAllocator.get()
	if err != nil {
		return err
