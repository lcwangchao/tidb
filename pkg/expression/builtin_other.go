...
func (b *builtinInIntSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	...
	hasNull := b.hasNull
	for _, arg := range args {
		evaledArg, isNull, err := arg.EvalInt(sctx, row)
		if err != nil {
			return 0, true, err
		}
		if isNull {
...
func (b *builtinInStringSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	...
	hasNull := b.hasNull
	for _, arg := range args {
		evaledArg, isNull, err := arg.EvalString(sctx, row)
		if err != nil {
			return 0, true, err
		}
		if isNull {
...
func (b *builtinInRealSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	...
	hasNull := b.hasNull
	for _, arg := range args {
		evaledArg, isNull, err := arg.EvalReal(sctx, row)
		if err != nil {
			return 0, true, err
		}
		if isNull {
...
func (b *builtinInDecimalSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	...
	hasNull := b.hasNull
	for _, arg := range args {
		evaledArg, isNull, err := arg.EvalDecimal(sctx, row)
		if err != nil {
			return 0, true, err
		}
		if isNull {
...
func (b *builtinInTimeSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	...
	hasNull := b.hasNull
	for _, arg := range args {
		evaledArg, isNull, err := arg.EvalTime(sctx, row)
		if err != nil {
			return 0, true, err
		}
		if isNull {
...
func (b *builtinInDurationSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	...
	hasNull := b.hasNull
	for _, arg := range args {
		evaledArg, isNull, err := arg.EvalDuration(sctx, row)
		if err != nil {
			return 0, true, err
		}
		if isNull {
...
func (b *builtinInJSONSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	...
	for _, arg := range b.args[1:] {
		evaledArg, isNull, err := arg.EvalJSON(sctx, row)
		if err != nil {
			return 0, true, err
		}
		if isNull {
...
func (b *builtinRowSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
...
func (b *builtinSetStringVarSig) evalString(sctx sessionctx.Context, row chunk.Row) (res string, isNull bool, err error) {
	var varName string
	sessionVars := sctx.GetSessionVars()
	varName, isNull, err = b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return "", isNull, err
	}
	datum, err := b.args[1].Eval(sctx, row)
	isNull = datum.IsNull()
	if isNull || err != nil {
		return "", isNull, err
	}
...
func (b *builtinSetRealVarSig) evalReal(sctx sessionctx.Context, row chunk.Row) (res float64, isNull bool, err error) {
	var varName string
	sessionVars := sctx.GetSessionVars()
	varName, isNull, err = b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
...
func (b *builtinSetDecimalVarSig) evalDecimal(sctx sessionctx.Context, row chunk.Row) (*types.MyDecimal, bool, error) {
	sessionVars := sctx.GetSessionVars()
	varName, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
...
func (b *builtinSetIntVarSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	sessionVars := sctx.GetSessionVars()
	varName, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return 0, isNull, err
...
func (b *builtinSetTimeVarSig) evalTime(sctx sessionctx.Context, row chunk.Row) (types.Time, bool, error) {
	sessionVars := sctx.GetSessionVars()
	varName, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
	...
	if err != nil || datum.IsNull() {
		return types.ZeroTime, datum.IsNull(), handleInvalidTimeError(sctx, err)
	}
...
func (b *builtinGetStringVarSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	sessionVars := sctx.GetSessionVars()
	varName, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return "", isNull, err
...
func (b *builtinGetIntVarSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	sessionVars := sctx.GetSessionVars()
	varName, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return 0, isNull, err
...
func (b *builtinGetRealVarSig) evalReal(sctx sessionctx.Context, row chunk.Row) (float64, bool, error) {
	sessionVars := sctx.GetSessionVars()
	varName, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return 0, isNull, err
...
func (b *builtinGetDecimalVarSig) evalDecimal(sctx sessionctx.Context, row chunk.Row) (*types.MyDecimal, bool, error) {
	sessionVars := sctx.GetSessionVars()
	varName, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return nil, isNull, err
...
func (b *builtinGetTimeVarSig) evalTime(sctx sessionctx.Context, row chunk.Row) (types.Time, bool, error) {
	sessionVars := sctx.GetSessionVars()
	varName, isNull, err := b.args[0].EvalString(sctx, row)
	if isNull || err != nil {
		return types.ZeroTime, isNull, err
...
func (b *builtinValuesIntSig) evalInt(sctx sessionctx.Context, _ chunk.Row) (int64, bool, error) {
	row := sctx.GetSessionVars().CurrInsertValues
	if row.IsEmpty() {
		return 0, true, nil
	}
	if b.offset < row.Len() {
		...
		if len(val) < 8 {
			...
			v, err := binary.ToInt(sctx.GetSessionVars().StmtCtx)
			if err != nil {
...
// See https://dev.mysql.com/doc/refman/5.7/en/miscellaneous-functions.html#function_values
func (b *builtinValuesRealSig) evalReal(sctx sessionctx.Context, _ chunk.Row) (float64, bool, error) {
	row := sctx.GetSessionVars().CurrInsertValues
	if row.IsEmpty() {
		return 0, true, nil
	}
...
// See https://dev.mysql.com/doc/refman/5.7/en/miscellaneous-functions.html#function_values
func (b *builtinValuesDecimalSig) evalDecimal(sctx sessionctx.Context, _ chunk.Row) (*types.MyDecimal, bool, error) {
	row := sctx.GetSessionVars().CurrInsertValues
	if row.IsEmpty() {
...
// See https://dev.mysql.com/doc/refman/5.7/en/miscellaneous-functions.html#function_values
func (b *builtinValuesStringSig) evalString(sctx sessionctx.Context, _ chunk.Row) (string, bool, error) {
	row := sctx.GetSessionVars().CurrInsertValues
	if row.IsEmpty() {
		return "", true, nil
	}
...
// See https://dev.mysql.com/doc/refman/5.7/en/miscellaneous-functions.html#function_values
func (b *builtinValuesTimeSig) evalTime(sctx sessionctx.Context, _ chunk.Row) (types.Time, bool, error) {
	row := sctx.GetSessionVars().CurrInsertValues
	if row.IsEmpty() {
		return types.ZeroTime, true, nil
	}
...
// See https://dev.mysql.com/doc/refman/5.7/en/miscellaneous-functions.html#function_values
func (b *builtinValuesDurationSig) evalDuration(sctx sessionctx.Context, _ chunk.Row) (types.Duration, bool, error) {
	row := sctx.GetSessionVars().CurrInsertValues
	if row.IsEmpty() {
...
// See https://dev.mysql.com/doc/refman/5.7/en/miscellaneous-functions.html#function_values
func (b *builtinValuesJSONSig) evalJSON(sctx sessionctx.Context, _ chunk.Row) (types.BinaryJSON, bool, error) {
	row := sctx.GetSessionVars().CurrInsertValues
	if row.IsEmpty() {
...
func (b *builtinBitCountSig) evalInt(sctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
...
func (b *builtinGetParamStringSig) evalString(sctx sessionctx.Context, row chunk.Row) (string, bool, error) {
	sessionVars := sctx.GetSessionVars()
