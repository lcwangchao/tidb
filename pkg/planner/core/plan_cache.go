...
func rebuildRange(p Plan) error {
	...
	case *PointGetPlan:
		...
		if x.HandleConstant != nil {
			dVal, err := convertConstant2Datum(p.SCtx(), x.HandleConstant, x.handleFieldType)
			if err != nil {
				return err
			}
		...
		for i, param := range x.IndexConstants {
			if param != nil {
				dVal, err := convertConstant2Datum(p.SCtx(), param, x.ColsFieldType[i])
				if err != nil {
					return err
				}
	...
	case *BatchPointGetPlan:
		...
		for i, param := range x.HandleParams {
			if param != nil {
				dVal, err := convertConstant2Datum(p.SCtx(), param, x.HandleType)
				if err != nil {
					return err
				}
				iv, err := dVal.ToInt64(sc)
		...
		for i, params := range x.IndexValueParams {
			...
			for j, param := range params {
				if param != nil {
					dVal, err := convertConstant2Datum(p.SCtx(), param, x.IndexColTypes[j])
					if err != nil {
						return err
					}
...
func convertConstant2Datum(sctx sessionctx.Context, con *expression.Constant, target *types.FieldType) (*types.Datum, error) {
	val, err := con.Eval(sctx, chunk.Row{})
	if err != nil {
		return nil, err
	}
	dVal, err := val.ConvertTo(sctx.GetSessionVars().StmtCtx, target)
	if err != nil {
		return nil, err
	}
	// The converted result must be same as original datum.
	cmp, err := dVal.Compare(sctx.GetSessionVars().StmtCtx, &val, collate.GetCollator(target.GetCollate()))
	if err != nil || cmp != 0 {
