...
func (*aggregationPushDownSolver) pushAggCrossUnion(agg *LogicalAggregation, unionSchema *expression.Schema, unionChild LogicalPlan) (LogicalPlan, error) {
	...
	for _, aggFunc := range agg.AggFuncs {
		...
		for _, arg := range newAggFunc.Args {
			newArgs = append(newArgs, expression.ColumnSubstitute(agg.SCtx(), arg, unionSchema, expression.Column2Exprs(unionChild.Schema().Columns)))
		}
		newAggFunc.Args = newArgs
		newAgg.AggFuncs = append(newAgg.AggFuncs, newAggFunc)
	}
	for _, gbyExpr := range agg.GroupByItems {
		newExpr := expression.ColumnSubstitute(agg.SCtx(), gbyExpr, unionSchema, expression.Column2Exprs(unionChild.Schema().Columns))
		...
		if proj != nil {
			...
			if join, ok1 := child.(*LogicalJoin); ok1 && a.checkValidJoin(join) && p.SCtx().GetSessionVars().AllowAggPushDown {
				...
				for _, gbyItem := range agg.GroupByItems {
					newGbyItems = append(newGbyItems, expression.ColumnSubstitute(p.SCtx(), gbyItem, proj.schema, proj.Exprs))
				...
				if noSideEffects {
					for _, aggFunc := range agg.AggFuncs {
						...
						for _, arg := range aggFunc.Args {
							newArgs = append(newArgs, expression.ColumnSubstitute(p.SCtx(), arg, proj.schema, proj.Exprs))
						}
						...
						if len(aggFunc.OrderByItems) != 0 {
							...
							for _, oby := range aggFunc.OrderByItems {
								newOrderByItems = append(newOrderByItems, expression.ColumnSubstitute(p.SCtx(), oby.Expr, proj.schema, proj.Exprs))
							}
