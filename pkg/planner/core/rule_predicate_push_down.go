...
func (p *LogicalProjection) PredicatePushDown(predicates []expression.Expression, opt *logicalOptimizeOp) (ret []expression.Expression, retPlan LogicalPlan) {
	...
	for _, cond := range predicates {
		substituted, hasFailed, newFilter := expression.ColumnSubstituteImpl(p.SCtx(), cond, p.Schema(), p.Exprs, true)
...
func DeriveOtherConditions(
	...
	for _, expr := range p.OtherConditions {
		if deriveLeft {
			leftRelaxedCond := expression.DeriveRelaxedFiltersFromDNF(p.SCtx(), expr, leftSchema)
			if leftRelaxedCond != nil {
				leftCond = append(leftCond, leftRelaxedCond)
			}
			notNullExpr := deriveNotNullExpr(p.SCtx(), expr, leftSchema)
			if notNullExpr != nil {
				leftCond = append(leftCond, notNullExpr)
			}
		}
		if deriveRight {
			rightRelaxedCond := expression.DeriveRelaxedFiltersFromDNF(p.SCtx(), expr, rightSchema)
			...
			notNullExpr := deriveNotNullExpr(p.SCtx(), expr, rightSchema)
			if notNullExpr != nil {
