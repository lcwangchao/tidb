...
func (w *worker) onRecoverTable(d *ddlCtx, t *meta.Meta, job *model.Job) (ver int64, err error) {
	...
	case model.StateWriteOnly:
		...
		err = updateLabelRules(w.ddlCtx, job, tblInfo, oldRules, tableRuleID, partRuleIDs, oldRuleIDs, tblInfo.ID)
		if err != nil {
			job.State = model.JobStateCancelled
...
func updateLabelRules(ddlCtx *ddlCtx, job *model.Job, tblInfo *model.TableInfo, oldRules map[string]*label.Rule, tableRuleID string, partRuleIDs, oldRuleIDs []string, tID int64) error {
	if oldRules == nil {
		return nil
	}
	...
	return ddlCtx.infoSyncer.UpdateLabelRules(context.TODO(), patch)
}
