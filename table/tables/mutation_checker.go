...
func checkRowInsertionConsistency(
	...
	for columnID, decodedDatum := range decodedData {
		...
		cmp, err := decodedDatum.Compare(sessVars.StmtCtx.ValCtx, &inputDatum, collate.GetCollator(decodedDatum.Collation()))
		if err != nil {
			return errors.Trace(err)
		}
		if cmp != 0 {
...
func CompareIndexAndVal(sctx *stmtctx.StatementContext, rowVal types.Datum, idxVal types.Datum, collator collate.Collator, cmpMVIndex bool) (int, error) {
	...
	if cmpMVIndex {
		...
		for elemIdx := 0; elemIdx < count; elemIdx++ {
			...
			cmpRes, err = jsonDatum.Compare(sctx.ValCtx, &idxVal, collate.GetBinaryCollator())
			if err != nil {
		...
		cmpRes, err = idxVal.Compare(sctx.ValCtx, &rowVal, collator)
	}
	return cmpRes, err
}
