...
func (t *partitionedTable) locateRangeColumnPartition(ctx table.TblContext, partitionExpr *PartitionExpr, r []types.Datum) (int, error) {
	...
	idx := sort.Search(len(upperBounds), func(i int) bool {
		...
		ret, isNull, err := upperBounds[i].EvalInt(ctx.GetSessionContext(), evalBuffer.ToRow())
		if err != nil {
			lastError = err
	...
	if idx >= len(upperBounds) {
		...
		if t.meta.Partition.Expr != "" {
			e, err := expression.ParseSimpleExprWithTableInfo(ctx.GetSessionContext(), t.meta.Partition.Expr, t.meta)
			if err == nil {
				val, _, err := e.EvalInt(ctx.GetSessionContext(), chunk.MutRowFromDatums(r).ToRow())
				if err == nil {
...
func (t *partitionedTable) locateRangePartition(ctx table.TblContext, partitionExpr *PartitionExpr, r []types.Datum) (int, error) {
	...
	if col, ok := partitionExpr.Expr.(*expression.Column); ok {
		...
		defer t.evalBufferPool.Put(evalBuffer)
		evalBuffer.SetDatums(r...)
		val, isNull, err = partitionExpr.Expr.EvalInt(ctx.GetSessionContext(), evalBuffer.ToRow())
		if err != nil {
			return 0, err
		}
		ret = val
...
func (t *partitionedTable) locateHashPartition(ctx table.TblContext, partExpr *PartitionExpr, numParts uint64, r []types.Datum) (int, error) {
	...
	defer t.evalBufferPool.Put(evalBuffer)
	evalBuffer.SetDatums(r...)
	ret, isNull, err := partExpr.Expr.EvalInt(ctx.GetSessionContext(), evalBuffer.ToRow())
	if err != nil {
		return 0, err
	}
	if isNull {
...
func (t *partitionedTable) RemoveRecord(sctx table.TblContext, h kv.Handle, r []types.Datum) error {
	...
	tbl := t.GetPartition(pid)
	err = tbl.RemoveRecord(sctx, h, r)
	if err != nil {
		return errors.Trace(err)
	}

	if _, ok := t.reorganizePartitions[pid]; ok {
		pid, err = t.locateReorgPartition(sctx, r)
		if err != nil {
			return errors.Trace(err)
		}
		tbl = t.GetPartition(pid)
