...
func TestInfo(t *testing.T) {
	...
	dom := NewDomain(mockStore, ddlLease, 0, 0, 0, mockFactory, variable.GlobalDomVars, nil)
	defer func() {
		dom.Close()
		err := s.Close()
		require.NoError(t, err)
...
func TestStatWorkRecoverFromPanic(t *testing.T) {
	...
	ddlLease := 80 * time.Millisecond
	dom := NewDomain(store, ddlLease, 0, 0, 0, mockFactory, variable.GlobalDomVars, nil)
