...
func (a *skewDistinctAggRewriter) rewriteSkewDistinctAgg(agg *LogicalAggregation, opt *logicalOptimizeOp) LogicalPlan {
	...
	for i, aggFunc := range agg.AggFuncs {
		...
		if aggFunc.HasDistinct {
			...
			for _, arg := range aggFunc.Args {
				firstRow, err := aggregation.NewAggFuncDesc(agg.ExprCtx(), ast.AggFuncFirstRow,
			...
			if newAggFunc.Name == ast.AggFuncCount {
				...
				sumAggFunc, err := aggregation.NewAggFuncDesc(agg.ExprCtx(), ast.AggFuncSum,
	...
	for _, index := range cntIndexes {
		...
		if !exprType.Equal(targetType) {
			proj.Exprs[index] = expression.BuildCastFunction(agg.ExprCtx(), proj.Exprs[index], targetType)
		}
	}
