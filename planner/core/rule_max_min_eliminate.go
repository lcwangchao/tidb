...
func (*maxMinEliminator) eliminateSingleMaxMin(agg *LogicalAggregation, opt *logicalOptimizeOp) *LogicalAggregation {
	...
	if len(expression.ExtractColumns(f.Args[0])) > 0 {
		...
		if !mysql.HasNotNullFlag(f.Args[0].GetType().GetFlag()) {
			...
			isNullFunc := expression.NewFunctionInternal(expression.NewExprContext(ctx), ast.IsNull, types.NewFieldType(mysql.TypeTiny), f.Args[0])
			notNullFunc := expression.NewFunctionInternal(expression.NewExprContext(ctx), ast.UnaryNot, types.NewFieldType(mysql.TypeTiny), isNullFunc)
