...
func (la *LogicalAggregation) canPullUp() bool {
	...
	for _, f := range la.AggFuncs {
		for _, arg := range f.Args {
			expr := expression.EvaluateExprWithNull(la.ExprCtx(), la.children[0].Schema(), arg)
...
func (la *LogicalApply) deCorColFromEqExpr(expr expression.Expression) expression.Expression {
	...
	if col, lOk := sf.GetArgs()[0].(*expression.Column); lOk {
		if corCol, rOk := sf.GetArgs()[1].(*expression.CorrelatedColumn); rOk {
			...
			return expression.NewFunctionInternal(la.ExprCtx(), ast.EQ, types.NewFieldType(mysql.TypeTiny), ret, col)
		}
	}
		...
		if len(apply.CorCols) == 0 {
			...
			if apply.canPullUpAgg() && agg.canPullUp() {
				...
				for i, col := range outerPlan.Schema().Columns {
					first, err := aggregation.NewAggFuncDesc(agg.ExprCtx(), ast.AggFuncFirstRow, []expression.Expression{col}, false)
					if err != nil {
						return nil, err
					}
				...
				for i, aggFunc := range agg.AggFuncs {
					...
					desc, err := aggregation.NewAggFuncDesc(agg.ExprCtx(), agg.AggFuncs[i].Name, aggArgs, agg.AggFuncs[i].HasDistinct)
					if err != nil {
						return nil, err
					}
					...
					if len(apply.CorCols) == 0 {
						...
						for _, eqCond := range eqCondWithCorCol {
							...
							if agg.schema.ColumnIndex(eqCond.GetArgs()[1].(*expression.Column)) == -1 {
								newFunc, err := aggregation.NewAggFuncDesc(apply.ExprCtx(), ast.AggFuncFirstRow, []expression.Expression{clonedCol}, false)
								if err != nil {
									return nil, err
								}
						...
						if len(defaultValueMap) > 0 {
							...
							for i, val := range defaultValueMap {
								...
								ifNullFunc := expression.NewFunctionInternal(agg.ExprCtx(), ast.Ifnull, types.NewFieldType(mysql.TypeLonglong), agg.schema.Columns[i], val)
