	...
	for _, r := range ranges {
		if !r.IsPointNullable(ctx) {
			...
			if col, ok := hashExpr.(*expression.Column); ok && col.RetType.EvalType() == types.ETInt {
				...
				posHigh, highIsNull, err := hashExpr.EvalInt(expression.NewDefaultEvalContext(), chunk.MutRowFromDatums(r.HighVal).ToRow())
				if err != nil {
					return nil, nil, err
				}

				posLow, lowIsNull, err := hashExpr.EvalInt(expression.NewDefaultEvalContext(), chunk.MutRowFromDatums(r.LowVal).ToRow())
				if err != nil {
					return nil, nil, err
				}
		...
		pos, isNull, err := hashExpr.EvalInt(expression.NewDefaultEvalContext(), chunk.MutRowFromDatums(highLowVals).ToRow())
		if err != nil {
	...
	for _, r := range ranges {
		if !r.IsPointNullable(ctx) {
			if len(partCols) == 1 && partCols[0].RetType.EvalType() == types.ETInt {
				col := partCols[0]
				posHigh, highIsNull, err := col.EvalInt(expression.NewEvalContext(ctx), chunk.MutRowFromDatums(r.HighVal).ToRow())
				if err != nil {
					return nil, nil, err
				}

				posLow, lowIsNull, err := col.EvalInt(expression.NewEvalContext(ctx), chunk.MutRowFromDatums(r.LowVal).ToRow())
				if err != nil {
					return nil, nil, err
				}
...
func (l *listPartitionPruner) findUsedListPartitions(sctx sessionctx.Context, conds []expression.Expression) (map[int]struct{}, error) {
	if len(conds) == 0 {
		return l.fullRange, nil
	}
	...
	for _, r := range ranges {
		...
		value, isNull, err := pruneExpr.EvalInt(expression.NewEvalContext(sctx), chunk.MutRowFromDatums(r.HighVal).ToRow())
		if err != nil {
			return nil, err
		}
...
func (s *partitionProcessor) findUsedListPartitions(ctx sessionctx.Context, tbl table.Table, partitionNames []model.CIStr,
	...
	if partExpr.ForListPruning.ColPrunes == nil {
		used, err = listPruner.findUsedListPartitions(ctx, conds)
	} else {
...
func partitionRangeForInExpr(sctx sessionctx.Context, args []expression.Expression,
	...
	for i := 1; i < len(args); i++ {
		...
		if pruner.partFn != nil {
			...
			val, _, err = partFnConst.EvalInt(expression.NewEvalContext(sctx), chunk.Row{})
		} else {
...
func (p *rangePruner) extractDataForPrune(sctx sessionctx.Context, expr expression.Expression) (dataForPrune, bool) {
	...
	c, isNull, err := constExpr.EvalInt(expression.NewDefaultEvalContext(), chunk.Row{})
	if err == nil && !isNull {
		ret.c = c
		return ret, true
	}
...
func (p *rangeColumnsPruner) pruneUseBinarySearch(sctx sessionctx.Context, op string, data *expression.Constant) (start int, end int) {
	...
	compare := func(ith int, op string, v *expression.Constant) bool {
		for i := range p.partCols {
			...
			var val int64
			val, isNull, err = expr.EvalInt(expression.NewDefaultEvalContext(), chunk.Row{})
			if err != nil {
				savedError = err
				return true
			}
