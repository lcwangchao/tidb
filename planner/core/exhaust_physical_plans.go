...
func (p *LogicalUnionScan) exhaustPhysicalPlans(prop *property.PhysicalProperty) ([]PhysicalPlan, bool, error) {
	...
	}.Init(p.SCtx(), p.StatsInfo(), p.SelectBlockOffset(), childProp)
...
func findMaxPrefixLen(candidates [][]*expression.Column, keys []*expression.Column) int {
	...
	for _, candidateKeys := range candidates {
		matchedLen := 0
		for i := range keys {
			if !(i < len(candidateKeys) && keys[i].Equal(candidateKeys[i])) {
				break
			}
			matchedLen++
		}
...
func (p *LogicalJoin) GetMergeJoin(prop *property.PhysicalProperty, schema *expression.Schema, statsInfo *property.StatsInfo, leftStatsInfo *property.StatsInfo, rightStatsInfo *property.StatsInfo) []PhysicalPlan {
	...
	for _, lhsChildProperty := range p.leftProperties {
		...
		mergeJoin := PhysicalMergeJoin{basePhysicalJoin: baseJoin}.Init(p.SCtx(), statsInfo.ScaleByExpectCnt(prop.ExpectedCnt), p.SelectBlockOffset())
...
func (p *LogicalJoin) getEnforcedMergeJoin(prop *property.PhysicalProperty, schema *expression.Schema, statsInfo *property.StatsInfo) []PhysicalPlan {
	...
	for _, item := range prop.SortItems {
		...
		for joinKeyPos := 0; joinKeyPos < len(leftJoinKeys); joinKeyPos++ {
			...
			if item.Col.Equal(leftJoinKeys[joinKeyPos]) {
				key = leftJoinKeys[joinKeyPos]
				hasLeftColInProp = true
			}
			if item.Col.Equal(rightJoinKeys[joinKeyPos]) {
	...
	enforcedPhysicalMergeJoin := PhysicalMergeJoin{basePhysicalJoin: baseJoin, Desc: desc}.Init(p.SCtx(), statsInfo.ScaleByExpectCnt(prop.ExpectedCnt), p.SelectBlockOffset())
...
func (p *LogicalJoin) constructIndexJoin(
	...
	}.Init(p.SCtx(), p.StatsInfo().ScaleByExpectCnt(prop.ExpectedCnt), p.SelectBlockOffset(), chReqProps...)
	if path != nil {
...
func (p *LogicalJoin) constructIndexMergeJoin(
	...
	for _, plan := range indexJoins {
		...
		for i := 0; canKeepOuterOrder && i < len(prop.SortItems); i++ {
			if !prop.SortItems[i].Col.Equal(join.OuterJoinKeys[keyOff2KeyOffOrderByIdx[i]]) {
...
func (cwc *ColWithCmpFuncManager) BuildRangesByRow(ctx sessionctx.Context, row chunk.Row) ([]*ranger.Range, error) {
	...
	for i, opType := range cwc.OpType {
		...
		newExpr, err := expression.NewFunction(expression.NewExprContext(ctx), opType, types.NewFieldType(mysql.TypeTiny), cwc.TargetCol, cwc.TmpConstant[i])
		if err != nil {
			return nil, err
		}
	...
	for i, idxCol := range idxCols {
		...
		if ijHelper.curIdxOff2KeyOff[i] >= 0 {
			...
			if collate.NewCollationEnabled() && types.IsString(idxCol.RetType.GetType()) && types.IsString(outerKeys[ijHelper.curIdxOff2KeyOff[i]].RetType.GetType()) {
				et, err := expression.CheckAndDeriveCollationFromExprs(expression.NewExprContext(ijHelper.innerPlan.SCtx()), "equal", types.ETInt, idxCol, outerKeys[ijHelper.curIdxOff2KeyOff[i]])
				if err != nil {
...
loopOtherConds:
	for _, filter := range ijHelper.join.OtherConditions {
		...
		if lCol, ok := sf.GetArgs()[0].(*expression.Column); ok && lCol.Equal(nextCol) {
			anotherArg = sf.GetArgs()[1]
			funcName = sf.FuncName.L
		} else if rCol, ok := sf.GetArgs()[1].(*expression.Column); ok && rCol.Equal(nextCol) {
...
func (ijHelper *indexJoinBuildHelper) removeUselessEqAndInFunc(idxCols []*expression.Column, notKeyEqAndIn []expression.Expression) (usefulEqAndIn, uselessOnes []expression.Expression) {
	...
	for idxColPos, notKeyColPos := 0, 0; idxColPos < len(idxCols); idxColPos++ {
		...
		if notKeyColPos < len(notKeyEqAndIn) && ijHelper.curNotUsedIndexCols[notKeyColPos].Equal(idxCols[idxColPos]) {
			notKeyColPos++
			continue
		}
...
func (ijHelper *indexJoinBuildHelper) createMutableIndexJoinRange(relatedExprs []expression.Expression, ranges []*ranger.Range, path *util.AccessPath, innerKeys, outerKeys []*expression.Column) ranger.MutableRanges {
	...
	if expression.MaybeOverOptimized4PlanCache(expression.NewExprContext(ijHelper.join.SCtx()), relatedExprs) {
...
func (p *LogicalJoin) tryToGetMppHashJoin(prop *property.PhysicalProperty, useBCJ bool) []PhysicalPlan {
	...
	}.Init(p.SCtx(), p.StatsInfo(), p.SelectBlockOffset(), childrenProps...)
	join.SetSchema(p.schema)
	return []PhysicalPlan{join}
}
...
func (p *LogicalExpand) exhaustPhysicalPlans(prop *property.PhysicalProperty) ([]PhysicalPlan, bool, error) {
	...
	if p.SCtx().GetSessionVars().IsMPPAllowed() {
		...
		}.Init(p.SCtx(), p.StatsInfo().ScaleByExpectCnt(prop.ExpectedCnt), p.SelectBlockOffset(), mppProp)
...
func (lt *LogicalTopN) getPhysTopN(prop *property.PhysicalProperty) []PhysicalPlan {
	...
	for _, tp := range allTaskTypes {
		...
		topN := PhysicalTopN{
			...
			Count:       lt.Count,
			Offset:      lt.Offset,
		}.Init(lt.SCtx(), lt.StatsInfo(), lt.SelectBlockOffset(), resultProp)
		ret = append(ret, topN)
	}
	return ret
}
...
func MatchItems(p *property.PhysicalProperty, items []*util.ByItems) bool {
	...
	for i, col := range p.SortItems {
		sortItem := items[i]
		if sortItem.Desc != col.Desc || !sortItem.Expr.Equal(col.Col) {
			return false
		}
	}
	return true
...
func (lw *LogicalWindow) tryToGetMppWindows(prop *property.PhysicalProperty) []PhysicalPlan {
	...
	window := PhysicalWindow{
		...
		Frame:           lw.Frame,
		storeTp:         kv.TiFlash,
	}.Init(lw.SCtx(), lw.StatsInfo().ScaleByExpectCnt(prop.ExpectedCnt), lw.SelectBlockOffset(), childProperty)
...
func (la *LogicalAggregation) getEnforcedStreamAggs(prop *property.PhysicalProperty) []PhysicalPlan {
	...
	for _, taskTp := range taskTypes {
		...
		*copiedChildProperty = *childProp // It's ok to not deep copy the "cols" field.
		...
		}.initForStream(la.SCtx(), la.StatsInfo().ScaleByExpectCnt(prop.ExpectedCnt), la.SelectBlockOffset(), copiedChildProperty)
	...
	finalAggAdjust := func(aggFuncs []*aggregation.AggFuncDesc) {
		for i, agg := range aggFuncs {
			if agg.Mode == aggregation.FinalMode && agg.Name == ast.AggFuncCount {
				aggFuncs[i], _ = aggregation.NewAggFuncDesc(expression.NewExprContext(la.SCtx()), ast.AggFuncSum, agg.Args, false)
			}
		}
	}
...
func (p *LogicalLimit) exhaustPhysicalPlans(prop *property.PhysicalProperty) ([]PhysicalPlan, bool, error) {
	...
	for _, tp := range allTaskTypes {
		...
		}.Init(p.SCtx(), p.StatsInfo(), p.SelectBlockOffset(), resultProp)
...
func (p *LogicalUnionAll) exhaustPhysicalPlans(prop *property.PhysicalProperty) ([]PhysicalPlan, bool, error) {
	...
	if canUseMpp && prop.TaskTp == property.RootTaskType {
		...
		mppUA := PhysicalUnionAll{mpp: true}.Init(p.SCtx(), p.StatsInfo().ScaleByExpectCnt(prop.ExpectedCnt), p.SelectBlockOffset(), chReqProps...)
...
func (ls *LogicalSort) getPhysicalSort(prop *property.PhysicalProperty) *PhysicalSort {
	ps := PhysicalSort{ByItems: ls.ByItems}.Init(ls.SCtx(), ls.StatsInfo().ScaleByExpectCnt(prop.ExpectedCnt), ls.SelectBlockOffset(), &property.PhysicalProperty{TaskTp: prop.TaskTp, ExpectedCnt: math.MaxFloat64, RejectSort: true, CTEProducerStatus: prop.CTEProducerStatus})
	return ps
}
...
func (ls *LogicalSort) getNominalSort(reqProp *property.PhysicalProperty) *NominalSort {
	...
	ps := NominalSort{OnlyColumn: onlyColumn, ByItems: ls.ByItems}.Init(
		ls.SCtx(), ls.StatsInfo().ScaleByExpectCnt(prop.ExpectedCnt), ls.SelectBlockOffset(), prop)
	return ps
}
	...
	if prop.TaskTp == property.RootTaskType {
		...
		if ls.canPushToCopImpl(kv.TiFlash, true) {
			...
			ps := NominalSort{OnlyColumn: true, ByItems: ls.ByItems}.Init(
				ls.SCtx(), ls.StatsInfo().ScaleByExpectCnt(prop.ExpectedCnt), ls.SelectBlockOffset(), newProp)
...
func (p *LogicalMaxOneRow) exhaustPhysicalPlans(prop *property.PhysicalProperty) ([]PhysicalPlan, bool, error) {
	...
	mor := PhysicalMaxOneRow{}.Init(p.SCtx(), p.StatsInfo(), p.SelectBlockOffset(), &property.PhysicalProperty{ExpectedCnt: 2, CTEProducerStatus: prop.CTEProducerStatus})
...
func (p *LogicalSequence) exhaustPhysicalPlans(prop *property.PhysicalProperty) ([]PhysicalPlan, bool, error) {
	...
	for _, propChoice := range possibleChildrenProps {
		...
		seq := PhysicalSequence{}.Init(p.SCtx(), p.StatsInfo(), p.SelectBlockOffset(), childReqs...)
