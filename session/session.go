...
func (s *session) PreparedPlanCache() *kvcache.SimpleLRUCache {
	...
	if s.preparedPlanCache == nil { // lazy construction
		s.preparedPlanCache = kvcache.NewSimpleLRUCache(uint(s.GetSessionVars().DomVars.PreparedPlanCacheSize.Load()),
			s.GetSessionVars().DomVars.PreparedPlanCacheMemoryGuardRatio.Load(), plannercore.PreparedPlanCacheMaxMemory.Load())
	}
