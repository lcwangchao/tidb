...
func (e *bitOrUint64) UpdatePartialResult(sctx sessionctx.Context, rowsInGroup []chunk.Row, pr PartialResult) (memDelta int64, err error) {
	...
	for _, row := range rowsInGroup {
		inputValue, isNull, err := e.args[0].EvalInt(expression.NewEvalContext(sctx), row)
		if err != nil {
			return memDelta, err
		}
		if isNull {
...
func (e *bitXorUint64) Slide(sctx sessionctx.Context, getRow func(uint64) chunk.Row, lastStart, lastEnd uint64, shiftStart, shiftEnd uint64, pr PartialResult) error {
	...
	for i := uint64(0); i < shiftStart; i++ {
		inputValue, isNull, err := e.args[0].EvalInt(expression.NewEvalContext(sctx), getRow(lastStart+i))
		if err != nil {
			return err
		}
		if isNull {
	...
	for i := uint64(0); i < shiftEnd; i++ {
		inputValue, isNull, err := e.args[0].EvalInt(expression.NewEvalContext(sctx), getRow(lastEnd+i))
		if err != nil {
			return err
		}
		if isNull {
