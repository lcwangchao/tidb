...
func ConvertConfigItem2JSON(ctx sessionctx.Context, key string, val expression.Expression) (body string, err error) {
	...
	case types.ETString:
		var s string
		s, isNull, err = val.EvalString(expression.NewEvalContext(ctx), chunk.Row{})
		if err == nil && !isNull {
			str = fmt.Sprintf("%q", s)
		}
	case types.ETInt:
		var i int64
		i, isNull, err = val.EvalInt(expression.NewEvalContext(ctx), chunk.Row{})
		if err == nil && !isNull {
	...
	case types.ETReal:
		var f float64
		f, isNull, err = val.EvalReal(expression.NewEvalContext(ctx), chunk.Row{})
		if err == nil && !isNull {
			str = fmt.Sprintf("%v", f)
		}
	case types.ETDecimal:
		var d *types.MyDecimal
		d, isNull, err = val.EvalDecimal(expression.NewEvalContext(ctx), chunk.Row{})
		if err == nil && !isNull {
