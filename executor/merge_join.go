...
func (t *mergeJoinTable) fetchNextOuterGroup(ctx context.Context, exec *MergeJoinExec, requiredRows int) error {
	...
	if !t.executed && t.groupChecker.IsExhausted() {
		...
		t.filtersSelected, err = expression.VectorizedFilter(exec.EvalCtx(), t.filters, t.childChunkIter, t.filtersSelected)
		if err != nil {
			return err
		}
...
func (e *MergeJoinExec) compare(outerRow, innerRow chunk.Row) (int, error) {
	...
	for i := range outerJoinKeys {
		cmp, _, err := e.compareFuncs[i](e.EvalCtx(), outerJoinKeys[i], innerJoinKeys[i], outerRow, innerRow)
		if err != nil {
			return 0, err
		}
