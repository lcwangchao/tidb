	...
	if e.isRangeFrame {
		...
		for start = mathutil.Max(e.lastStartRow, e.stagedStartRow); start < e.rowCnt; start++ {
			...
			for i := range e.orderByCols {
				res, _, err = e.start.CmpFuncs[i](e.orderByCols[i], e.start.CalcFuncs[i], e.getRow(start), e.getRow(e.curRowIdx))
				if err != nil {
					return 0, err
				}
				if res != 0 {
...
func (e *PipelinedWindowExec) getEnd(ctx sessionctx.Context) (uint64, error) {
	...
	if e.isRangeFrame {
		...
		for end = mathutil.Max(e.lastEndRow, e.stagedEndRow); end < e.rowCnt; end++ {
			...
			var err error
			for i := range e.orderByCols {
				res, _, err = e.end.CmpFuncs[i](e.end.CalcFuncs[i], e.orderByCols[i], e.getRow(e.curRowIdx), e.getRow(end))
				if err != nil {
					return 0, err
				}
				if res != 0 {
