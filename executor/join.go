...
func (w *probeWorker) join2Chunk(probeSideChk *chunk.Chunk, hCtx *hashContext, joinResult *hashjoinWorkerResult,
	...
	var err error
	selected, err = expression.VectorizedFilter(expression.NewEvalContext(w.hashJoinCtx.sessCtx), w.hashJoinCtx.outerFilter, chunk.NewIterator4Chunk(probeSideChk), selected)
	if err != nil {
		joinResult.err = err
		return false, joinResult
	}
	...
	for chk := range buildSideResultCh {
		...
		if !w.hashJoinCtx.useOuterToBuild {
			...
			if len(w.hashJoinCtx.outerFilter) == 0 {
				...
				selected, err = expression.VectorizedFilter(expression.NewEvalContext(w.hashJoinCtx.sessCtx), w.hashJoinCtx.outerFilter, chunk.NewIterator4Chunk(chk), selected)
				if err != nil {
					return err
				}
...
func (e *NestedLoopApplyExec) fetchSelectedOuterRow(ctx context.Context, chk *chunk.Chunk) (*chunk.Row, error) {
	...
	for {
		if e.outerChunkCursor >= e.outerChunk.NumRows() {
			...
			e.outerSelected, err = expression.VectorizedFilter(e.EvalCtx(), e.outerFilter, outerIter, e.outerSelected)
			if err != nil {
				return nil, err
			}
...
func (e *NestedLoopApplyExec) fetchAllInners(ctx context.Context) error {
	...
	for {
		...
		e.innerSelected, err = expression.VectorizedFilter(e.EvalCtx(), e.innerFilter, innerIter, e.innerSelected)
		if err != nil {
			return err
		}
