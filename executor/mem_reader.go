...
func (m *memIndexReader) getMemRows(ctx context.Context) ([][]types.Datum, error) {
	...
	err := iterTxnMemBuffer(m.ctx, m.cacheTable, m.kvRanges, m.desc, func(key, value []byte) error {
		...
		matched, _, err := expression.EvalBool(expression.NewEvalContext(m.ctx), m.conditions, mutableRow.ToRow())
		if err != nil || !matched {
			return err
		}
...
func (iter *txnMemBufferIter) next() ([]types.Datum, error) {
	...
	for ; err == nil && curr.Valid(); err = curr.Next() {
		...
		if !rowcodec.IsNewFormat(curr.Value()) {
			...
			matched, _, err := expression.EvalBool(expression.NewEvalContext(iter.ctx), iter.conditions, mutableRow.ToRow())
			if err != nil {
				return nil, errors.Trace(err)
			}
			if !matched {
		...
		matched, _, err := expression.EvalBool(expression.NewEvalContext(iter.ctx), iter.conditions, row)
		if err != nil {
			return nil, errors.Trace(err)
		}
		if !matched {
