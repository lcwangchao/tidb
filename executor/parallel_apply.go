...
func (e *ParallelNestedLoopApplyExec) outerWorker(ctx context.Context) {
	...
	for {
		...
		selected, err = expression.VectorizedFilter(e.EvalCtx(), e.outerFilter, outerIter, selected)
		if err != nil {
			e.putResult(nil, err)
			return
		}
...
func (e *ParallelNestedLoopApplyExec) fetchAllInners(ctx context.Context, id int) (err error) {
	...
	for {
		...
		e.innerSelected[id], err = expression.VectorizedFilter(e.EvalCtx(), e.innerFilter[id], innerIter, e.innerSelected[id])
		if err != nil {
			return err
		}
