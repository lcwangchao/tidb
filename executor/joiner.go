...
func (j *baseJoiner) filter(input, output *chunk.Chunk, outerColLen int, lUsed, rUsed []int) (bool, error) {
	var err error
	j.selected, err = expression.VectorizedFilter(expression.NewEvalContext(j.ctx), j.conditions, chunk.NewIterator4Chunk(input), j.selected)
	if err != nil {
		return false, err
	}
...
func (j *baseJoiner) filterAndCheckOuterRowStatus(
	...
	var err error
	j.selected, j.isNull, err = expression.VectorizedFilterConsiderNull(expression.NewEvalContext(j.ctx), j.conditions, chunk.NewIterator4Chunk(input), j.selected, j.isNull)
	if err != nil {
		return nil, err
	}
...
func (j *semiJoiner) tryToMatchInners(outer chunk.Row, inners chunk.Iterator, chk *chunk.Chunk, _ ...NAAJType) (matched bool, hasNull bool, err error) {
	...
	for inner := inners.Current(); inner != inners.End(); inner = inners.Next() {
		...
		matched, _, err = expression.EvalBool(expression.NewEvalContext(j.ctx), j.conditions, j.shallowRow.ToRow())
		if err != nil {
...
func (naaj *nullAwareAntiSemiJoiner) tryToMatchInners(outer chunk.Row, inners chunk.Iterator, _ *chunk.Chunk, _ ...NAAJType) (matched bool, hasNull bool, err error) {
	...
	for inner := inners.Current(); inner != inners.End(); inner = inners.Next() {
		...
		valid, _, err := expression.EvalBool(expression.NewEvalContext(naaj.ctx), naaj.conditions, naaj.shallowRow.ToRow())
		if err != nil {
			return false, false, err
		}
...
func (j *antiSemiJoiner) tryToMatchInners(outer chunk.Row, inners chunk.Iterator, _ *chunk.Chunk, _ ...NAAJType) (matched bool, hasNull bool, err error) {
	...
	for inner := inners.Current(); inner != inners.End(); inner = inners.Next() {
		...
		matched, isNull, err := expression.EvalBool(expression.NewEvalContext(j.ctx), j.conditions, j.shallowRow.ToRow())
		if err != nil {
			return false, false, err
		}
		if matched {
...
func (naal *nullAwareAntiLeftOuterSemiJoiner) tryToMatchInners(outer chunk.Row, inners chunk.Iterator, chk *chunk.Chunk, opt ...NAAJType) (matched bool, _ bool, err error) {
	...
	for inner := inners.Current(); inner != inners.End(); inner = inners.Next() {
		...
		valid, _, err := expression.EvalBool(expression.NewEvalContext(naal.ctx), naal.conditions, naal.shallowRow.ToRow())
		if err != nil {
			return false, false, err
		}
		if valid {
