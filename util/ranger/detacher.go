...
func allSinglePoints(sc *stmtctx.StatementContext, points []*point) []*point {
	...
	for i := 0; i < len(points); i += 2 {
		...
		cmp, err := left.value.Compare(sc.ValCtx, &right.value, collate.GetCollator(left.value.Collation()))
		if err != nil || cmp != 0 {
			return nil
		}
...
func isSameValue(sc *stmtctx.StatementContext, lhs, rhs *valueInfo) (bool, error) {
	...
	cmp, err := lhs.value.Compare(sc.ValCtx, rhs.value, collate.GetBinaryCollator())
	if err != nil {
		return false, err
	}
	return cmp == 0, nil
