...
func (r *builder) buildFromConstant(expr *expression.Constant) []*point {
	dt, err := expr.Eval(expression.NewDefaultEvalContext(), chunk.Row{})
	if err != nil {
		r.err = err
		return nil
	}
...
func (r *builder) buildFromBinOp(expr *expression.ScalarFunction) []*point {
	...
	if col, ok = expr.GetArgs()[0].(*expression.Column); ok {
		ft = col.RetType
		value, err = expr.GetArgs()[1].Eval(expression.NewDefaultEvalContext(), chunk.Row{})
		if err != nil {
			return nil
		}
		op = expr.FuncName.L
		...
		ft = col.RetType
		value, err = expr.GetArgs()[0].Eval(expression.NewDefaultEvalContext(), chunk.Row{})
		if err != nil {
			return nil
		}
...
func (r *builder) buildFromIn(expr *expression.ScalarFunction) ([]*point, bool) {
	...
	for _, e := range list {
		...
		dt, err := v.Eval(expression.NewDefaultEvalContext(), chunk.Row{})
		if err != nil {
...
func (r *builder) newBuildFromPatternLike(expr *expression.ScalarFunction) []*point {
	...
	pdt, err := expr.GetArgs()[1].(*expression.Constant).Eval(expression.NewDefaultEvalContext(), chunk.Row{})
	...
	edt, err := expr.GetArgs()[2].(*expression.Constant).Eval(expression.NewDefaultEvalContext(), chunk.Row{})
	if err != nil {
		r.err = errors.Trace(err)
		return getFullRange()
	}
