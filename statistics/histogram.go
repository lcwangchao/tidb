...
func (hg *Histogram) ConvertTo(sc *stmtctx.StatementContext, tp *types.FieldType) (*Histogram, error) {
	...
	for row := iter.Begin(); row != iter.End(); row = iter.Next() {
		...
		d, err := d.ConvertTo(sc.ValCtx, tp)
		if err != nil {
...
func MergeHistograms(sc *stmtctx.StatementContext, lh *Histogram, rh *Histogram, bucketSize int, statsVer int) (*Histogram, error) {
	...
	lh.NDV += rh.NDV
	lLen := lh.Len()
	cmp, err := lh.GetUpper(lLen-1).Compare(sc.ValCtx, rh.GetLower(0), collate.GetBinaryCollator())
	if err != nil {
		return nil, errors.Trace(err)
	}
	offset := int64(0)
...
func mergeBucketNDV(sc *stmtctx.StatementContext, left *bucket4Merging, right *bucket4Merging) (*bucket4Merging, error) {
	...
	upperCompare, err := right.upper.Compare(sc.ValCtx, left.upper, collate.GetBinaryCollator())
	if err != nil {
		return nil, err
	}
	// __right__|
	...
	if upperCompare == 0 {
		lowerCompare, err := right.lower.Compare(sc.ValCtx, left.lower, collate.GetBinaryCollator())
		if err != nil {
			return nil, err
		}
		//      |____right____|
	...
	lowerCompareUpper, err := right.lower.Compare(sc.ValCtx, left.upper, collate.GetBinaryCollator())
	if err != nil {
		return nil, err
	}
	//                  |_right_|
...
func mergePartitionBuckets(sc *stmtctx.StatementContext, buckets []*bucket4Merging) (*bucket4Merging, error) {
	...
	for i := len(buckets) - 1; i >= 0; i-- {
		...
		compare, err := buckets[i].upper.Compare(sc.ValCtx, res.upper, collate.GetBinaryCollator())
		if err != nil {
			return nil, err
		}
		if compare == 0 {
...
func MergePartitionHist2GlobalHist(sc *stmtctx.StatementContext, hists []*Histogram, popedTopN []TopNMeta, expBucketNumber int64, isIndex bool) (*Histogram, error) {
	...
	for _, hist := range hists {
		...
		if hist.Len() > 0 {
			...
			res, err := tmpValue.Compare(sc.ValCtx, minValue, collate.GetBinaryCollator())
			if err != nil {
				return nil, err
			}
			if res < 0 {
	...
	for _, meta := range popedTopN {
		...
		res, err := d.Compare(sc.ValCtx, minValue, collate.GetBinaryCollator())
		if err != nil {
			return nil, err
		}
		if res < 0 {
	...
	slices.SortFunc(buckets, func(i, j *bucket4Merging) int {
		res, err := i.upper.Compare(sc.ValCtx, j.upper, collate.GetBinaryCollator())
		if err != nil {
			sortError = err
		}
		if res != 0 {
		...
		res, err = i.lower.Compare(sc.ValCtx, j.lower, collate.GetBinaryCollator())
		if err != nil {
			sortError = err
		}
		return res
	...
	for i := len(buckets) - 1; i >= 0; i-- {
		...
		if sum >= totCount*bucketCount/expBucketNumber && sum-prevSum >= gBucketCountThreshold {
			for ; i > 0; i-- { // if the buckets have the same upper, we merge them into the same new buckets.
				res, err := buckets[i-1].upper.Compare(sc.ValCtx, buckets[i].upper, collate.GetBinaryCollator())
				if err != nil {
					return nil, err
				}
				if res != 0 {
