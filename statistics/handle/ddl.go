...
func (h *Handle) insertColStats2KV(physicalID int64, colInfos []*model.ColumnInfo) (err error) {
	...
	if sctx.GetSessionVars().StmtCtx.AffectedRows() > 0 {
		...
		for _, colInfo := range colInfos {
			...
			value, err = value.ConvertTo(sctx.GetSessionVars().StmtCtx.ValCtx, &colInfo.FieldType)
			if err != nil {
				return
			}
			if value.IsNull() {
				...
				value, err = value.ConvertTo(sctx.GetSessionVars().StmtCtx.ValCtx, types.NewFieldType(mysql.TypeBlob))
				if err != nil {
					return
				}
